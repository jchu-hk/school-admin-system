 ...
 *
 * UI 原型 Section 20: 学生门户
 *
 * 布局结构:
 * ┌──────────────────────────────────────────────────────────┐
 * │  PortalHeader (顶部导航: Logo + 学生姓名 + 角色标签 + 退出) │
 * ├──────────────┬───────────────────────────────────────────┤
 * │              │                                           │
 │  PortalSidebar │          Content Area                     │
 * │  (角色过滤菜单) │       (Outlet: 子页面)                    │
 * │              │                                           │
 * ├──────────────┴───────────────────────────────────────────┤
 ...
 *
 * 功能逻辑:
 * 1. 页面加载 → GET /portal/menus → 根据角色回传菜单配置
 * 2. GET /portal/student/profile → 获取学生姓名/班级
 * 3. 侧边菜单动态渲染，高亮当前路由
 * 4. 桌面 (≥768px): 左侧收缩式侧边栏 + 右侧内容区
 * 5. 移动 (<768px): 底部 Tab Bar + 全屏内容区
 *
 * React Router 路由嵌套:
 *   /portal/student         → 本框架 + StudentDashboard (默认)
 *   /portal/student/profile → T16 (个人档案)
 *   /portal/student/leave   → T17 (请假管理)
 *   /portal/student/attendance / notification / settings → 占位
 */
