 ...
 *
 * 对应后端端点:
 *   POST   /api/portal/leave                   → 为孩子创建请假申请（含 student_id）
 *   GET    /api/portal/leave?student_id=xxx     → 获取对应孩子的请假记录列表（分页）
 *   GET    /api/portal/leave/:id                → 获取请假详情
 *   PATCH  /api/portal/leave/:id/cancel         → 撤回请假（取消）
 *   GET    /api/portal/profile?student_id=xxx   → 获取孩子档案（脱敏）
 */
...
export class ParentLeaveApiError extends Error {
  constructor(
