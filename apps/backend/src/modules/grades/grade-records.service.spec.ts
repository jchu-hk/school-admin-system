...
describe('GradeRecordsService', () => {
  ...
  let _gradeRecordRepository: Repository<GradeRecord>;
  let _gradeReviewRepository: Repository<GradeReview>;
  let _alertRepository: Repository<GradeAuditAlert>;
  ...
  beforeEach(async () => {
    ...
    _gradeRecordRepository = module.get<Repository<GradeRecord>>(
      getRepositoryToken(GradeRecord),
    );
    _gradeReviewRepository = module.get<Repository<GradeReview>>(
      getRepositoryToken(GradeReview),
    );
    _alertRepository = module.get<Repository<GradeAuditAlert>>(
      getRepositoryToken(GradeAuditAlert),
    );
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException if record not found', async () => {
      ...
      await expect(service.findOne('record-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('update', () => {
    it('should update a draft grade record', async () => {
      ...
      mockGradeRecordRepository.findOne.mockResolvedValue(mockRecord);
      mockGradeRecordRepository.save.mockResolvedValue({
        ...mockRecord,
        ...dto,
      });
    ...
    it('should throw BadRequestException if record is not draft', async () => {
      ...
      await expect(service.update('record-1', {})).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('revoke', () => {
    ...
    it('should throw BadRequestException if revoke period expired', async () => {
      ...
      await expect(
        service.revoke('record-1', { reason: 'Test' }, 'teacher-1'),
