...
export class GradeRecordsService {
  ...
  async create(dto: CreateGradeRecordDto, userId: string): Promise<GradeRecord> {
  ...
  async findAll(query: QueryGradeRecordsDto) {
    ...
    if (query.studentId) qb.andWhere('gr.studentId = :studentId', { studentId: query.studentId })
    if (query.teacherId) qb.andWhere('gr.teacherId = :teacherId', { teacherId: query.teacherId })
    if (query.classId) qb.andWhere('gr.classId = :classId', { classId: query.classId })
    if (query.academicYear) qb.andWhere('gr.academicYear = :academicYear', { academicYear: query.academicYear })
    if (query.term) qb.andWhere('gr.term = :term', { term: query.term })
    if (query.status) qb.andWhere('gr.status = :status', { status: query.status })
    if (query.examName) qb.andWhere('gr.examName ILIKE :examName', { examName: `%${query.examName}%` })
  ...
  async submit(id: string, dto: SubmitGradeRecordDto, userId: string): Promise<GradeRecord> {
    const record = await this.findOne(id)
    ...
    if (record.teacherId !== userId) {
      throw new ForbiddenException('Only the teacher who created the record can submit it')
    }
  ...
  async revoke(id: string, dto: RevokeGradeRecordDto, userId: string): Promise<GradeRecord> {
    const record = await this.findOne(id)

    // 检查撤回条件
    if (record.status !== RecordStatus.PENDING_APPROVAL) {
      throw new BadRequestException('Only pending approval records can be revoked')
    }

    if (record.teacherId !== userId) {
      throw new ForbiddenException('Only the teacher who created the record can revoke it')
    }
  ...
  async approve(id: string, dto: ApproveGradeRecordDto, userId: string): Promise<GradeRecord> {
    const record = await this.findOne(id)

    if (record.status !== RecordStatus.PENDING_APPROVAL) {
      throw new BadRequestException('Only pending approval records can be approved')
    }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const review = manager.create(GradeReview, {
        ...
        level: nextLevel,
        comment: dto.comment,
        previousData: { approvalLevel: currentLevel, status: RecordStatus.PENDING_APPROVAL },
  ...
  async reject(id: string, dto: ApproveGradeRecordDto, userId: string): Promise<GradeRecord> {
    const record = await this.findOne(id)

    if (record.status !== RecordStatus.PENDING_APPROVAL) {
      throw new BadRequestException('Only pending approval records can be rejected')
    }
