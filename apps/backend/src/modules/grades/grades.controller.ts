...
export class GradesController {
  ...
  approveRecord(
    @Param('id') id: string,
    @Body() dto: ApproveGradeRecordDto,
    @Req() req: any,
  ) {
  ...
  async downloadPdf(
    ...
    const { filepath, filename } = await this.gradePdfService.downloadPdf(
      id,
      req.user.id,
    );
  ...
  async generateBatchPdf(
    ...
    return this.gradePdfService.generateBatchPdf(
      body.classId,
      body.academicYear,
      body.term,
      body.examName,
