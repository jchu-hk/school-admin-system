...
export class GradeAlertsService {
  ...
  async findAll(query: QueryAlertsDto) {
    ...
    if (query.severity) qb.andWhere('ga.severity = :severity', { severity: query.severity })
    if (query.status) qb.andWhere('ga.status = :status', { status: query.status })
    if (query.teacherId) qb.andWhere('ga.teacherId = :teacherId', { teacherId: query.teacherId })
  ...
  async findOne(id: string): Promise<GradeAuditAlert> {
    const alert = await this.alertRepository.findOne({
      where: { id },
      relations: ['teacher', 'gradeRecord', 'gradeReview', 'acknowledgedByUser'],
    })
    if (!alert) throw new Error('Alert not found')
    return alert
  }

  async acknowledge(id: string, dto: AcknowledgeAlertDto, userId: string): Promise<GradeAuditAlert> {
  ...
  async updateStatus(id: string, dto: UpdateAlertStatusDto, userId: string): Promise<GradeAuditAlert> {
    const alert = await this.findOne(id)
  ...
  async getOpenAlertsCount(userId: string): Promise<number> {
    // TODO: 根据用户角色筛选
