...
export class GradePdfService {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        record.subjects.forEach((subject) => {
          ...
          doc.text(
            String(subject.classRank),
            tableLeft + colWidths[0] + colWidths[1] + colWidths[2],
            rowY,
          );
          doc.text(
            String(subject.classAvg),
            tableLeft +
              colWidths[0] +
              colWidths[1] +
              colWidths[2] +
          ...
          if (subject.teacherComment) {
            ...
            doc
              .fontSize(9)
              .text(`评语: ${subject.teacherComment}`, tableLeft, rowY);
            doc.fontSize(10);
            rowY += 5;
          }
        ...
        doc
          .fontSize(9)
          .font('Helvetica')
          .text(`生成时间: ${new Date().toLocaleString('zh-HK')}`, {
            align: 'right',
          });
  ...
  async generateBatchPdf(
    ...
    _term: string,
    _examName: string,
  ): Promise<{ count: number; zipUrl: string }> {
