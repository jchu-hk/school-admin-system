...
export class CreateStudentProfilesTable1782530388000 implements MigrationInterface {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          {
            name: 'long_term_medication_notes',
            type: 'text',
            isNullable: true,
          },
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // Drop indexes
    await queryRunner.query(
      `DROP INDEX IF EXISTS IDX_student_profiles_current_grade;`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS IDX_student_profiles_is_archived;`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS IDX_student_profiles_enrollment_status;`,
    );

    // Drop foreign key
    await queryRunner.query(
      `ALTER TABLE student_profiles DROP CONSTRAINT IF EXISTS FK_student_profiles_student_id;`,
    );
