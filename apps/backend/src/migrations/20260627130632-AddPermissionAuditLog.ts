...
export class AddPermissionAuditLog20260627130632 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
        ...
        `);
    await queryRunner.query(
      `CREATE INDEX "IDX_operator_id" ON "permission_audit_logs" ("operator_id")`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_target_user_id" ON "permission_audit_logs" ("target_user_id")`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_audit_created_at" ON "permission_audit_logs" ("created_at")`,
    );
  }
