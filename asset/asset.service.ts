...
export class AssetService {
  ...
  async findAllAssets(query: AssetQueryDto): Promise<{
    ...
    const { page = 1, pageSize = 10, schoolId, category, status, location, keyword, isActive } = query;
  ...
  async findAllRentals(query: AssetRentalQueryDto): Promise<{
    ...
    const { page = 1, pageSize = 10, assetId, borrowerId, status, keyword } = query;
    ...
    if (keyword) {
      ...
      filtered = data.filter(
        (rental) =>
          ...
          (rental.borrowerDepartment &&
            rental.borrowerDepartment.toLowerCase().includes(lowerKeyword)) ||
          (rental.purpose && rental.purpose.toLowerCase().includes(lowerKeyword)),
      );
    }
