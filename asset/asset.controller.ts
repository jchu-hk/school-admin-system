...
export class AssetController {
  ...
  @ApiResponse({ status: 200, description: '获取租借记录详情成功', type: AssetRental })
  @Roles(UserRole.SYSTEM_ADMIN, UserRole.SCHOOL_DIRECTOR, UserRole.SCHOOL_STAFF)
  ...
  approveRental(
    ...
    const user = req.user as any;
    return this.assetService.approveRental(id, user?.sub, user?.name || 'System', dto);
  }
  ...
  @Roles(UserRole.SYSTEM_ADMIN, UserRole.SCHOOL_DIRECTOR, UserRole.SCHOOL_STAFF)
  returnAsset(@Param('id', ParseUUIDPipe) id: string, @Body() dto: ReturnRentalDto) {
  ...
  @Roles(UserRole.SYSTEM_ADMIN, UserRole.SCHOOL_DIRECTOR, UserRole.SCHOOL_STAFF)
  rejectRental(@Param('id', ParseUUIDPipe) id: string, @Body('note') note: string) {
