...
**问题**: 用户反馈发现学生管理存在根本性设计错误

| 缺陷 | 严重程度 |
|------|----------|
| 学生档案与系统用户混淆 | P0 CRITICAL |
| 学号未自动生成 | P0 |
| 基本信息缺失 | P0 |
| 班级下拉框无数据 | P0 |
| 新建记录不显示 | P1 |

**根本原因**: 当前 students 实际是 users 表中 role='student'，包含 username/password，这是概念错误

**正确设计**:
- students 表独立于 users 表
- 学号自动生成 (YYYYNNNN 格式)
- 班级分配按学年动态管理
...
**PR #195**: OPEN, CI ✅ PASSING (lint+build), E2E ❌ (测试环境断连)
**设计文件**: `docs/design/student-management-ui.html`

**CHECKER 审核结论**: ✅ 通过
- 学号自动生成 ✅
- 基本信息完整 ✅
- **无 username/password 字段** ✅
- 监护人信息区域 ✅
- 班级分配说明 ✅
...
**问题**: 为什么早期 QA/Checker 未发现这个根本性设计错误？

**分析**:
1. 设计评审覆盖率不足 - 早期设计文档可能未明确定义"学生档案≠系统用户"
2. QA 测试未覆盖实际用户场景 - 测试用例可能只验证字段存在，未验证业务概念正确性
3. 缺少真实用户参与 - 直到真人使用才暴露问题

**改进措施**:
- 设计文档需明确定义实体边界
- QA 验收需包含业务概念验证
- 重要功能需真实用户参与 UAT
- CHECKER 需关注业务逻辑正确性，不只是技术实现
