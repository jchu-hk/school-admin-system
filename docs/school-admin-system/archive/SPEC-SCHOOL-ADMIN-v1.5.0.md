...
> 批量录入出勤记录时，须先显示确认预览页，用户确认后方可保存。保存后5分钟内可批量撤销。

| 功能 | 说明 |
|------|------|
| 确认预览页 | 显示即将录入的记录摘要（学生数量、日期、状态类型统计）|
| 批量撤销 | 保存后5分钟内可一次性撤销全部记录（需确认），超出时限须逐条删除 |
| 撤销权限 | 仅录入人或校务主任可撤销 |
...
| 操作 | 触发条件 | 审批人 | 证明文件 |
|------|----------|--------|---------|
| 临时提升为校务主任 | 任何用户 | 校长 | 必须：人事任职通知 |
| 跨班级数据访问授权 | 教师跨班查看 | 校务主任 | 必须：活动通知/工作安排证明 |
| 数据导出权限授予 | 批量导出授权 | 校务主任 | 必须：数据使用说明 |
| 系统管理员权限变更 | SYSTEM角色变更 | 校长 + 校务主任 | 必须：权限变更申请单 |
| 家长账户关联学生解绑 | 账户异常 | 校务主任 | 必须：情况说明 |
...
### v1.5.0 — 2026-06-05 | 用户模拟反馈整合（USER-SIMULATION-FEEDBACK.md v1.0.0）
