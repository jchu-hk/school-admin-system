# 智能校务助理系统 — 完整功能规格书
## Smart School Admin AI System — Complete Functional Specification

---

### 📋 文档版本信息

| 字段 | 内容 |
|------|------|
| 文档名称 | 智能校务助理系统 — 完整功能规格书 |
| 文档编号 | SPEC-SCHOOL-ADMIN-001 |
| 当前版本 | **v2.0.0-draft.1** |
| 文档状态 | ~~已核准 (Approved)~~ **→ 变更中 (Change in Progress)** |
| 存放位置 | `/docs/school-admin-system/SPEC-COMPLETE.md` |
| 主维护人 | 系统架构团队 |
| 审批人 | 校务主任 / 项目经理 |

### 📌 版本修订规则

| 规则 | 说明 |
|------|------|
| **版本号格式** | `Major.Minor.Patch` (语义化版本) |
| **Major (主版本)** | 重大架构变更、功能模块增减、不兼容变更 |
| **Minor (次版本)** | 新增功能、模块扩展、字段变更（向后兼容）|
| **Patch (修订版)** | 文档修正、格式调整、错别字纠正（不影响功能）|
| **Changelog 粒度** | 每次正式变更必须记录，Patch 版本可选 |
| **审批要求** | Minor 及以上变更需项目经理/校务主任审批 |
| **分支策略** | 在 CI/CD 流程中维护，变更通过 Pull Request 合并 |

---

## 第一部分：需求规格说明书 (Requirements Specification)

### 1.1 系统概述

**项目名称：** 智能校务助理系统 (Smart School Admin AI System)
**项目目标：** 为香港中学校务处构建一个AI驱动的智能工作平台
**核心价值：**
- **自动化**：重复性工作的智能处理与提醒
- **整合化**：分散系统的数据统一管理
- **合规化**：确保符合香港教育局及隐私条例要求
- **协同化**：支持多同工协作与权限管理

### 1.2 用户角色

| 角色 | 职责范围 | 系统权限 |
|------|----------|----------|
| 校务主任 | 统筹管理、决策审批 | 全权限 |
| 校务处同工 | 日常执行、数据处理 | 操作权限 |
| 教师 | 查询、申请、填报 | 受限权限 |
| 家长/学生 | 查询、申请、缴费 | 门户权限 |

### 1.3 工作分类

#### 恒常性事务（按日/周/月循环）
- **每日必做**：处理学生迟到/早退记录、接听家长查询、分派校内通讯、统计午餐订饭人数、管理校车点到、处理突发请假及各类收费
- **每周/每月**：统计教师代课安排、更新全校通讯录、盘点文具仓存、整理校务处会议记录、处理水电网络等常规缴费

#### 周期性事务（按学年阶段推进）
- **开学前后（7-9月）**：协助编班及分发课本、处理转校/新生注册、回收各项同意书、购买学生保险
- **考试与成绩期（10-6月）**：协助报名DSE及校内试、印制/密封试卷、处理特别考试安排、发布成绩单
- **行政及人事**：协助教师招聘、更新EDB网上资料、管理校车/饭盒供应商合约
- **收生与升学**：协助中一自行分配学位面试、处理中六联招/JUPAS文件及推荐信
- **学年结束（6-8月）**：盘点及销毁旧文件、整理全校学生档案移交、结算各项杂费

#### 财务及资产（贯穿全年）
- **财务**：零用现金报销、学费/堂费点算、奖学金/车船津贴申请
- **资产**：校产条码盘点、租借场地管理、课本/冷气机等设备保养记录

---

## 第二部分：系统架构设计

### 2.1 技术架构

```
┌─────────────────────────────────────────────────────────────────────┐
│                         用户界面层 (Presentation Layer)              │
│  ╔══════════════╗  ┌──────────────┐  ┌──────────────╗              │
│  ║  admin-app   ║  │  飞书/钉钉    │  │  portal-app  ║              │
│  ║  (教职工后台) ║  │  (内部协作)   │  │ (QR考勤+门户) ║              │
│  ║  端口 8080    ║  └──────────────┘  ║  端口 8081    ║              │
│  ╚══════════════╝                    ╚══════════════╝              │
│  Web UI (React) · SPA 独立部署                                       │
└─────────────────────────────────────────────────────────────────────┘
                                    │
┌─────────────────────────────────────────────────────────────────────┐
│                         AI 核心引擎层 (AI Core Engine)               │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐              │
│  │  自然语言处理  │  │   流程自动化   │  │   数据分析    │              │
│  │    (NLP)     │  │  (Workflow)  │  │ (Analytics)  │              │
│  └──────────────┘  └──────────────┘  └──────────────┘              │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐              │
│  │   文档识别    │  │   提醒通知    │  │   知识库     │              │
│  │    (OCR)     │  │  (Reminder)  │  │ (Knowledge)  │              │
│  └──────────────┘  └──────────────┘  └──────────────┘              │
└─────────────────────────────────────────────────────────────────────┘
                                    │
┌─────────────────────────────────────────────────────────────────────┐
│                      集成与数据层 (Integration & Data)               │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ │
│  │ WebSAMS  │ │  eClass  │ │ SchoolApp│ │  考勤系统  │ │  财务系统  │ │
│  │ (EDB)    │ │          │ │ (缴费)   │ │          │ │          │ │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘ │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │                  校务数据库 (学校私有部署)                     │ │
│  │         PostgreSQL + Redis + 加密文件存储                     │ │
│  └──────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.2 数据模型设计

#### 核心实体关系

```
┌─────────────┐       ┌─────────────┐       ┌─────────────┐
│   学生 Student │◄─────►│   班级 Class  │◄─────►│   教师 Teacher │
└──────┬──────┘       └──────┬──────┘       └──────┬──────┘
       │                     │                     │
       ▼                     ▼                     ▼
┌─────────────┐       ┌─────────────┐       ┌─────────────┐
│   出勤记录    │       │   课程表     │       │   代课记录    │
│  Attendance │       │   Timetable │       │   Substitute│
└─────────────┘       └─────────────┘       └─────────────┘

┌─────────────┐       ┌─────────────┐       ┌─────────────┐
│   事务任务    │◄─────►│   工作流程    │◄─────►│   提醒通知    │
│    Task     │       │   Workflow  │       │   Reminder  │
└─────────────┘       └─────────────┘       └─────────────┘

┌─────────────┐       ┌─────────────┐       ┌─────────────┐
│   财务记录    │       │   资产项目    │       │   供应商     │
│   Finance   │       │    Asset    │       │   Vendor    │
└─────────────┘       └─────────────┘       └─────────────┘
```

> 📘 **详细数据库架构设计**，请参阅：
> - `DB-SCHEMA.md` — 完整表结构定义、索引设计、外键关系
> - `DATA-DICTIONARY.md` — 数据项定义、枚举值、业务规则、数据质量规范

---

## 第三部分：Module 1 — 每日晨检仪表板

### 3.1 模块概述

| 属性 | 描述 |
|------|------|
| 模块名称 | Daily Operations Dashboard (每日晨检仪表板) |
| 模块ID | MOD-DAILY-001 |
| 优先级 | P0 (关键) |
| 用户 | 校务处同工, 校务主任 |
| 使用频率 | 每日, 7:00 AM - 9:00 AM 高峰期 |

---

### Function F-DASH-001: 仪表板主视图

**输入：** 无（默认显示当日数据）

**仪表板自定义功能（用户模拟反馈A-05 — 新增）：**
> 用户可按个人偏好调整仪表板的组件显示、顺序和筛选条件，配置保存在用户个人设置中。

| 功能 | 说明 | 适用角色 |
|------|------|---------|
| 组件显示/隐藏 | 可选择显示或隐藏特定组件 | 全部 |
| 组件拖拽排序 | 拖拽调整组件位置，保存后生效 | 全部 |
| 紧急程度筛选 | 对待处理任务按「紧急/一般/低」筛选（用户模拟反馈B-09）| 校务处同工 |
| 默认日期范围 | 可设置默认显示日期范围（如：本月/本周/今日）| 全部 |

> **实现说明：** 用户配置存储在用户偏好表（`user_preferences`），不影响系统默认配置。管理员可强制固定某些组件不可隐藏。

**输出：**
```json
{
  "date": "2026-05-23",
  "weekday": "星期五",
  "components": [
    {"name": "出勤率", "value": "98.75%", "status": "normal"},
    {"name": "迟到/早退", "value": "5人", "status": "warning"},
    {"name": "家长查询", "value": "3个待处理", "status": "urgent"},
    {"name": "校车状态", "value": "全部到达", "status": "normal"},
    {"name": "午餐订单", "value": "980份", "status": "confirmed"},
    {"name": "请假申请", "value": "2个待处理", "status": "pending"}
  ]
}
```

---

### Function F-ATT-001: 学生出勤概览

**输入：**
| 字段 | 类型 | 来源 | 描述 |
|------|------|------|------|
| date | Date | System | 当前日期(默认)或选择日期 |
| class_filter | Array[String] | 用户选择 | 可选：按班级筛选 |
| sync_source | Enum | System | "eClass", "manual", "biometric" |

**处理流程：**

```
Step 1: 数据拉取 (Data Ingestion)
Step 2: 数据清洗与标准化 (Data Cleansing)
Step 3: 聚合汇总 (Aggregation)
Step 4: 异常检测 (Anomaly Detection)
Step 5: 预警标记 (Alert Tagging)
Step 6: 响应组装 (Response Assembly)
```

**数据同步状态展示（用户模拟反馈B-01 — 新增）：**
> 每个数据源（eClass API、门禁刷卡机、人脸识别闸机）须独立显示同步状态，不合并为"部分成功"。

| 数据源 | 状态 | 显示内容 |
|--------|------|---------|
| eClass API | ✅ 成功 / ❌ 失败 / ⚠️ 部分成功 | 最后同步时间 + 记录数 |
| 门禁刷卡机（每台设备） | ✅ 正常 / ❌ 离线 | 设备名称 + 最后同步时间 |
| 人脸识别闸机（每台设备） | ✅ 正常 / ❌ 离线 | 设备名称 + 最后同步时间 |

**受影响学生智能列表（用户模拟反馈B-01）：**
> 当任一数据源同步失败时，系统自动生成「受影响学生列表」，显示：
- 学生姓名、班级
- 所属数据源/设备
- 建议操作（「确认到校」/「标记待确认」）

```json
{
  "affected_students": [
    {
      "student_id": "2023S10123",
      "name": "陳大明",
      "class": "1A",
      "affected_sources": ["人脸识别闸机-RFID-003"],
      "suggested_action": "confirm_present",
      "last_known_status": "unknown"
    }
  ]
}
```

**批量录入确认预览 + 批量撤销（用户模拟反馈B-02 — 新增）：**
> 批量录入出勤记录时，须先显示确认预览页，用户确认后方可保存。保存后15分钟内可批量撤销。

| 功能 | 说明 |
|------|------|
| 确认预览页 | 显示即将录入的记录摘要（学生数量、日期、状态类型统计）|
| 批量撤销 | 保存后**15分钟**内可一次性撤销全部记录（需二次确认），超出时限须逐条删除 |
| 撤销权限 | 仅录入人或校务主任可撤销 |

#### Step 1: 数据拉取 (Data Ingestion)

本步骤从多个数据源实时/定时拉取出勤原始数据，支持以下来源：

**1.1 eClass API（首选数据源）**

| 接口端点 | 方法 | 描述 | 数据量 |
|----------|------|------|--------|
| `/api/attendance/students/{class_id}/{date}` | GET | 按班级和日期获取出勤记录 | 每班约30-40人 |
| `/api/attendance/student/{student_id}/history` | GET | 获取单个学生出勤历史 | 单个学生 |
| `/api/class/{class_id}/students` | GET | 获取班级学生列表 | 每班约30-40人 |
| `/api/attendance/sync` | POST | 批量同步出勤数据 | 每次最多500条 |

**API 调用示例：**
```bash
# 获取指定班级某日的出勤数据
curl -X GET "https://school.eclass.com/api/attendance/students/1A/2026-05-27" \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json"

# 响应：
{
  "class_id": "1A",
  "date": "2026-05-27",
  "records": [
    {
      "student_id": "2023S10101",
      "student_name": "陳小明",
      "status": "present",
      "check_in_time": "07:58:32",
      "check_in_method": "card",
      "device_id": "RFID-001"
    },
    {
      "student_id": "2023S10102",
      "student_name": "李小红",
      "status": "late",
      "check_in_time": "08:12:15",
      "check_in_method": "card",
      "device_id": "RFID-001"
    }
  ],
  "summary": {
    "total": 38,
    "present": 35,
    "absent": 1,
    "late": 2
  }
}
```

**1.2 人工录入（Backup）**

当系统接口不可用或数据异常时，校务处同工可通过 Web 表单手动录入：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| class_id | String | 是 | 班级代码 |
| date | Date | 是 | 出勤日期 |
| student_id | String | 是 | 学号 |
| status | Enum | 是 | present / absent / late / excused |
| check_in_time | Time | 条件 | 到校时间（迟到时必填）|
| reason | Text | 条件 | 缺席/迟到原因（缺席/迟到时必填）|
| document | File | 可选 | 证明文件（医生证明等）|

**表单布局：**
```
┌──────────────────────────────────────────────────────────┐
│  人工录入出勤记录                                      │
├──────────────────────────────────────────────────────────┤
│  班级:    [全部 ▼]   日期:    [2026-05-27 📅]        │
│                                                      │
│  ┌────────────────────────────────────────────────┐  │
│  │ 学号    │ 姓名   │ 状态      │ 到校时间 │ 备注 │  │
│  ├────────────────────────────────────────────────┤  │
│  │ S00101 │ 陳小明 │ ○出席  ●迟到 ○缺席 ○请假 │ 08:02 │ [  ] │  │
│  │ S00102 │ 李小红 │ ●出席  ○迟到 ○缺席 ○请假 │ 07:55 │ [  ] │  │
│  │ S00103 │ 王小華 │ ○出席  ○迟到 ●缺席 ○请假 │  --   │ [病假] │  │
│  └────────────────────────────────────────────────┘  │
│                                                      │
│  [批量导入 Excel]          [保存]  [取消]           │
└──────────────────────────────────────────────────────────┘
```

**1.3 生物识别设备（实时数据源）**

| 设备类型 | 品牌/型号 | 数据格式 | 实时性 |
|----------|----------|----------|--------|
| 门禁刷卡机 | 智慧卡考勤机 | JSON over HTTP | 准实时（<30s延迟）|
| 人脸识别闸机 | 海康/大华人脸识别 | JSON over HTTP | 准实时（<5s延迟）|
| 移动考勤App | 学校自研App | REST API | 实时 |

**Webhook 配置示例：**
```json
{
  "webhook_url": "https://school-admin.internal/api/attendance/webhook",
  "events": ["check_in", "check_out", "anomaly"],
  "auth": {
    "type": "hmac_sha256",
    "secret": "{webhook_secret}"
  },
  "retry": {
    "max_attempts": 3,
    "interval_seconds": 5
  }
}
```

**1.4 数据拉取策略**

| 策略 | 触发条件 | 执行时间 | 适用场景 |
|------|----------|---------|----------|
| **实时同步** | Webhook推送触发 | 事件驱动 | 门禁/刷卡/刷脸 |
| **定时拉取** | 每日定时任务 | 07:00 AM / 15:30 PM | eClass系统同步 |
| **按需拉取** | 用户查询时触发 | 实时 | 历史数据补录 |
| **批量导入** | 人工上传文件 | 手动触发 | 数据异常恢复 |

**定时任务配置（例）：**
```yaml
attendance_sync:
  schedule: "0 7,15 * * *"  # 每天7:00和15:00执行
  sources:
    - name: eclass_api
      priority: 1
      timeout: 30s
      retry: 3
    - name: biometric_backup
      priority: 2
      timeout: 10s
      retry: 2
  merge_strategy: "latest_wins"  # 以最新记录为准
  alert_on_failure: true
```

#### Step 2: 数据清洗与标准化 (Data Cleansing)

| 处理规则 | 说明 | 示例 |
|----------|------|------|
| **去重** | 同一学生同一天多条记录，保留最新 | 刷卡2次→保留最终状态 |
| **状态优先** | excused > present > late > early > absent | 有请假条优先于缺席 |
| **时间标准化** | 统一为 Asia/Hong_Kong 时区 | UTC+0 → UTC+8 |
| **格式校验** | 校验学号格式、日期格式 | 学号应为YYYY+S+NNN格式 |
| **异常值过滤** | 过滤明显错误记录 | 时间在06:00前或21:00后 |

#### Step 3: 聚合汇总 (Aggregation)

```python
# 伪代码：出勤聚合计算
def aggregate_attendance(records, group_by="class"):
    summary = {
        "total_students": len(records),
        "present": records.filter(status="present").count(),
        "absent": records.filter(status="absent").count(),
        "late": records.filter(status="late").count(),
        "early": records.filter(status="early").count(),
        "excused": records.filter(status="excused").count(),
        "attendance_rate": f"{(present / total * 100):.2f}%"
    }
    if group_by == "grade":
        return group_by_grade(records)
    elif group_by == "class":
        return group_by_class(records)
    return summary
```

#### Step 4: 异常检测 (Anomaly Detection)

| 异常类型 | 检测规则 | 预警级别 |
|----------|----------|----------|
| 连续缺席 | 同一学生连续缺席≥3天 | medium |
| 连续迟到 | 7天内迟到≥3次 | medium |
| 突发缺席 | 过去30天缺席率<5%，今日突然>20% | high |
| 班级异常 | 班级出勤率低于全校平均值2个标准差 | low |
| 模式逃逸 | 特定学生连续多天同一时间段缺席 | high |

#### Step 5: 预警标记 (Alert Tagging)

| alert_level | 触发条件 | 通知对象 | 通知方式 |
|-------------|----------|----------|----------|
| `high` | 连续缺席≥5天 / 突发异常 | 校务主任 + 班主任 + 家长 | 短信 + App推送 |
| `medium` | 连续缺席3-4天 / 连续迟到3次/7天 | 班主任 + 家长 | App推送 |
| `low` | 单次异常 / 班级出勤偏低 | 校务处 | 系统通知 |
| `none` | 正常出勤 | 无 | 无 |

**输出：**
```json
{
  "attendance_summary": {
    "date": "2026-05-23",
    "total_students": 1200,
    "present": 1185,
    "absent": 10,
    "late": 5,
    "attendance_rate": "98.75%"
  },
  "by_grade": [
    {
      "grade": "S1",
      "total": 200,
      "present": 195,
      "absent": 3,
      "late": 2
    }
  ],
  "anomalies": [
    {
      "student_id": "2023S10123",
      "name": "陳大文",
      "class": "1A",
      "pattern": "consecutive_late",
      "days": 3,
      "alert_level": "medium"
    }
  ],
  "requires_action": true
}
```


#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 开学日早上7:00，系统已同步eClass API出勤数据 | 校务处同工登录系统，打开出勤概览页面 | 系统显示当日全校出勤率、各班出勤明细，无异常告警 |
| AC-02 | eClass API同步失败（门禁设备离线）| 系统自动检测到同步失败 | 仪表板出勤组件显示⚠️警告标记，「受影响学生」列表显示受影响的5名学生姓名及所属设备 |
| AC-03 | 学生王小明今日刷卡到校，状态为"present" | 校务处同工查询1A班出勤详情 | 列表中王小明显示状态=出席(present)，到校时间=07:55 |
| AC-04 | 连续缺席≥3天的学生存在异常 | 系统检测到连续缺席告警 | 异常学生记录出现在「异常检测」区域，告警级别=medium，通知已发送给班主任 |
| AC-05 | 校务处同工手动录入出勤记录（5名学生）| 点击「保存」前显示确认预览页 | 预览页显示：班级1B、日期2026-05-23、状态类型统计（出席4人、迟到1人），点击「确认」后保存 |
| AC-06 | 批量录入保存后15分钟内 | 录入人发现自己录错，取消全部记录 | 点击「批量撤销」→ 确认撤销 → 5条记录全部删除，状态回退 |
| AC-07 | 批量录入保存后超过15分钟 | 录入人发现自己录错，尝试批量撤销 | 系统拒绝批量撤销，提示「已超过15分钟撤销时限，请逐条删除或联系校务主任」 |


---

### Function F-ATT-002: 迟到/早退记录

**输入：**
| 字段 | 类型 | 必填 | 描述 |
|------|------|------|------|
| student_id | String | 是 | 学生证号或刷卡 |
| type | Enum | 是 | "late_arrival" / "early_departure" |
| timestamp | DateTime | 自动 | 记录时间(默认当前) |
| reason | String | 可选 | 原因类别或自由文本 |
| parent_notified | Boolean | 自动 | 系统追踪通知 |

**业务规则：**
- 迟到阈值：8:00 AM后（可配置）
- 早退阈值：3:30 PM前（可配置）
- 自动家长通知：迟到>30分钟或无故
- 模式检测：7天内3+次触发辅导员预警

**输出：**
```json
{
  "record_id": "REC-20260523-001",
  "student": {
    "id": "2023S10123",
    "name": "陳大文",
    "class": "1A"
  },
  "type": "late_arrival",
  "timestamp": "2026-05-23T08:15:00+08:00",
  "duration_minutes": 15,
  "reason_category": "traffic",
  "parent_notified": true,
  "notification_sent_at": "2026-05-23T08:16:00+08:00",
  "pattern_alert": {
    "triggered": true,
    "consecutive_count": 3,
    "recommended_action": "contact_parent"
  }
}
```

---

### Function F-INQ-001: 家长查询队列管理

**输入（用户模拟反馈B-05 — 新增通话质量字段）：**
| 字段 | 类型 | 来源 | 描述 |
|------|------|------|------|
| channel | Enum | System | "phone", "email", "whatsapp", "in_person", "app" |
| parent_info | Object | 输入 | 姓名、联系方式、学生ID |
| query_content | String | 输入/音频 | 文本或转录语音 |
| urgency | Enum | AI/自动 | "low", "medium", "high", "critical" |
| call_duration_minutes | Integer | 可选（电话渠道建议填写）| 通话时长（分钟）|
| call_result | Enum | 可选（电话渠道建议填写）| `resolved` / `follow_up_required` / `escalated` |
| parent_satisfaction | Enum | 可选 | `positive` / `neutral` / `negative` |

**AI处理：**
1. **意图分类：** 归类查询类型
   - 校车时间 (bus_schedule)
   - 午膳餐单 (lunch_menu)
   - 费用查询 (fee_inquiry)
   - 请假 (absence_report)
   - 一般行政 (general_admin)
   - 投诉 (complaint)
2. **情感分析：** 检测紧急程度
3. **自动回复生成：** 匹配FAQ数据库
4. **路由分配：** 分配给适当人员或自动回复

#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 家长致电校务处查询小明今日校车情况 | 校务处同工在家长查询队列看到该来电，标记为phone渠道 | 系统记录来电渠道=phone，通话结束后弹出通话记录表单（通话时长、结果、家长情绪为必填项）|
| AC-02 | 家长WhatsApp消息咨询午膳餐单 | 系统接收WhatsApp消息，AI自动识别意图为lunch_menu | AI分析intent=lunch_menu，urgency=low，自动匹配FAQ模板「午膳餐单」，建议回复可一键发送 |
| AC-03 | 家长通过微信门户提交校车延误投诉（情绪激动）| AI分析该消息内容 | sentiment=negative，urgency=medium，触发高优先级标记，自动升级至校务主任处理 |
| AC-04 | 家长查询队列中有多条pending状态的查询（>10分钟未处理）| 系统例行检查 | 超时标记warning，推送提醒至校务处同工App |
| AC-05 | 校务处同工使用快速回复模板回复家长 | 选择「校車延誤通知」模板，填写延误时间=15分钟，点击发送 | 系统生成回复内容（包含延误时间、原因、预计到校时间），通过微信/短信/邮件发送，家长收到通知 |
| AC-06 | 家长查询涉及跨部门问题（如校车+午膳）| 校务处同工标记为需要转交 | 系统支持将该查询转交给其他部门同事，原始记录保留处理历史 |
| AC-07 | 家长通过App提交文字查询（学校假期安排）| AI分析intent=general_admin | 系统匹配合适FAQ回复，自动回复家长（无需人工处理），query_status=auto_replied |
| AC-08 | 家长通过电话查询，沟通内容涉及敏感信息（如家庭情况）| 校务处同工记录通话内容 | 通话记录仅记录元数据（时长、结果、情绪），不记录敏感内容摘要，PDPO合规 |


**输出：**
```json
{
  "inquiry_id": "INQ-20260523-089",
  "timestamp": "2026-05-23T08:30:00+08:00",
  "parent": {
    "name": "黃家長",
    "phone": "9123xxxx",
    "student_name": "黃小明",
    "student_class": "2B"
  },
  "channel": "phone",
  "original_query": "我想問下今日校車會唔會遲？",
  "ai_analysis": {
    "intent": "bus_schedule_inquiry",
    "sentiment": "neutral",
    "urgency": "medium",
    "confidence": 0.94
  },
  "suggested_response": "今日校車按正常時間行駛，第一班約7:45到達學校。如有臨時改動，會透過App通知家長。",
  "auto_response_eligible": true,
  "status": "pending_response",
  "escalation_required": false
}
```

---

### Function F-INQ-002: 快速回复模板

**模板分类：**

| 类别 | 模板数量 | 示例 |
|------|----------|------|
| 校車 | 8 templates | "校車延誤通知", "臨時改道通知" |
| 午膳 | 6 templates | "餐單查詢", "特殊飲食安排" |
| 收费 | 10 templates | "學費繳納方式", "雜費明細" |
| 请假 | 5 templates | "病假程序", "事假申請" |
| 一般 | 12 templates | "校曆查詢", "聯絡方式" |

---

### Function F-LUNCH-001: 午膳订单汇总

**输入：**
| 字段 | 类型 | 来源 | 描述 |
|------|------|------|------|
| order_date | Date | System | 目标日期 |
| vendor_filter | Array | 可选 | 按供应商筛选 |
| status_filter | Enum | 可选 | "confirmed", "pending", "cancelled" |

**家长自助午膳变更（用户模拟反馈B-03/D-02 — 新增）：**
> 家长可通过微信门户自助提交午膳变更申请（如更改款式、加单、取消），无需通过电话联系校务处。

| 变更类型 | 受理条件 | 系统行为 |
|----------|---------|---------|
| 加单 | 截止时间前 | 自动进入待确认队列，校务处审批 |
| 取消 | 截止时间前 | 自动确认，取消订单 |
| 更改款式 | 截止时间前 | 自动进入待确认队列，校务处审批 |

**更改截止时间规则（用户模拟反馈B-08 — 新增）：**
> 午膳变更设有统一截止时间，超过截止时间后系统自动拒绝并引导家长联系校务处。

| 规则 | 说明 |
|------|------|
| 默认截止时间 | 每日下午 14:00（可按供应商配置）|
| 超时提示 | 系统显示「更改截止时间为下午2:00，已过。请联系学校」，并内嵌「致电校务处」和「发送消息」快捷按钮 |
| 通知提醒 | 系统在截止时间前1小时推送微信消息提醒家长 |

**输出：**
```json
{
  "order_date": "2026-05-23",
  "summary": {
    "total_orders": 980,
    "confirmed": 950,
    "pending": 30
  },
  "by_vendor": [
    {
      "vendor_id": "V001",
      "vendor_name": "陽光膳食",
      "total": 450,
      "meal_types": {
        "regular": 420,
        "vegetarian": 25,
        "special_diet": 5
      }
    }
  ],
  "prediction_tomorrow": {
    "estimated_total": 975,
    "confidence": "high",
    "recommended_prep": 1000
  },
  "vendor_report_pdf": "https://.../report_20260523.pdf"
}
```

---

### Function F-BUS-001: 校车实时追踪

**输入：**
| 字段 | 类型 | 来源 | 描述 |
|------|------|------|------|
| bus_id | String | GPS | 校车标识 |
| timestamp | DateTime | GPS | 最后更新 |
| location | GeoJSON | GPS | 经纬度坐标 |
| route_id | String | System | 分配路线 |

**输出（用户模拟反馈A-04/B-07/D-04 — 增强）：**
```json
{
  "bus_id": "BUS-A1",
  "route": "將軍澳線",
  "status": "in_transit",
  "current_location": {
    "lat": 22.3085,
    "lng": 114.2632,
    "address": "寶琳路近將軍澳隧道"
  },
  "schedule": {
    "estimated_arrival": "2026-05-23T07:42:00+08:00",
    "scheduled_arrival": "2026-05-23T07:45:00+08:00",
    "variance_minutes": -3
  },
  "students_onboard": 45,
  "next_stop": "寶琳邨",
  "next_stop_eta": "2026-05-23T07:38:00+08:00",
  "alert_status": "normal",
  "delay_reason": "東涌公路交通意外",
  "incident_type": "minor_accident"
}
```


#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 校车BUS-A1正常行驶，GPS数据正常推送 | 家长登录微信门户，点击校车追踪 | 显示校车实时位置（地图标注）、预计到校时间、下一站信息 |
| AC-02 | 校车BUS-A1因东涌公路交通意外延误15分钟（>阈值）| 系统检测到延误超过阈值 | 系统自动向全部乘搭学生家长发送微信+短信延误通知，包含：延误原因+意外类型标签+预计到达时间 |
| AC-03 | 校车BUS-A1延误仅8分钟（未超阈值>10分钟）| 家长打开校车追踪页面 | 详情页显示实时位置，但**不发送**微信/App推送通知，家长可主动查看 |
| AC-04 | 校车到校后GPS确认到达校门 | 系统自动检测 | 自动向家长发送「孩子已安全到校」确认通知，无需手动操作 |
| AC-05 | 校车行程结束，校务处查看乘搭学生列表 | 校务处同工打开校车详情页 | 显示完整乘搭学生名单，点击「全部通知」-> 一次性发送微信推送至全部家长 |
| AC-06 | 校车路线需临时变更（道路施工）| 校务处同工手动更新校车路线 | 系统更新路线信息，家长端实时显示新路线预计到达时间 |


**乘搭学生列表（用户模拟反馈A-04 — 新增）：**
> 校车详情页显示完整乘搭学生名单，支持一键通知所有相关家长。

| 信息项 | 内容 |
|--------|------|
| 乘搭学生 | 姓名、班级、家长联系方式（隐藏）|
| 通知状态 | 每位学生的通知状态（已通知/未通知/家长已读）|
| 一键通知 | 点击「全部通知」→ 系统自动发送微信/App推送至全部相关家长 |

**延误通知增强（用户模拟反馈D-04 — 新增）：**
> 延误通知须包含意外类型标签和家长最关心的安全信息。

| 通知节点 | 内容 |
|----------|------|
| 延误发生时 | 预计延误时间 + 原因 + 意外类型标签 + 孩子已安全登车确认 |
| 到校确认 | 孩子已安全到达学校 |
| 意外类型标签 | `traffic_jam`（交通拥堵）/ `minor_accident`（轻微事故）/ `serious_accident`（严重事故）|

**延误通知阈值配置（评审报告修正 — 业务逻辑不合理#4）：**
> 为避免轻微延误（<10分钟）造成信息过载，系统须支持可配置的延误通知阈值。

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| 延误通知阈值 | >10分钟 | 仅延误超过此阈值才发送微信/App通知家长 |
| 短信备用阈值 | >20分钟 | 延误超过此阈值同时发送短信备用 |
| 阈值配置入口 | 校务主任后台 → 系统配置 → 校车管理 | 可按校车路线单独配置 |
| 阈值说明 | 轻微延误（<10分钟）不入通知，仅在详情页显示 | 家长可在App内主动查看实时校车位置 |

---

### Function F-BUS-002: 校车点大名记录

**输入：**
| 字段 | 类型 | 必填 | 描述 |
|------|------|------|------|
| student_id | String | 是 | 学生卡扫描 |
| bus_id | String | 是 | 校车标识 |
| check_type | Enum | 是 | "onboard", "alight" |
| timestamp | DateTime | 自动 | 签到时间 |
| location | String | 自动 | GPS或手动 |

**输出：**
```json
{
  "checkin_id": "CHK-20260523-456",
  "student_id": "2023S10123",
  "student_name": "陳大文",
  "bus_id": "BUS-A1",
  "check_type": "alight",
  "timestamp": "2026-05-23T07:43:15+08:00",
  "location": "School Main Gate",
  "status": "arrived_safely",
  "parent_notification_sent": true
}
```

---

### Function F-LEAVE-001: 请假申请处理

**输入：**
| 字段 | 类型 | 必填 | 描述 |
|------|------|------|------|
| student_id | String | 是 | 学生标识 |
| leave_type | Enum | 是 | "sick", "personal", "family", "other" |
| start_date | Date | 是 | 请假开始 |
| end_date | Date | 是 | 请假结束 |
| reason | String | 可选 | 详细原因 |
| parent_name | String | 是 | 联系家长 |
| contact_phone | String | 是 | 紧急联络 |
| supporting_doc | File | 可选 | 证明文件 |

**业务规则（用户模拟反馈C-01 — 新增代课老师课时数显示）：**
- 病假>2天需提交医疗证明
- 事假需提前24小时审批
- 紧急请假可事后补办（当天）
- 模式检测：每学期>5天病假触发福利检查
- **病假自动批准风险管控（评审报告修正 — 业务逻辑不合理#2）：** 有医生证明的病假申请虽可自动标记为「已提交」，但**系统须随机抽取5%-10%进行AI辅助核验**，对高风险案例（如同一天多名学生提交、重复使用同一诊所证明等）触发人工复查，由校务主任确认后方正式批准，以防伪造证明

**代课老师推荐增强（用户模拟反馈C-01）：**
> 推荐代课老师时，须显示每位候选人的当日已有课时数，避免重复安排。

| 推荐字段 | 说明 |
|----------|------|
| 候选人姓名 | 显示教师姓名 |
| 任教科目 | 显示所教科目的学科 |
| 当日已排课时数 | **新增：** 显示该教师当日已有课时数（如"今日已有4节课"）|

**跟进提醒设置（用户模拟反馈B-06 — 新增）：**
> 连续病假等需要跟进的场景，处理人员可在系统中设定下次跟进提醒日期和内容。

| 字段 | 类型 | 说明 |
|------|------|------|
| follow_up_date | Date | 下次跟进日期 |
| follow_up_content | String | 跟进内容摘要 |
| follow_up_reminder | Boolean | 是否设置系统提醒 |

**OCR进度提示（用户模拟反馈D-01 — 新增）：**
> 家长上传医生证明后，系统须显示明确的处理进度，避免重复上传。

| 状态 | 显示内容 |
|------|---------|
| 上传中 | 进度条 + 「上传中，请稍候...」|
| 识别中 | 进度条 + 「正在识别证明文件，请稍候...」|
| 识别完成 | 「识别成功，正在处理...」|
| 处理完成 | 「请假申请已提交」|

**审批进度状态条（用户模拟反馈C-07 — 新增）：**
> 教师提交请假申请后，可实时追踪审批进度。

| 状态节点 | 说明 |
|----------|------|
| `submitted` | 已提交，等待审批 |
| `pending_approval` | 审批中（显示当前审批人）|
| `approved` | 已批准 |
| `rejected` | 已拒绝（显示拒绝原因）|

**输出：**
```json
{
  "application_id": "LEAVE-20260523-012",
  "student": {
    "id": "2023S10123",
    "name": "陳大文",
    "class": "1A"
  },
  "leave_details": {
    "type": "sick",
    "start_date": "2026-05-23",
    "end_date": "2026-05-23",
    "days": 1,
    "reason": "發燒及感冒"
  },
  "status": "approved",
  "parent_contact": "陳家長 9123xxxx",
  "notifications_sent": {
    "class_teacher": true,
    "subject_teachers": 8,
    "admin_officer": true
  },
  "medical_cert_required": false,
  "follow_up_needed": false
}
```


#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 学生王小明因发烧需请假1天，家长提交请假申请（无医生证明）| 家长在微信门户提交病假申请，附言"发烧" | 系统生成请假记录，status=pending_approval，等待校务处审批 |
| AC-02 | 学生王小明因发烧需请假3天，家长提交请假申请（有医生证明）| 家长上传医生证明PDF，系统OCR识别 | OCR识别完成，显示进度提示，系统标记medical_cert_required=true，提交至审批队列 |
| AC-03 | 某日多名学生（>=3人）同时提交同一诊所的医生证明 | 系统AI核验队列随机抽取核验 | 高风险案例自动标记，由校务主任人工复查确认后方正式批准 |
| AC-04 | 请假申请已提交，等待审批中 | 教师在请假审批页面查看进度 | 审批进度状态条显示：submitted -> pending_approval（审批中：张主任）|
| AC-05 | 请假申请需要跟进（连续病假）| 校务处同工在处理请假时设置跟进提醒 | 系统记录跟进日期和内容，到期前1天自动推送App提醒 |
| AC-06 | 请假申请审批通过 | 系统自动发送通知 | 班主任收到App推送确认，所有任课教师收到调课通知 |


---

### Function F-FEE-001: 每日收费追踪

**输入：**
| 字段 | 类型 | 来源 | 描述 |
|------|------|------|------|
| date | Date | System | 收取日期 |
| fee_type | Enum | 筛选 | "air_con", "activity", "material", "other" |
| payment_method | Enum | 筛选 | "cash", "cheque", "fps", "octopus", "e_payment" |

**业务规则：**
- 现金处理需双人见证核实
- 每笔交易必须出具收据
- 关门前进每日对账
- 差异>$50需调查

**电子收据自动推送（用户模拟反馈B-10 — 新增）：**
> 非现金缴费（FPS、八达通、e-payment）收款成功后，系统自动向付款人推送电子收据，无需人工发送。

| 推送渠道 | 触发时机 | 内容 |
|----------|---------|------|
| App推送 | 收款成功即时 | 电子收据PDF链接 |
| 邮件 | 收款成功即时 | 电子收据PDF附件 |
| 短信（备用）| 收款成功即时 | 「您的缴费已成功，收据已发送至App/邮箱，请查收。」|

**缴费延迟提示（用户模拟反馈D-03 — 新增）：**
> 第三方支付（FPS等）到账存在延迟，界面须正确引导用户。

| 场景 | 界面显示 |
|------|---------|
| 缴费成功（第三方处理中）| 「您的缴费已提交，预计5分钟内更新。请勿重复缴费。」|
| 最后缴费记录 | 显示上次缴费时间戳（如「上次缴费：2026-05-27 14:23:12」）|
| 状态持续未更新（>10分钟）| 提示「缴费状态未更新，请联系校务处」+ 快捷联系按钮 |

**输出：**
```json
{
  "collection_date": "2026-05-23",
  "summary": {
    "total_collected": 15800.00,
    "transaction_count": 45,
    "by_type": {
      "air_con": 12000.00,
      "activity": 3000.00,
      "material": 800.00
    },
    "by_method": {
      "cash": 5000.00,
      "fps": 6800.00,
      "octopus": 4000.00
    }
  },
  "witness_verification": {
    "staff_1": "張同工",
    "staff_2": "李同工",
    "verified": true,
    "timestamp": "2026-05-23T16:00:00+08:00"
  },
  "receipts_issued": 45,
  "discrepancy": 0.00,
  "status": "balanced"
}
```

---

## 第四部分：Module 2 — 周期性校务管理

### 4.1 模块概述

| 属性 | 描述 |
|------|------|
| 模块名称 | Cyclical Operations Module (周期性校务管理) |
| 模块ID | MOD-CYCL-001 |
| 优先级 | P0 (关键) |
| 用户 | 全体校务人员, 教务协调员, 收生主任 |

**学年时间轴：**
```
7月        8月        9月        10-12月       1-3月        4-6月        6-8月
┌────────┐┌────────┐┌────────┐┌────────┐┌────────┐┌────────┐┌────────┐
│ 开学前  ││ 开学月  ││ 稳定期  ││  第一学  ││  第二学  ││  考试及  ││  学年结  │
│ 准备期  ││         ││         ││  期中期  ││  期中期  ││  毕业期  ││  束期    │
└────────┘└────────┘└────────┘└────────┘└────────┘└────────┘└────────┘
```

---

### Function F-ENRL-001: 新生注册管理

**目的：** 处理所有中一入学及跨校转学生

**输入：**
| 字段 | 类型 | 必填 | 来源 |
|------|------|------|------|
| application_no | String | 是 | 系统生成 |
| student_name_zh | String | 是 | 申请表 |
| date_of_birth | Date | 是 | 出生证明 |
| gender | Enum | 是 | 申请表 |
| school_of_origin | String | 条件 | 仅转学生 |
| parent_name | String | 是 | 申请表 |
| parent_id | String | 是 | 香港身份证 |
| contact_phone | String | 是 | 申请表 |
| special_education_needs | Boolean | 是 | 医疗/评估报告 |
| documents | File[] | 是 | OCR扫描 |

**业务规则：**
- 中一注册截止日期：8月31日（EDB要求）
- 转学生：到校后14天内
- SEN披露为自愿性质
- 所有文件须与原件核对

**输出：**
```json
{
  "registration_id": "ENRL-2026-S1-0001",
  "student": {
    "student_id": "2026S10001",
    "hkid": "A123456(X)",
    "dob": "2011-03-15",
    "gender": "M"
  },
  "status": "documents_verified",
  "documents_checklist": {
    "birth_certificate": {"submitted": true, "verified": true},
    "hkid_copy": {"submitted": true, "verified": true},
    "report_card": {"submitted": true, "verified": false},
    "consent_form": {"submitted": false, "deadline": "2026-08-20"}
  },
  "class_assigned": "1A",
  "sen_flag": true,
  "webSAMS_synced": true
}
```

---

### Function F-ENRL-002: AI辅助编班

**目的：** 根据多项标准公平分配学生到各班

**平衡因素权重（默认）：**
| 因素 | 权重 | 描述 |
|------|------|------|
| gender_ratio | 25% | 维持约50:50男女比例 |
| academic_ability | 25% | 各班均匀分布 |
| sen_students | 20% | SEN学生均匀分配 |
| sibling_conflict | 15% | 防止敌对关系同班 |
| school_origin | 10% | 同一学校来源分散 |
| special_talent | 5% | 体育/艺术人才均衡 |

**输出：**
```json
{
  "allocation_id": "ALLOC-2026-S1-001",
  "total_students": 180,
  "num_classes": 5,
  "allocations": [
    {
      "class": "1A",
      "students": [...],
      "gender_ratio": {"M": 18, "F": 18},
      "sen_count": 2,
      "avg_ability_score": 72.5
    }
  ],
  "balance_score": 87.3,
  "conflicts": [...],
  "approval_required": true
}
```

---

### Function F-ENRL-003: 课本分发管理

**目的：** 管理新生及在读学生的课本分发、领用确认及费用结算全流程

**输入字段定义：**
| 字段 | 类型 | 必填 | 来源 | 描述 |
|------|------|------|------|------|
| distribution_id | String | 自动 | 系统生成 | 分发记录唯一ID，格式：`TXTBK-{学年}-{班级}-{序号}` |
| academic_year | String | 是 | 系统 | 学年，例如 `2026-2027` |
| class_id | String | 是 | 系统/用户选择 | 班级代码，如 `1A`、`2B` |
| student_id | String | 是 | 学生主档 | 学号/学生唯一标识 |
| textbook_batch_id | String | 是 | 系统 | 课本批次ID（对应采购批次） |
| subject | String | 是 | 课程设置 | 科目名称，如 `中文`、`英文`、`数学` |
| textbook_title | String | 是 | 课本目录 | 课本完整书名 |
| isbn | String | 可选 | 课本目录 | 国际标准书号 |
| unit_price | Number | 是 | 课本目录 | 单价（港币，精确到小数点后2位）|
| quantity | Integer | 是 | 系统默认值 | 数量（默认1本）|
| discount_percent | Number | 可选 | 系统/用户 | 折扣百分比（0-100，默认0）|
| distribution_status | Enum | 是 | 系统/用户 | `pending`（待分发）/ `distributed`（已分发）/ `replaced`（已更换）/ `returned`（已退回）|
| distributed_at | DateTime | 条件 | 自动 | 分发时间（状态为distributed时自动记录）|
| distributed_by | String | 条件 | 自动 | 分发员工ID |
| payment_status | Enum | 是 | 系统 | `paid`（已付款）/ `unpaid`（未付款）/ `waived`（已豁免）|
| payment_method | Enum | 可选 | 用户选择 | `cash` / `fps` / `octopus` / `e_payment` / `school_award` |
| barcode | String | 可选 | 系统/扫描 | 课本条码（可选扫描录入）|
| notes | String | 可选 | 用户 | 备注，如退换原因、补发说明 |

**处理流程：**

```
Step 1: 课本批次准备 (Batch Preparation)
Step 2: 班级分发清单生成 (Distribution List Generation)
Step 3: 分发登记 (Distribution Registration)
Step 4: 费用结算 (Fee Settlement)
Step 5: 退换处理 (Replacement/Return)
Step 6: 数据汇总与归档 (Summary & Archive)
```

**Step 1: 课本批次准备 (Batch Preparation)**

| 处理规则 | 说明 | 示例 |
|----------|------|------|
| 批次关联 | 每学年课本采购生成独立批次ID | `TXTBK-BATCH-2026-001` |
| 价格同步 | 从课本目录（`textbook_catalog`）同步单价 | 系统自动填充 |
| 供应商关联 | 批次关联供应商，便于退换追溯 | 永锋书局 |

**Step 2: 班级分发清单生成 (Distribution List Generation)**

| 处理规则 | 说明 |
|----------|------|
| 班级学生拉取 | 根据班级ID从学生主档拉取该学年应领书学生名单 |
| 科目过滤 | 根据班级课程设置（`class_subject_config`）确定该生应领科目课本 |
| 数量规则 | 每科目每生1本（特殊需求如补发另行处理）|
| 豁免识别 | 系统根据学生 `subsidy_eligibility` 字段自动标记 `payment_status=waived` |

**Step 3: 分发登记 (Distribution Registration)**

| 处理规则 | 说明 |
|----------|------|
| 扫码登记 | 支持条码枪扫描录入，确认分发 |
| 手动登记 | 支持列表勾选批量确认分发 |
| 双重确认 | 分发后须由领书学生/家长签名确认（可电子签名）|
| 记录时间戳 | 每次分发操作自动记录时间和操作人员 |

**Step 4: 费用结算 (Fee Settlement)**

| 处理规则 | 说明 |
|----------|------|
| 自动汇总 | 系统自动汇总该生全部课本费用（数量×单价×折扣）|
| 支付方式 | 支持现金/FPS/八达通/e-payment/学校奖励金 |
| 豁免处理 | 符合全额资助资格学生自动标记 `payment_status=waived`，无需缴费 |
| 部分豁免 | 根据资助等级部分豁免，系统自动计算应付金额 |
| 电子收据 | 付款成功后自动推送电子收据（参 F-FEE-001）|

**Step 5: 退换处理 (Replacement/Return)**

| 处理规则 | 说明 |
|----------|------|
| 退换条件 | 课本错发/损坏/学生转学等可申请退换 |
| 退货退款 | 退回课本按原价的80%退款（折旧）|
| 换货 | 换货不收费，直接更新分发记录 |
| 时间限制 | 开学30天内接受退换申请，之后需特殊审批 |

**Step 6: 数据汇总与归档 (Summary & Archive)**

| 处理规则 | 说明 |
|----------|------|
| 每日汇总 | 生成每日分发汇总报表（分发数量/金额/未领清单）|
| 学期归档 | 学期末自动归档分发数据，生成年度课本费用报告 |
| 库存联动 | 分发数量自动从批次库存中扣减 |

**验收标准 (AC)：**

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 学年已设置，课本目录已维护，班级学生名单已导入 | 校务处同工进入课本分发模块，选择班级和学年 | 系统显示该班级全部学生及应领课本清单（按科目列出）|
| AC-02 | 学生A应领12本课本（含全免资格）| 校务处同工为学生A办理分发登记（12本全部确认分发）| 系统记录12条分发记录，payment_status自动标记为`waived`，无需缴费操作 |
| AC-03 | 课本批次关联供应商"永锋书局"，其中数学课本定价HK$185 | 校务处同工扫描数学课本条码或手动选择"数学-《新世代数学》"| 系统自动填充：供应商=永锋书局，单价=HK$185.00，数量=1 |
| AC-04 | 学生B使用FPS扫码缴费HK$185 | 校务处同工在收款界面确认学生B缴纳数学课本费HK$185（FPS方式）| 系统生成收款记录，payment_status更新为`paid`，自动推送电子收据PDF至家长App/邮箱 |
| AC-05 | 分发后发现数学课本错发为旧版 | 校务处同工申请更换，扫描正确版本条码 | 系统更新分发记录：旧记录status=`replaced`，新记录status=`distributed`，不重复收费 |
| AC-06 | 开学第5天，某学生申请退回数学课本（保存完好）| 校务处同工在退换模块选择学生和退书科目，提交退货 | 系统生成退货记录，退款金额=HK$185×80%=HK$148，自动更新payment_status |
| AC-07 | 班级1A当日共分发50本课本，总价HK$8,500 | 校务处同工点击"生成汇总报告" | 系统生成当日分发报表：分发数量50本、总金额HK$8,500、未领学生3人 |
| AC-08 | 某学生未在规定时间内领书（开学30天后）| 校务处同工尝试为其办理分发 | 系统弹出提示「超过30天无记录，请联系校务主任审批」，记录留痕 |

**业务规则：**
- 每学年初（开学前两周）开放课本分发系统
- 开学30天后需校务主任特别审批方可分发
- 课本费用不纳入学费管理，独立结算
- 供应商退换货周期：10个工作日

**输出：**
```json
{
  "distribution_id": "TXTBK-2026-1A-001",
  "class": "1A",
  "student": {"id": "2026S10001", "name": "王小明"},
  "textbooks": [
    {
      "subject": "中文",
      "title": "新編中國語文",
      "price": 185.00,
      "status": "distributed",
      "barcode": "TXTBK-2026-12345",
      "distributed_at": "2026-09-01T09:30:00+08:00"
    }
  ],
  "summary": {
    "total_textbooks": 12,
    "distributed": 12,
    "total_cost": 2220.00,
    "payment_status": "paid"
  }
}
```

---

### Function F-EXAM-001: DSE报考管理

**目的：** 处理香港中学文凭考试报名

**输入：**
| 字段 | 类型 | 必填 | 来源 |
|------|------|------|------|
| student_id | String | 是 | WebSAMS |
| subject_selections | Array[Subject] | 是 | 学生选择 |
| special_arrangements | Object | 条件 | 医疗/SEN报告 |
| photo | File | 是 | 照片上传 |
| declaration_signed | Boolean | 是 | 同意书 |

**科目分类：**
| 类别 | 科目 |
|------|------|
| Category A - 核心 | 中文, 英文, 数学, 公民与社会 |
| Category A - 选修 | 生物, 化学, 物理, 经济等 |
| Category B | 应用学习 |
| Category C | 其他语言 |

**业务规则：**
- 最少6科（包括4个核心）
- 最多8科
- 逾期报名费：每科HK$560
- 截止后退选需医疗证明

**输出：**
```json
{
  "registration_id": "DSE-2026-001234",
  "student": {
    "id": "2023S10123",
    "name": "陳大文",
    "class": "6A"
  },
  "subjects": [
    {"subject_code": "CN", "subject_name": "中國語文", "category": "A_core", "status": "registered"},
    {"subject_code": "BA", "subject_name": "企業概論", "category": "A_elective", "status": "registered"}
  ],
  "special_arrangements": {
    "extra_time": true,
    "duration_extension": "25%",
    "separate_room": true,
    "approved_by_hkeaa": true
  },
  "total_subjects": 6,
  "registration_status": "submitted_to_hkeaa"
}
```

---

### Function F-EXAM-002: 试卷管理

**流程：**
```
试卷需求确认 → 印刷申请 → 印刷厂印制 → 密封交付 
      ↓
储存保险箱 → 考试前分发 → 考试使用 → 密封回收 
      ↓
保存期限 → 审批销毁 → 记录归档
```

**子功能：**
| 子功能 | ID | 描述 |
|--------|-----|------|
| 试卷需求统计 | F-EXAM-002a | 计算每科/每班试卷总数 |
| 印刷申请管理 | F-EXAM-002b | 生成供应商印刷订单 |
| 密封追踪 | F-EXAM-002c | 追踪密封号码及保管链 |
| 保险箱管理 | F-EXAM-002d | 安全存储及访问记录 |
| 分发记录 | F-EXAM-002e | 考试日分发及监考员签收 |
| 回收与销毁 | F-EXAM-002f | 考后回收及销毁记录 |

---

### Function F-EXAM-003: 特别考试安排

**安排类型：**
| 类型代码 | 描述 | 所需审批 |
|---------|------|----------|
| EXTRA_TIME | 25%或50%额外时间 | HKEAA |
| SEP_ROOM | 独立考场 | 学校+HKEAA |
| SCRIBE | 抄写员 | HKEAA |
| READER | 读卷员 | HKEAA |
| BRAILLE | 盲文试卷 | HKEAA |
| WHEELCHAIR | 轮椅通道书桌 | 学校 |

**输出：**
```json
{
  "arrangement_id": "SEA-2026-S6-CHEM-001",
  "student": {
    "id": "2023S10501",
    "name": "張同學",
    "sen_type": "ASD",
    "sen_severity": "moderate"
  },
  "exam": {
    "subject": "化學",
    "date": "2026-04-18",
    "paper": "卷二"
  },
  "arrangements": [
    {"type": "extra_time", "description": "25%額外時間", "approval_ref": "SEA-2025-CHEM-555"},
    {"type": "sep_room", "description": "獨立考場 - 201室", "invigilator_assigned": "陳老師"}
  ]
}
```

---

### Function F-EXAM-004: 成绩单生成与发布

**流程：**
1. 从WebSAMS/eClass汇总考试成绩
2. 计算加权分数及排名
3. 生成描述性评语（AI辅助）
4. 校长/副校长审核批准流程
5. 生成PDF成绩单
6. 通过eClass/SchoolApp/实体分发

**教师成绩自撤回功能（用户模拟反馈C-03 — 新增，评审报告修正 — 业务逻辑不合理#3）：**
> 教师提交成绩后，在审批人审批前，可在48小时内自行撤回修改，无需联系教务主任退回。

| 条件 | 说明 |
|------|------|
| 撤回期限 | 提交后48小时内 |
| 撤回条件 | 仅在审批人尚未审批时可撤回 |
| 撤回次数 | 不限次数 |
| 操作记录 | 每次撤回记录至审计日志（F-USER-005）|
| **审计告警（评审修正）** | **教师撤回成绩后，系统自动触发审计告警（alert），记录：撤回教师、原始成绩、撤回时间戳、撤回理由（必填）；告警推送给校务主任，而非无记录处理** |

**家长成绩单PDF导出（用户模拟反馈D-07 — 新增）：**
> 家长可在微信门户将孩子的成绩单导出为PDF，方便保存和打印。

| 导出内容 | 说明 |
|----------|------|
| 完整成绩单 | 各科成绩、等级、排名、班主任评语 |
| 导出格式 | PDF（A4竖版）|
| 水印 | 可选：「仅供家长个人使用」|
| 历史成绩 | 可选择导出历史学期成绩单 |

**班级成绩分布图（用户模拟反馈C-06 — 新增）：**
> 教师可查看任教班级在全年级的成绩分布柱状图，辅助了解学生定位。

| 图表类型 | 内容 |
|----------|------|
| 分数分布柱状图 | 班级各科分数与年级平均分的对比 |
| 等级分布饼图 | A/B/C/D等级占比 |
| 排名变化折线图 | 学生本次与上次考试的排名变化 |

**输出：**
```json
{
  "report_card_batch": "RC-2026-S1-TERM1-001",
  "class": "1A",
  "total_students": 36,
  "reports": [
    {
      "student_id": "2025S10001",
      "subjects": [
        {
          "subject": "中文",
          "score": 85,
          "grade": "A",
          "class_rank": 5,
          "class_avg": 72.3,
          "comment": "閱讀理解能力強，寫作表達有進步。"
        }
      ],
      "overall_score": 78.5,
      "class_rank": 8,
      "conduct_grade": "B+",
      "attendance_rate": "95%",
      "status": "pending_approval"
    }
  ],
  "approval_workflow": {
    "teacher_comments_done": 30,
    "principal_approval_done": 0,
    "next_deadline": "2026-02-15"
  }
}
```

---

### Function F-ADM-001: 中一自行分配学位

**流程时间表（EDB标准）：**
```
1月 → 学校公布收生准则 → 家长递交申请表 
2月 → 学校进行面试 → 公布正取/备取名单 
3-4月 → EDB公布自行分配阶段结果 
5月 → 正取学生确认学位 → 获录取学生完成注册
```

**评分系统：**
| 准则 | 最高分 |
|------|--------|
| 学业表现（成绩表） | 30 |
| 面试表现 | 30 |
| 兄弟姐妹在校 | 10 |
| 家长校友 | 5 |
| 其他成就 | 10 |
| 校长酌情权 | 15 |

---

### Function F-ADM-002: JUPAS联招管理

**流程：**
1. 收集学生JUPAS选择
2. 生成学校推荐信
3. 处理校长/教师推荐信
4. 追踪申请状态
5. 处理上诉程序

**推荐信AI辅助写作（用户模拟反馈C-04 — 新增）：**
> 教师填写推荐信时，系统提供字数统计和AI写作提示，帮助教师写出符合要求的推荐信。

| 辅助功能 | 说明 |
|----------|------|
| 字数统计 | 实时显示当前字数（如「约280字」），建议字数：300-500字 |
| 最低字数提示 | 低于200字时显示「字数较少，建议补充更多细节」|
| AI写作大纲 | 根据学生档案自动生成写作大纲建议（学业表现/个人特质/课外活动三部分）|
| 术语一致性检查 | 自动检查术语与系统术语表的一致性 |
| 历史推荐信参考 | 可查看该校其他教师的推荐信示例（脱敏后）|

**输出：**
```json
{
  "jupas_id": "JUPAS-2026-S6-00045",
  "student": {"id": "2023S10678", "name": "陳大文", "class": "6A"},
  "jupas_app_no": "24601234",
  "choices": [
    {
      "priority": 1,
      "institution": "香港大學",
      "program": "工商管理學學士",
      "program_code": "JS4013",
      "status": "confirmed"
    }
  ],
  "reference_letters": [
    {"teacher": "李老師", "subject": "數學", "status": "draft", "deadline": "2026-03-15"}
  ],
  "school_reference": {
    "status": "in_progress",
    "includes": ["校內成績", "班主任評語", "學校排名"],
    "submission_deadline": "2026-03-31"
  }
}
```

---

### Function F-YREND-001: 档案清理与销毁

**保存期限（EDB指引）：**
| 文档类型 | 保存期限 | 期限后处理 |
|---------|---------|-----------|
| 学生注册表 | 7年（毕业后） | 销毁 |
| 成绩表 | 永久保存 | N/A |
| 处分记录 | 7年 | 销毁 |
| 健康记录 | 学生离校后3年 | 销毁 |
| 财务收据 | 7年 | 销毁 |
| 会议记录 | 5年 | 移交校监 |
| 员工合同 | 雇员离职后7年 | 销毁 |
| 毕业照 | 永久保存 | N/A |

---

### Function F-YREND-002: 学年财务结算

**输出：**
```json
{
  "reconciliation_id": "YREC-2025-2026",
  "fiscal_year": "2025-2026",
  "summary": {
    "total_fees_collected": 4580000.00,
    "total_expenses": 3890000.00,
    "net_balance": 690000.00,
    "budget_variance": -120000.00
  },
  "by_category": [
    {
      "category": "tuition",
      "budget": 3000000.00,
      "collected": 2950000.00,
      "outstanding": 50000.00
    }
  ],
  "outstanding_fees": [...],
  "audit_status": "ready_for_audit"
}
```

---

## 第五部分：Module 3 — 财务及资产管理

### Function F-FIN-001: 学费管理

**费用类型：**
| 费用类型代码 | 描述 | 强制性 |
|------------|------|--------|
| TUITION | 学费 | 是（资助学校豁免） |
| SUBSIDY | 堂费 | 是 |
| AIRCON | 冷气费 | 可选 |
| ACTIVITY | 活动费 | 按活动 |
| BUS | 校车费 | 可选 |
| INSURANCE | 学生保险 | 年度 |
| SCHOLARSHIP | 奖学金 | N/A（负数） |

**输出（用户模拟反馈A-06 — 新增欠费子状态）：**
```json
{
  "fee_assessment_id": "FA-2026-2027-S6-00045",
  "student": {
    "id": "2023S10678",
    "name": "陳大文",
    "subsidy_eligibility": "full_grant"
  },
  "fee_items": [
    {
      "type": "tuition",
      "annual_amount": 0.00,
      "net_payable": 0.00,
      "status": "exempted"
    },
    {
      "type": "subsidy",
      "annual_amount": 3100.00,
      "edb_subsidy": 1550.00,
      "net_payable": 1550.00,
      "outstanding": 1033.00,
      "sub_status": "installment_plan",
      "installments": [
        {"term": 1, "amount": 517.00, "due": "2026-09-15", "paid": true},
        {"term": 2, "amount": 517.00, "due": "2026-11-15", "paid": false, "sub_status": "installment_plan"},
        {"term": 3, "amount": 516.00, "due": "2027-02-15", "paid": false, "sub_status": "overdue"}
      ]
    }
  ],
  "summary": {
    "total_annual": 3100.00,
    "total_paid": 517.00,
    "total_outstanding": 1033.00,
    "outstanding_breakdown": {
      "installment_plan": 1033.00,
      "overdue": 0.00,
      "disputed": 0.00
    }
  }
}
```


#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 学生王小明已评估全额资助资格，堂费HK$3100 | 校务处同工进入学费管理，查看王小明学费明细 | tuition=exempted，subsidy应付HK$1550，payment_status=exempted |
| AC-02 | 学生王小明分期付款，第二期HK$517到期未缴 | 系统例行检查发现逾期 | sub_status升级为overdue，显示红色告警标签，推送App通知至家长 |
| AC-03 | 家长对学费金额提出申诉 | 校务处同工收到申诉，更新状态 | sub_status更新为disputed，暂停催款流程，outstanding_breakdown记录disputed金额 |
| AC-04 | 学期末，学费账目需对账 | 校务处同工点击「生成学费对账报表」 | 系统生成PDF报表：总收入、未缴总额、各状态（paid/installment_plan/overdue/disputed）分布 |


> **sub_status 枚举值说明：**
> - `installment_plan`：分期付款中（正常状态）
> - `overdue`：逾期未付（需跟进）
> - `disputed`：争议中（家长提出申诉）
> - `paid`：已缴清

---

### Function F-FIN-002: 零用现金报销

**功能ID：** F-FIN-002
**功能名称：** 零用现金报销
**模块：** MOD-FIN-001 (财务资产管理)
**子模块：** 收费
**优先级：** P0
**责任人：** 校务处同工

#### 功能描述

零用现金报销功能允许校务处同工通过系统提交零用现金（ Petty Cash ）支出申请，经双人见证（适用于超过 HK$500 的交易）和校务主任审批后完成报销。系统支持 OCR 收据识别、自动双人见证推送和备用金余额追踪。

#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 校务处同工提交一笔HK$856的文具采购报销申请，上传收据 | 提交报销申请 | 系统自动执行OCR识别，显示「🤖 OCR识别金额: HK$856.00」并以黄色底色高亮；同时显示「📋 系统登记金额」；显示「⚠️ 请人工核对收据原件」复核提示 |
| AC-02 | OCR识别金额与实际收据金额一致，校务处同工确认提交 | 提交双人见证 | 系统自动向第一见证人（李主任）发送见证任务推送（App + 短信），显示交易摘要和金额 |
| AC-03 | 第一见证人完成见证操作 | 第一见证人点击「确认见证」| 系统记录见证时间戳，自动向第二见证人（陈副主任）发送待见证任务推送，见证状态更新为「待第二见证人」 |
| AC-04 | 第二见证人完成见证操作 | 第二见证人点击「确认见证」| 系统锁定交易，状态更新为「待审批」，当前备用金余额自动扣减HK$856 |
| AC-05 | 校务主任收到审批待办通知，审批该笔报销 | 校务主任点击「批准」| 系统记录审批时间和审批人，交易状态更新为「已批准」，校务处同工收到批准通知 |
| AC-06 | 单笔报销金额超过HK$3,000（动态限额上限）| 校务处同工尝试提交 | 系统显示警告：「单笔交易超过HK$3,000限额，请联系校务主任调整动态限额」并阻止提交 |
| AC-07 | 第一见证人在30分钟内未完成见证 | 系统例行检查（每5分钟）| 系统发送提醒至第一见证人；超过30分钟自动提醒校务主任介入处理 |
| AC-08 | 校务主任选择拒绝该报销申请 | 点击「拒绝」并填写拒绝原因 | 系统记录拒绝原因，交易状态更新为「已拒绝」，申请人收到拒绝通知（含原因）|
| AC-09 | OCR识别金额与系统登记金额不一致 | OCR识别完成后 | 系统以红色高亮显示差异金额，弹出警告：「⚠️ OCR识别金额与系统登记金额不一致，请人工核对收据」|
| AC-10 | 备用金余额低于HK$500，申请人尝试提交报销 | 提交报销申请 | 系统显示警告：「⚠️ 备用金余额不足（当前HK$324），请先申请备用金补充」，可选择继续提交或取消 |

#### 测试用例

| # | 测试场景 | 预期结果 |
|---|----------|---------|
| TC-01 | 小额报销（≤HK$500）单人见证 | 系统跳过第二见证人流程，直接进入审批 |
| TC-02 | 大额报销（>HK$500）双人见证 | 第一见证人完成后自动推送第二见证人任务 |
| TC-03 | OCR识别成功，金额一致 | 显示绿色状态「OCR识别成功」，黄色高亮显示金额 |
| TC-04 | OCR识别失败 | 显示「⚠️ OCR识别失败，请手动输入金额」，系统仍可正常提交 |
| TC-05 | 见证人手机无网络 | 推送失败后自动重试3次，仍失败则发送短信备用通知 |
| TC-06 | 备用金余额为0 | 禁止提交任何报销申请，提示先补充备用金 |
| TC-07 | 报销单据丢失 | 支持重新上传收据图片，保留历史记录审计 |
| TC-08 | 动态限额CPI调整生效 | 学年切换时系统自动计算新限额并通知校务主任确认 |

#### 异常处理

| 异常情况 | 处理方式 |
|----------|---------|
| OCR识别失败 | 降级为手动输入金额，显示「⚠️ 请手动输入金额」提示 |
| 第一见证人超时未见证 | 30分钟后提醒，1小时后通知校务主任可指定替代见证人 |
| 第二见证人拒绝见证 | 记录拒绝原因，退回申请人重新提交或说明情况 |
| 审批人离线 | 系统保留待审批状态，待审批人上线后处理 |
| 网络中断 | 本地缓存提交内容，网络恢复后自动重试并通知结果 |
| 备用金不足 | 阻止提交，提示先通过F-FIN-001申请备用金补充 |

**业务规则（评审报告修正 — 业务逻辑不合理#3）：**
- **单笔交易限额：动态限额机制（评审修正）**：单笔HK$3,000限额为基础限额，每学年根据香港政府统计处公布的综合消费物价指数（CPI）自动调整，公式：`实际限额 = 基础限额 × (当年CPI指数 / 基准CPI指数)`；调整结果由校务主任确认后生效，并在系统公告通知
- 现金交易>HK$500需双重授权
- 所有交易需原始收据
- 每月对账必需
- 备用金补充：最高HK$5,000

**OCR收据识别展示规范（用户模拟反馈A-02 — 新增）：**
> OCR识别的金额须与系统记录金额明确区分，并要求审核人员人工核对原件。

| 展示要素 | 规范 |
|----------|------|
| OCR金额标签 | 显示「🤖 OCR识别金额」（非系统原始金额）|
| 系统记录金额 | 显示「📋 系统登记金额」|
| 视觉区分 | OCR金额以**黄色底色高亮**显示，系统金额以白色正常显示 |
| 复核提示 | 以**粗体红色文字**显示：「⚠️ 请人工核对收据原件」|
| 收据缩略图 | 审批页面侧栏显示收据图片缩略图，支持点击放大 |

**双人见证实时推送（用户模拟反馈B-04 — 新增）：**
> 第一见证人完成见证后，系统自动向第二见证人发送待见证任务推送（App推送 + 短信），无需手动通知。

| 触发条件 | 系统行为 |
|----------|---------|
| 第一见证人完成见证 | 自动向第二见证人发送待见证任务推送（内容：交易摘要 + 金额 + 见证链接）|
| 第二见证人完成见证 | 系统锁定交易 → 流程完成 |
| 第二见证人超时（30分钟未处理）| 自动提醒校务主任介入 |

**输出：**
```json
{
  "transaction_id": "PC-2026-05-0234",
  "type": "payment",
  "amount": 856.00,
  "payee": "光明文具公司",
  "description": "購買期末考試試卷紙 (500張)",
  "category": "printing",
  "authorizations": [
    {"name": "李主任", "role": "first_authorizer", "signed_at": "2026-05-23T11:45:00+08:00"},
    {"name": "陳副主任", "role": "second_authorizer", "signed_at": "2026-05-23T11:50:00+08:00"}
  ],
  "dual_authorization_completed": true,
  "current_float_balance": 3244.00
}
```

---

### Function F-FIN-003: 奖学金与津贴申请

**奖学金/津贴类型：**
| 类型 | 来源 | 金额 | 资格 |
|------|------|------|------|
| 校本奖学金 | 学校 | HK$1,000-5,000 | 学业优秀 |
| 书簿津贴 | 教育局 | HK$3,000-5,000 | 低收入家庭 |
| 车船津贴 | 教育局 | HK$1,000-3,000 | 偏远交通 |
| 校本清贫助学金 | 学校 | HK$500-2,000 | 校长酌情 |

---

### Function F-ASSET-001: 校产条码盘点

**资产分类：**
```
固定资产 | 电子设备 | 家具 | 乐器 | 运动器材 | 实验室设备 | 
图书馆藏书 | 视听器材 | 电脑设备 | 网络设备
```

**输出：**
```json
{
  "inventory_session_id": "INV-2026-ANNUAL-001",
  "total_registered_assets": 2456,
  "assets_verified": 2389,
  "verification_rate": "97.3%",
  "discrepancies": [
    {
      "asset_id": "ASSET-2020-LAPTOP-0045",
      "name": "聯想手提電腦",
      "registered_location": "301室",
      "scan_location": "not_found",
      "status": "missing",
      "responsible_person": "王老師",
      "investigation_status": "pending"
    }
  ],
  "condition_summary": {
    "excellent": 120,
    "good": 1856,
    "fair": 298,
    "poor": 98
  }
}
```

---

### Function F-ASSET-002: 场地租借管理

**场地及定价：**
| 场地 | 每小时租金 | 按金 | 保险要求 |
|------|-----------|------|---------|
| 礼堂 | HK$800 | HK$2,000 | 是 |
| 篮球场 | HK$400 | HK$1,000 | 是 |
| 课室 | HK$200 | HK$500 | 否 |
| 活动室 | HK$300 | HK$500 | 否 |
| 游泳池 | HK$600 | HK$1,500 | 是 |

---

### Function F-ASSET-003: 设备保养管理

**保养类型：**
| 类型 | 频率 | 示例 |
|------|------|------|
| 定期保养 | 每月/每季 | 冷气系统, 升降机, 消防设备 |
| 预防性保养 | 年度 | 冷气机清洗, 灭火筒更换 |
| 故障维修 | 按需 | 任何设备故障 |
| 安全检测 | 年度 | 电力系统, 气体装置, 升降机 |

---

### Function F-VEND-001: 供应商注册与评估

**供应商分类：**
```
图书供应 | 文具供应 | 膳食供应（饭盒）| 校车服务 | 设备维修 |
印刷服务 | 清洁服务 | 保险公司 | 网络服务 | 活动物资
```

---

## 第六部分：Module 6 — 用户与权限管理系统

### 6.1 模块概述

| 属性 | 描述 |
|------|------|
| 模块名称 | User & Access Management (用户与权限管理系统) |
| 模块ID | MOD-USER-001 |
| 优先级 | P0 (关键 — 所有其他模块的基础) |
| 用户 | 校务主任, 系统管理员 |

---

### Function F-USER-001: 用户生命周期管理

**目的：** 统一管理系统内所有用户身份的全生命周期

**用户角色定义：**

| 角色代码 | 角色名称 | 描述 | 可授权功能范围 |
|----------|----------|------|---------------|
| `SCHOOL_ADMIN` | 校务主任 | 统筹管理，全权限 | 全部功能 |
| `OFFICER` | 校务处同工 | 日常操作 | 所属业务范围 |
| `TEACHER` | 教师 | 查询与填报 | 受限功能 |
| `PARENT` | 家长 | 门户查询 | 仅家长门户 |
| `STUDENT` | 学生 | 自主查询 | 仅学生门户 |
| `SYSTEM` | 系统管理员 | 技术运维 | 系统配置，用户管理 |

**输入：**
| 字段 | 类型 | 必填 | 来源 |
|------|------|------|------|
| user_type | Enum | 是 | `SCHOOL_ADMIN`, `OFFICER`, `TEACHER`, `PARENT`, `STUDENT`, `SYSTEM` |
| staff_id / student_id / parent_id | String | 条件 | 内部编号 |
| name_zh | String | 是 | 姓名 |
| name_en | String | 可选 | 英文名 |
| email | String | 条件 | 学校邮箱或家长邮箱 |
| phone | String | 可选 | 联系电话 |
| department | String | 条件 | 任职部门（校务处/教务/总务等）|
| class_assignment | String | 条件 | 所属班级（仅教师/班主任）|
| linked_student_id | String | 条件 | 关联学生（仅家长）|
| employment_type | Enum | 条件 | `permanent`, `contract`, `supply`, `relief` |
| account_expiry | Date | 可选 | 账户过期日（合同截止）|

**业务规则（用户模拟反馈D-06 — 新增家长密码强制设置）：**
- 教师入职自动生成账户，离职自动停用（联动HR记录）
- 同一手机号/邮箱不可重复注册（防止冒用）
- 账户过期前30天推送预警
- 离职/毕业自动触发账户禁用流程（保留审计记录）
- 家长账户与学生账户绑定，一对多（多个子女）
- **家长首次注册时须强制设置「家长密码」（独立于登录密码），用于多子女账号关联验证；若未设置，系统引导至「设置家长密码」页面后方可使用完整功能**
- **家长密码找回支持短信OTP验证（F-USER-006），不再依赖到校办理**

**输出：**
```json
{
  "user_id": "USR-2026-00001",
  "account_status": "active",
  "profile": {
    "user_type": "OFFICER",
    "staff_id": "S00123",
    "name_zh": "張三豐",
    "email": "cheungsf@school.edu.hk",
    "phone": "91234567",
    "department": "校務處",
    "created_at": "2026-09-01T09:00:00+08:00",
    "account_expiry": "2027-08-31",
    "password_changed_at": "2026-09-01T09:00:00+08:00",
    "password_expires_in_days": 90,
    "last_login": "2026-05-23T08:30:00+08:00",
    "status": "active"
  },
  "linked_accounts": [],
  "provisioned_systems": ["school_admin_ai", "feishu", "eclass"],
  "offboarding_status": null,
  "audit_trail": [
    {"action": "account_created", "actor": "USR-SYS-ADMIN", "timestamp": "2026-09-01T09:00:00+08:00"}
  ]
}
```


#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 新教师入职（合同制，9月1日开始）| 系统管理员新建教师账户，填写信息 | 系统自动生成账户，发送欢迎邮件，account_expiry设为2027-08-31 |
| AC-02 | 教师账户过期前30天 | 系统例行检查 | 自动向该教师发送账户到期预警，提醒联系校务处 |
| AC-03 | 教师离职（8月31日合同到期）| 系统执行账户到期自动处理 | 账户状态=disabled，保留审计记录，eClass/飞书同步禁用 |
| AC-04 | 家长首次注册，未设置家长密码 | 家长尝试使用完整功能（如添加第二个子女）| 系统引导至「设置家长密码」页面，设置完成后方可继续操作 |
| AC-05 | 家长遗忘家长密码（已绑定手机）| 家长点击「忘记家长密码」| 系统发送短信OTP，验证后允许设置新家长密码，无需到校办理 |
| AC-06 | 同一手机号被两个不同姓名尝试注册 | 系统检测到重复手机号 | 拒绝注册，提示「该手机号已被其他账户使用」|
| AC-07 | 一个家长账号绑定3个子女 | 家长依次添加3个子女学号 | 系统逐一验证，全部成功后显示「已绑定3名子女」，家长密码验证通过 |


---

### Function F-USER-002: 身份认证

**目的：** 提供安全、多因素的登录验证机制

**认证方式支持：**

| 认证方式 | 适用角色 | 描述 |
|----------|----------|------|
| 账号密码 | 全部 | 盐值哈希存储（bcrypt/argon2）|
| 单点登录 (SSO) | 教师, 校务主任 | 学校统一身份（LDAP / Azure AD / Google Workspace）|
| 短信验证码 (OTP) | 家长, 学生 | 6位数字，一次性有效（10分钟）|
| 飞书 OAuth | 全部 | 飞书扫码登录（内部协作场景）|
| 生物识别 | 教师, 学生 | 设备指纹 / 刷脸（App端）|
| 硬件Token | SYSTEM | TOTP / FIDO2 安全密钥 |

**登录流程：**
```
用户输入账号 → 账号存在检查 → 认证方式路由
    │
    ├─ 密码登录：验证密码 → 检查密码强度/过期 → 密码错误计数
    ├─ SSO：重定向至IdP → 验证SAML/OIDC断言 → 会话建立
    ├─ OTP：发送验证码 → 验证OTP → 绑定设备标记
    └─ 飞书OAuth：授权回调 → 验证open_id → 会话建立

认证成功 → 生成会话Token → 记录登录日志 → 返回用户Profile
认证失败 → 记录失败日志 → 触发告警（>3次）→ 锁定账户（>5次）
```

**密码策略：**
| 策略项 | 要求 |
|--------|------|
| 最小长度 | 8位 |
| 复杂度 | 须包含：大写字母 + 小写字母 + 数字 + 特殊字符 |
| 有效期 | 90天（可配置）|
| 历史记录 | 最近5个密码不可复用 |
| 错误锁定 | 5次错误，锁定15分钟 |
| 账户解锁 | 管理员手动 / 邮件/短信验证码自助 |

**输出：**
```json
{
  "auth_result": {
    "success": true,
    "user_id": "USR-2026-00001",
    "user_type": "OFFICER",
    "session_token": "eyJhbGciOiJIUzI1NiIs...",
    "token_type": "Bearer",
    "expires_at": "2026-05-23T20:30:00+08:00",
    "refresh_token": "rt_a1b2c3d4e5f6...",
    "mfa_completed": true,
    "mfa_method": "totp",
    "device_fingerprint": "fp_8f3a9b2c...",
    "ip_address": "203.0.113.42",
    "user_agent": "Mozilla/5.0..."
  },
  "risk_check": {
    "new_device": false,
    "new_location": true,
    "city": "Hong Kong",
    "risk_level": "low"
  }
}
```

---

### Function F-USER-003: 功能授权 (RBAC + ABAC)

**目的：** 细粒度控制用户对功能模块、数据范围和操作类型的访问权限

**角色权限矩阵（默认）：**

| 功能模块 | SCHOOL_ADMIN | OFFICER | TEACHER | PARENT | STUDENT | SYSTEM |
|----------|:-----------:|:-------:|:-------:|:------:|:-------:|:------:|
| 仪表板查看 | ✅ 全部 | ✅ 本职 | ✅ 本班 | ✅ 子女 | ✅ 本人 | ❌ |
| 出勤管理 | ✅ 全部 | ✅ 全部 | ✅ 本班 | ❌ | ❌ | ❌ |
| 家长查询处理 | ✅ 全部 | ✅ 全部 | ❌ | ❌ | ❌ | ❌ |
| 请假审批 | ✅ 全部 | ✅ 全部 | ✅ 本班 | 申请 | ❌ | ❌ |
| 费用收取 | ✅ 全部 | ✅ 全部 | ❌ | ❌ | ❌ | ❌ |
| 费用查询 | ✅ 全部 | ✅ 全部 | ✅ 本班学生费用状态 | ✅ 子女缴费记录 | ❌ | ❌ | ❌ |
| 成绩管理 | ✅ 全部 | ✅ 查看 | ✅ 本班 | ✅ 子女 | ✅ 本人 | ❌ |
| DSE/考试管理 | ✅ 全部 | ✅ 操作 | ✅ 本班 | ❌ | ❌ | ❌ |
| 财务报销 | ✅ 审批 | ✅ 提交 | ❌ | ❌ | ❌ | ❌ |
| 资产盘点 | ✅ 全部 | ✅ 查看 | ❌ | ❌ | ❌ | ❌ |
| 用户管理 | ✅ 全部 | ❌ | ❌ | ❌ | ❌ | ✅ |
| 系统配置 | ✅ 全部 | ❌ | ❌ | ❌ | ❌ | ✅ |
| 审计日志 | ✅ 查看 | ❌ | ❌ | ❌ | ❌ | ✅ |

> **🆕 v1.6.2 (#287) 前端菜单角色过滤 — 侧边栏按角色显示**：
> - Admin Dashboard 侧边栏现已按当前用户角色过滤菜单项
> - 实现方式：`Layout.tsx` 中每个菜单项定义 `roles: string[]`，运行时通过 `useMemo` 过滤
> - 用户信息存储：`userService.ts` 统一从 localStorage 读取角色
> - 双重防线：前端隐藏为 UI 体验，后端 `JwtAuthGuard + RolesGuard` 为安全底线
> - 完整角色-菜单映射表见 `SPEC-SYSTEM-DESIGN.md` §15.5
> - 详情：[#287 影响分析](https://github.com/jchu-hk/school-admin-system/issues/287#issuecomment-5077187860)

> **🔒 RBAC权限澄清 — TEACHER角色费用权限说明（v1.7.0 Patch修复）：**
> - **费用收取（❌ 无权限）**：TEACHER角色**不可**操作收费界面、录入收款记录或修改费用数据。收费操作（录入、审批、豁免）仅限SCHOOL_ADMIN和OFFICER角色。
> - **费用查询（✅ 本班学生费用状态）**：TEACHER角色可查看所教班级学生的费用缴纳状态（如欠费提醒确认），用于辅助班主任了解学生家庭情况以便给予适当支持。**但不得查看金额详情，仅显示"已缴清"/"欠费"/"分期付款"等状态标签。**
> - **家长费用查询（✅ 子女缴费记录）**：PARENT角色可查看所绑定子女的完整缴费记录和费用明细。

**ABAC规则引擎详细示例（Open Policy Agent / Rego — v1.7.0新增）：**

> 以下为OPA Rego策略示例，展示各类典型ABAC规则的实现方式。实际部署时可将此作为参考实现。

##### 1. 班级范围控制规则

```rego
# 班级范围控制：教师仅能操作所教班级的学生数据
package school.auth

# 教师班级范围规则
teacher_class_scope[grant] {
    input.user.role == "TEACHER"
    input.resource.type == "student_record"
    
    # 获取教师所教班级列表
    teacher_classes := teacher_assigned_classes[input.user.id]
    
    # 检查目标学生是否在教师所教班级
    student_class := input.resource.student_class
    student_class == teacher_classes[_]
    
    grant := {
        "decision": "allow",
        "scope": "class_based",
        "permitted_classes": teacher_classes
    }
}

# 教师所教班级列表（实际从数据库/IDP获取）
teacher_assigned_classes[user_id] := classes {
    # 从HR系统或eClass同步的教师-班级关系
    data.teachers[user_id].assigned_classes[classes]
}

# 默认拒绝：教师访问非所教班级数据
default teacher_class_scope = false
teacher_class_scope[grant] {
    input.user.role == "TEACHER"
    input.resource.type == "student_record"
    grant := {"decision": "deny", "reason": "student_not_in_assigned_class"}
}
```

##### 2. 家长子女范围控制规则

```rego
# 家长子女范围规则：家长仅能查看所绑定子女的数据
package school.auth

# 家长绑定的子女列表
parent_bound_children[parent_id] := child_ids {
    # 从家长-学生绑定关系表获取
    data.parent_student_bindings[parent_id].children[child_ids]
}

# 家长数据访问规则
parent_data_access[grant] {
    input.user.role == "PARENT"
    
    # 获取家长绑定的子女
    bound_children := parent_bound_children[input.user.id]
    
    # 检查目标学生是否在绑定列表
    student_id := input.resource.student_id
    student_id == bound_children[_]
    
    grant := {
        "decision": "allow",
        "scope": "child_based",
        "permitted_children": bound_children
    }
}

# 默认拒绝：家长访问非绑定子女数据
default parent_data_access = false
parent_data_access[grant] {
    input.user.role == "PARENT"
    grant := {"decision": "deny", "reason": "student_not_bound_to_parent"}
}
```

##### 3. 敏感字段访问控制规则

```rego
# 敏感字段访问规则：身份证号、电话号码、完整地址
package school.auth

# 敏感字段列表
sensitive_fields := {"hkid", "phone", "full_address", "bank_account"}

# 判断是否为敏感字段
is_sensitive_field(field) {
    sensitive_fields[field]
}

# 敏感字段默认拒绝（需显式授权）
sensitive_field_access[grant] {
    is_sensitive_field(input.resource.field)
    
    # 检查用户角色是否有敏感字段查看权限
    allowed_roles := {"SCHOOL_ADMIN", "OFFICER", "SYSTEM"}
    input.user.role == allowed_roles[_]
    
    grant := {
        "decision": "allow",
        "warning": "sensitive_field_access_logged"
    }
}

sensitive_field_access[grant] {
    is_sensitive_field(input.resource.field)
    input.user.role == "TEACHER"
    
    # 教师仅能查看家长联系方式（同电话号码规则）
    input.resource.field == "parent_phone"
    
    grant := {
        "decision": "allow",
        "masked_display": true,
        "warning": "field_displayed_masked"
    }
}

# 完整地址：仅紧急情况经校务主任授权后可见
sensitive_field_access[grant] {
    input.resource.field == "full_address"
    input.context.emergency_flag == true
    input.user.role == "SCHOOL_ADMIN"
    
    grant := {
        "decision": "allow",
        "unmasked_display": true,
        "reason": "emergency_access_authorized"
    }
}

default sensitive_field_access = false
sensitive_field_access[grant] {
    is_sensitive_field(input.resource.field)
    grant := {"decision": "deny", "reason": "sensitive_field_access_denied"}
}
```

##### 4. 时间范围控制规则（财务数据学年隔离）

```rego
# 财务数据学年隔离规则
package school.auth

# 获取当前学年
current_academic_year := "2025-2026" {
    time.now_ns() >= time.parse("2006-01-02", "2025-09-01")[0]
} else = "2024-2025"

# 历史财务数据仅读权限
financial_data_access[grant] {
    input.resource.category == "financial"
    
    # 获取数据所属学年
    data_year := input.resource.academic_year
    
    # 当前学年：可读写
    data_year == current_academic_year
    write_actions := {"create", "update", "delete"}
    
    input.action == write_actions[_]
    
    # 需检查财务报销权限
    input.user.role == "OFFICER"
    
    grant := {"decision": "allow", "scope": "current_year_write"}
}

financial_data_access[grant] {
    input.resource.category == "financial"
    
    # 历史学年：仅读
    data_year := input.resource.academic_year
    data_year != current_academic_year
    input.action == "read"
    
    grant := {"decision": "allow", "scope": "historical_read_only"}
}

financial_data_access[grant] {
    input.resource.category == "financial"
    data_year := input.resource.academic_year
    data_year != current_academic_year
    input.action != "read"
    
    grant := {"decision": "deny", "reason": "historical_data_no_write_access"}
}
```

##### 5. 紧急例外规则（学生安全）

```rego
# 学生安全紧急情况：临时提升访问权限
package school.auth

# 紧急访问规则：学生安全相关可临时提升权限
emergency_access_override[grant] {
    input.context.emergency_type == "student_safety"
    
    # 仅校务主任或系统管理员可触发紧急访问
    allowed_roles := {"SCHOOL_ADMIN", "SYSTEM"}
    input.user.role == allowed_roles[_]
    
    # 记录紧急访问原因
    grant := {
        "decision": "allow",
        "override": true,
        "scope": "full_access",
        "audit_required": true,
        "reason": input.context.emergency_reason,
        "valid_until": time.add_date(time.now_ns() / 1000000000, 0, 0, 1)
    }
}
```

##### 6. 操作类型权限规则

```rego
# 操作类型权限：CREATE / READ / UPDATE / DELETE / EXPORT / PRINT
package school.auth

# 操作类型权限矩阵
operation_permissions[permission] {
    input.user.role == "TEACHER"
    input.resource.type == "student_record"
    
    permission := {
        "action": input.action,
        "allowed": allowed_actions,
        "conditions": ["class_scope", "own_class_only"]
    }
    
    # 教师可读、可更新本班学生记录
    allowed_actions := {"read", "update"}
    allowed_actions[input.action]
}

operation_permissions[permission] {
    input.user.role == "PARENT"
    input.resource.type == "student_record"
    
    permission := {
        "action": input.action,
        "allowed": {"read"},
        "conditions": ["child_scope"]
    }
    
    # 家长仅能读取绑定子女记录
    input.action == "read"
}

# 导出权限：需额外检查
export_permission[grant] {
    input.action == "export"
    
    # 检查是否在导出白名单
    allowed_exports := {
        {"role": "SCHOOL_ADMIN", "resource": "any"},
        {"role": "OFFICER", "resource": "financial"},
        {"role": "OFFICER", "resource": "attendance"},
        {"role": "TEACHER", "resource": "own_class_grades"}
    }
    
    match := {"role": input.user.role, "resource": input.resource.category}
    match == allowed_exports[_]
    
    grant := {"decision": "allow", "export_format": "xlsx"}
}

export_permission[grant] {
    input.action == "export"
    grant := {"decision": "deny", "reason": "export_not_allowed"}
}
```

##### 7. 完整授权评估入口

```rego
# 主授权评估入口
package school.auth

# 综合授权评估
default authz = {"decision": "deny", "reason": "no_matching_rule"}

authz[grant] {
    # 1. 检查账户状态
    account_status := check_account_status(input.user.id)
    account_status.active == true
    
    # 2. 评估班级/子女范围
    scope_result := teacher_class_scope[_]
    scope_result.decision == "allow"
    
    # 3. 评估敏感字段规则
    sensitive_result := sensitive_field_access[_]
    sensitive_result.decision == "allow"
    
    grant := {
        "decision": "allow",
        "user_id": input.user.id,
        "role": input.user.role,
        "resource": input.resource,
        "action": input.action,
        "policies_matched": ["account_active", "class_scope", "sensitive_field"],
        "evaluated_at": time.now_ns()
    }
}

authz[grant] {
    input.context.emergency_type == "student_safety"
    emergency_result := emergency_access_override[_]
    emergency_result.decision == "allow"
    
    grant := emergency_result
}

# 账户状态检查
check_account_status(user_id) = status {
    user := data.users[user_id]
    status := {
        "active": user.status == "active",
        "disabled": user.disabled == true,
        "expired": time.now_ns() > time.parse_rfc3339(user.account_expires_at)[0]
    }
}
```

##### ABAC规则测试用例

| 测试场景 | 输入 | 预期结果 |
|----------|------|---------|
| 教师A（教2A班）查看学生B（2A班）出勤记录 | role=TEACHER, class=2A | ✅ Allow, scope=class_based |
| 教师A查看学生C（3B班）出勤记录 | role=TEACHER, class=3B | ❌ Deny, student_not_in_assigned_class |
| 家长X绑定学生Y，家长X查看学生Y成绩 | role=PARENT, child_id=Y | ✅ Allow, scope=child_based |
| 家长X尝试查看未绑定的学生Z成绩 | role=PARENT, child_id=Z | ❌ Deny, student_not_bound_to_parent |
| 校务主任查看学生香港身份证 | role=SCHOOL_ADMIN, field=hkid | ✅ Allow, sensitive_field_access_logged |
| 教师尝试查看学生完整地址 | role=TEACHER, field=full_address | ❌ Deny, sensitive_field_access_denied |
| 家长查看2023-2024学年（历史）学费记录 | role=PARENT, year=2023-2024 | ✅ Allow, historical_read_only |
| 家长修改2023-2024学年学费记录 | role=PARENT, year=2023-2024, action=update | ❌ Deny, historical_data_no_write_access |
| 校务主任紧急情况访问学生完整地址 | role=SCHOOL_ADMIN, emergency=true | ✅ Allow, override=true, audit_required=true |
| 校务处同工导出生均成绩记录 | role=OFFICER, action=export, resource=grades | ✅ Allow, format=xlsx |
| 教师尝试导出全校成绩记录 | role=TEACHER, action=export, resource=all_grades | ❌ Deny, export_not_allowed |

**数据范围控制（ABAC规则 — 评审报告修正 — 权限设计缺陷#1）：**

> **重要修正：** F-USER-003 原ABAC规则仅为概念性描述，实际实现须引入规则引擎（推荐使用 **Open Policy Agent (OPA)**），支持动态、细粒度的属性基访问控制策略。OPA以Rego语言编写策略，支持实时评估，无需重新部署即可更新策略。

| 维度 | 规则 |
|------|------|
| 班级范围 | 教师仅能操作所教班级的学生数据 |
| 子女范围 | 家长仅能查看所绑定子女的数据 |
| 时间范围 | 财务数据按学年隔离，历史数据仅读 |
| 字段脱敏 | 身份证号、联系方式对非授权角色脱敏显示 |
| 操作类型 | CREATE / READ / UPDATE / DELETE / EXPORT / PRINT |
| 紧急例外 | 学生安全紧急情况可临时提升访问权限，须事后审计 |

**输入：**
| 字段 | 类型 | 来源 | 描述 |
|------|------|------|------|
| user_id | String | System | 请求方用户ID |
| target_resource | String | 输入 | 目标资源（模块/数据实体）|
| target_id | String | 输入 | 目标记录ID |
| action | Enum | 输入 | `view`, `create`, `update`, `delete`, `export`, `print` |
| context | Object | 自动 | 请求上下文（IP, 设备, 时间, 班级, 关联学生）|

**授权决策流程：**
```
请求进入 → 检查账户状态（未禁用/未过期）→ 检查数据范围（ABAC）
    → 匹配角色权限（RBAC）→ 匹配操作权限 → 评估ABAC额外规则
    → 资源归属检查（本人/本班/本部门）→ 决定 Allow / Deny
    → 记录授权审计日志
```

**敏感字段界面展示策略（PDPO合规 — 用户模拟反馈A-03/A-04/D-05）：**
> 即使RBAC授权某用户可访问某字段，界面展示时仍须遵循最小可见原则（Minimum Disclosure Principle）。

| 字段类别 | 展示规则 | 查看条件 | 审计要求 |
|----------|---------|---------|---------|
| **香港身份证号** | 默认显示：`A123456(X)` 格式脱敏 | 点击「查看」→ 二次确认（密码或OTP）| 每次查看均记录至 F-USER-005 审计日志 |
| **电话号码** | 默认显示前3后4位：`912***567` | 点击「查看」→ 二次确认 | 每次查看均记录 |
| **完整地址** | 完全隐藏（不显示任何内容）| 仅紧急情况（如学生安全）经校务主任授权后查看 | 每次授权均记录理由 |
| **出生日期** | 显示年份+月份：`2011-03` | 默认可见 | 无 |
| **家长联系方式 | 仅教师/校务主任可见，显示同电话号码规则 | 同上 | 同上 |

> **实现说明：** 所有敏感字段的「查看」操作均触发 F-USER-005 审计日志写入，记录：请求方用户ID、目标字段、查看时间戳、请求IP地址。任何人均无权绕过此规则。

**家校通讯功能（用户模拟反馈C-02/D-05 — 新增）：**
> 教师与家长之间的双向通讯须通过系统内消息通道，禁止通过个人社交软件（WhatsApp/微信）处理校务相关事务。

| 功能 | 描述 |
|------|------|
| 教师→家长消息 | 教师可在学生详情页点击「联系家长」，发送系统消息 |
| 家长→教师消息 | 家长可通过微信门户「发送消息」功能联系老师 |
| 对话历史 | 所有系统内消息保留6个月，可供校务主任查阅（不包含内容，仅元数据）|
| 权限控制 | 教师仅能联系其所教班级的学生家长，不可跨班联系 |

**输出：**
```json
{
  "authorization_result": {
    "decision": "allow",
    "user_id": "USR-2026-00010",
    "role": "TEACHER",
    "resource": "STUDENT_RECORD",
    "record_id": "STU-2023-00456",
    "action": "read",
    "scope": {
      "scope_type": "class_based",
      "permitted_classes": ["2A", "2B"],
      "record_grade": "S2",
      "record_class": "2A"
    },
    "fields_visible": ["name_zh", "hkid_masked", "attendance_rate", "exam_scores"],
    "fields_hidden": ["phone", "address", "parent_phone"],
    "matched_policy": "RbacTeacherRead + AbacClassScope",
    "evaluated_at": "2026-05-23T08:35:00+08:00"
  }
}
```

---

### Function F-USER-004: 会话与 Token 管理

**目的：** 安全管理用户会话生命周期，支持主动登出、会话失效和Token刷新

**Token类型：**

| Token类型 | 用途 | 有效期 | 存储 |
|-----------|------|--------|------|
| Access Token | API鉴权 | 30分钟 | 内存/HttpOnly Cookie |
| Refresh Token | 续期Access Token | 7天 | HttpOnly Cookie |
| API Key | 系统间调用 | 可配置 | 加密存储 |
| SSO Assertion | 单点登录 | 8小时 | Session |

**会话管理功能：**
- 同时会话数量限制（同一用户最多3个并发会话）
- 强制登出（管理员/用户主动终止所有或指定会话）
- 异地登录检测（同一账户在新城市登录 → 推送告警）
- 长时间空闲自动登出（默认60分钟，可配置）
- Token主动作废（change_password / 权限变更 → 所有会话失效）

**输出：**
```json
{
  "sessions": [
    {
      "session_id": "SES-2026-00123-A",
      "device": "Chrome on Windows",
      "ip_address": "203.0.113.42",
      "location": "Hong Kong",
      "created_at": "2026-05-23T08:30:00+08:00",
      "last_active": "2026-05-23T09:15:00+08:00",
      "expires_at": "2026-05-23T14:30:00+08:00",
      "is_current": true
    },
    {
      "session_id": "SES-2026-00123-B",
      "device": "Feishu App on iOS",
      "ip_address": "198.51.100.17",
      "location": "Shenzhen",
      "created_at": "2026-05-22T18:00:00+08:00",
      "last_active": "2026-05-23T07:45:00+08:00",
      "expires_at": "2026-05-29T18:00:00+08:00",
      "is_current": false,
      "risk_alert": "new_location"
    }
  ],
  "active_session_count": 2,
  "max_allowed_sessions": 3
}
```

---

### Function F-USER-005: 审计日志与登录记录

**目的：** 记录所有身份认证事件和敏感操作，供安全审计与合规追溯

**登录事件记录：**

| 事件类型 | 触发条件 | 记录字段 |
|----------|----------|---------|
| `LOGIN_SUCCESS` | 认证成功 | user_id, IP, 设备, 地理位置, 认证方式 |
| `LOGIN_FAILED` | 密码/OTP错误 | user_id, IP, 错误类型, 失败次数 |
| `LOGIN_LOCKED` | 账户锁定 | user_id, IP, 锁定原因, 解锁时间 |
| `LOGOUT` | 用户主动登出 | user_id, session_id, 登出方式 |
| `SESSION_EXPIRED` | 会话超时 | user_id, session_id |
| `PASSWORD_CHANGED` | 密码更新 | user_id, 变更者, IP |
| `MFA_ENABLED` | MFA绑定成功 | user_id, MFA类型, IP |
| `UNAUTHORIZED_ACCESS` | 越权访问尝试 | user_id, 目标资源, 操作, 拒绝原因 |

**敏感操作审计：**

| 类别 | 记录操作 |
|------|---------|
| 用户管理 | 创建/修改/删除用户, 角色变更, 权限变更 |
| 数据导出/打印 | 批量导出学生资料, 打印成绩表 |
| 财务操作 | 费用减免, 退款, 报销审批 |
| 系统配置 | 开关功能模块, 修改业务规则 |
| **敏感字段查看（新增）** | **身份证号查看、电话号码查看、完整地址查看（需校务主任授权）** |

**家长通话记录规范（用户模拟反馈A-07 — 新增）：**
> 所有校务处人员与家长之间的通话，须在系统中留下结构化记录，不得仅以自由文本备注代替。

| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| call_duration_minutes | Integer | **是** | 通话时长（分钟） |
| call_result | Enum | **是** | `resolved`（已解决）/ `follow_up_required`（需跟进）/ `escalated`（升级处理）|
| next_follow_up_date | Date | 条件（当call_result=follow_up_required时）| 下次跟进日期 |
| parent_sentiment | Enum | 可选 | `positive` / `neutral` / `negative`（家长情绪，用于服务质素分析）|

**输出：**
```json
{
  "audit_log": {
    "log_id": "AUD-2026-05-23-001234",
    "timestamp": "2026-05-23T08:35:42+08:00",
    "event_type": "UNAUTHORIZED_ACCESS",
    "severity": "medium",
    "actor": {
      "user_id": "USR-2026-00015",
      "role": "TEACHER",
      "name_zh": "李老師",
      "session_id": "SES-2026-00089-A"
    },
    "target": {
      "resource": "STUDENT_RECORD",
      "record_id": "STU-2023-00789",
      "student_name": "黃小明"
    },
    "action": "export",
    "decision": "denied",
    "reason": "student_not_in_assigned_class",
    "ip_address": "203.0.113.42",
    "user_agent": "Mozilla/5.0...",
    "metadata": {
      "requested_fields": ["hkid", "address", "parent_phone"],
      "permitted_fields": ["name_zh", "class"]
    }
  }
}
```

**日志保留策略：**
| 事件类型 | 保留期 | 存储位置 |
|----------|--------|---------|
| 登录事件 | 3年 | 主数据库 + SIEM |
| 权限变更 | 7年 | 主数据库 |
| 敏感数据操作 | 7年 | 主数据库 + SIEM |
| 系统管理操作 | 7年 | 主数据库 + SIEM |

---

### Function F-USER-006: 密码与凭证重置

**目的：** 为用户提供安全自助的密码找回和凭证重置渠道

**重置流程 — 邮箱验证码：**
```
用户请求重置 → 验证账号存在 → 发送重置链接至注册邮箱 
    → 用户点击链接（15分钟内有效）→ 验证Token 
    → 输入新密码 → 密码复杂度校验 → 更新密码 
    → 作废所有现有会话 → 发送确认邮件 → 记录审计日志
```

**重置流程 — 短信OTP：**
```
用户请求重置 → 验证账号 + 手机号 → 发送6位OTP 
    → 验证OTP（5分钟有效，3次错误锁定）→ 输入新密码 
    → 更新密码 → 作废所有现有会话 → 发送确认短信
```

**管理员代重置：**
- 仅 `SCHOOL_ADMIN` 或 `SYSTEM` 可执行
- 需要二次授权（密码 + 手机验证码）
- 被重置用户将收到邮件/短信通知
- 完整操作记录于审计日志

**家长密码重置特别说明（用户模拟反馈D-06）：**
> 家长用户的「家长密码」（用于多子女账号关联验证）独立于登录密码，找回流程如下：

| 找回方式 | 适用场景 | 验证方式 |
|----------|---------|---------|
| 短信OTP | 家长已绑定手机号 | 发送6位OTP至注册手机，验证后重置 |
| 到校办理 | 家长无法收到短信（如更换手机号）| 校务处人工验证身份后重置 |

**输出：**
```json
{
  "password_reset_result": {
    "reset_id": "PWR-2026-05-23-5678",
    "user_id": "USR-2026-00001",
    "method": "email_otp",
    "status": "completed",
    "email_sent_to": "c***f@school.edu.hk",
    "completed_at": "2026-05-23T08:40:00+08:00",
    "sessions_invalidated": 3,
    "notification_sent": true,
    "next_password_expires": "2026-08-21"
  }
}
```

---

### Function F-USER-007: 权限变更审批流程

**目的：** 对高风险权限变更（如授权他人超范围访问）进行审批控制

**需要审批的敏感权限操作（用户模拟反馈A-01 — 新增证明文件要求）：**

| 操作 | 触发条件 | 审批人 | 证明文件 | 二次认证 |
|------|----------|--------|---------|---------|
| 临时提升为校务主任 | 任何用户 | 校长 | 必须：人事任职通知 | **必须：短信OTP或硬件Token** |
| 跨班级数据访问授权 | 教师跨班查看 | 校务主任 | 必须：活动通知/工作安排证明 | **必须：短信OTP** |
| 数据导出权限授予 | 批量导出授权 | 校务主任 | 必须：数据使用说明 | **必须：短信OTP** |
| 系统管理员权限变更 | SYSTEM角色变更 | 校长 + 校务主任 | 必须：权限变更申请单 | **必须：硬件Token或双人短信OTP** |
| 家长账户关联学生解绑 | 账户异常 | 校务主任 | 必须：情况说明 | **必须：短信OTP** |

> **评审报告修正 — 权限设计缺陷#2：** 所有敏感权限变更审批均须二次认证（短信OTP或硬件Token），防止账号被盗后恶意提升权限。二次认证在审批人提交审批时触发，而非申请提交时。

> **证明文件规范：** 支持格式：PDF、JPG、PNG，单个文件≤10MB。审批人须审查附件后方可批准。无证明文件的申请将被系统自动退回。

**输出：**
```json
{
  "privilege_escalation": {
    "request_id": "ESC-2026-05-23-001",
    "requester": {
      "user_id": "USR-2026-00008",
      "name_zh": "陳主任",
      "role": "OFFICER"
    },
    "target_user": {
      "user_id": "USR-2026-00015",
      "name_zh": "李老師",
      "role": "TEACHER"
    },
    "requested_privilege": {
      "type": "cross_class_data_access",
      "target_classes": ["2A", "2B", "2C"],
      "duration": "2026-05-23 至 2026-06-30",
      "justification": "協助處理跨境學生活動報名"
    },
    "status": "pending_approval",
    "approvers": [
      {"role": "校務主任", "user_id": "USR-2026-00001", "status": "pending"}
    ],
    "created_at": "2026-05-23T09:00:00+08:00",
    "deadline": "2026-05-23T17:00:00+08:00"
  }
}
```

---

## 第七部分：Module 7 — AI助理及自动化

（原有第六部分顺延）

### Function F-AI-001: 自然语言查询理解 (NLU)

**意图分类：**
```
校車相關 (Bus):
├── 校車時間查詢 (bus_time)
├── 校車路線查詢 (bus_route)
├── 校車延誤通知 (bus_delay)
└── 申請校車服務 (bus_apply)

午膳相關 (Lunch):
├── 午膳餐單查詢 (lunch_menu)
├── 特殊飲食安排 (lunch_special)
└── 更改午膳 (lunch_change)

費用相關 (Fee):
├── 繳費方式 (payment_method)
├── 欠費查詢 (outstanding_fee)
└── 申請資助 (subsidy_apply)
```

**AI处理流程：**
```
用户输入 → 语言检测 → 意图分类 → 实体提取 → 槽位填充 
    ↓
意图明确？ → 是 → 匹配FAQ / 执行功能 / 生成回复
    ↓ (否)
置信度<0.8？ → 是 → 转人工处理
    ↓ (否)
多意图识别 → 请求澄清
```

---

### Function F-AI-002: FAQ智能匹配

**FAQ数据库结构：**
| 字段 | 类型 | 描述 |
|------|------|------|
| faq_id | String | 唯一ID |
| category | Enum | 主题类别 |
| question_zh | String | 繁体中文问题 |
| question_en | String | 英文问题 |
| answer | Object | 多格式答案 |
| keywords | Array | 搜索关键词 |
| trigger_intents | Array | 意图代码 |
| view_count | Integer | 浏览次数 |
| helpful_count | Integer | 反馈有用次数 |

**匹配算法：**
```python
def match_faq(query_vector, faq_vectors):
    # 1. 关键词精确匹配
    # 2. TF-IDF相似度
    # 3. 语义嵌入相似度（同义词）
    # 4. 基于意图的路由
    
    scores = []
    for faq in faq_database:
        keyword_score = keyword_match(query, faq.keywords)
        tfidf_score = cosine_similarity(query_tfidf, faq.tfidf)
        semantic_score = cosine_similarity(query_embedding, faq.embedding)
        intent_score = intent_match(query_intent, faq.trigger_intents)
        
        final_score = (
            keyword_score * 0.3 +
            tfidf_score * 0.2 +
            semantic_score * 0.3 +
            intent_score * 0.2
        )
        scores.append((faq, final_score))
    
    return sorted(scores, key=lambda x: x[1], reverse=True)
```

---

### Function F-AUTO-001: 周期性任务触发器

**任务触发配置：**
```yaml
scheduled_tasks:
  # 每日任务
  - name: "晨检仪表板刷新"
    trigger: "daily @ 06:30"
    action: "refresh_dashboard_data"
    
  - name: "家长查询摘要"
    trigger: "daily @ 18:00"
    action: "generate_inquiry_summary"
    
  # 每周任务  
  - name: "代课统计报告"
    trigger: "weekly: every_friday 16:00"
    action: "generate_absence_report"
    
  # 每月任务
  - name: "月费缴费提醒"
    trigger: "monthly: 15th 09:00"
    action: "send_fee_reminder"
```

---

### Function F-AUTO-002: 智能提醒系统

**提醒级别：**
| 级别 | 渠道 | 时机 | 升级 |
|------|------|------|------|
| INFO | App Push, SMS | 可配置 | 无 |
| NORMAL | App, Email, SMS | 可配置 | +24小时升级 |
| URGENT | App, SMS, 电话 | 立即 | +2小时升级 |
| CRITICAL | 全渠道+学校领导 | 立即 | 立即升级 |

**消息送达回执机制（用户模拟反馈P0-01 — 新增）：**
> 所有通过微信/App推送的重要通知，须建立消息送达回执机制，并对高优先级通知启用短信备用渠道。

| 功能 | 说明 |
|------|------|
| 送达回执 | 家长打开通知 → 系统标记"已读"，显示「家长已读」状态 |
| 未读跟进 | 24小时内未读 → 自动重发一次 + 短信备用 |
| 失败告警 | 推送持续失败 → 告警至校务处，显示「通知发送失败」|
| 短信备用 | 对高优先级通知（迟到/安全/紧急）同时发送短信，确保触达 |

**短信备用渠道规则（用户模拟反馈P0-01 — 新增）：**
> 以下场景须同时发送短信，确保通知触达：

| 场景 | 渠道优先级 |
|------|-----------|
| 学生校车延误 | 微信推送 → 短信备用 |
| 学生出勤异常（连续迟到/缺席）| 微信推送 → 短信备用 |
| 紧急通知（台风/停课等）| 短信优先 + 微信推送 |
| 成绩发布 | 微信推送（无备用）|
| 日常缴费提醒 | 微信推送（无备用）|

**Token健康检查（用户模拟反馈P0-01 — 新增）：**
> 系统每24小时自动检查微信渠道token有效性，异常时发送告警至校务主任。

| 检查频率 | 异常处理 |
|----------|---------|
| 每24小时 | 微信token异常 → 立即告警至校务主任（邮件+短信）|

---

### Function F-AI-003: OCR文档识别

**支持的文档类型：**
| 文档 | 提取字段 | 准确率目标 |
|------|---------|-----------|
| 出生证明书 | 姓名, 性别, 出生日期, 父母姓名 | >98% |
| 香港身份证 | 姓名, 身份证号, 出生日期, 性别 | >99% |
| 学校报告表 | 学生姓名, 班别, 各科成绩, 操行等级 | >95% |
| 医疗证明书 | 学生姓名, 医生姓名, 诊断, 建议休息日数 | >90% |
| 保险证书 | 保单号码, 生效日期, 到期日期, 受保人 | >98% |

---

## 第八部分：Module 5 — 整合及合规

### Function F-INT-001: WebSAMS数据同步

**同步模式：**
| 模式 | 频率 | 数据量 | 用途 |
|------|------|--------|------|
| 实时 | 事件驱动 | 小量 | 学生更新 |
| 定时 | 每日23:00 | 中量 | 出勤, 成绩 |
| 批量 | 每周/每月 | 大量 | 年度处理 |
| 按需 | 手动触发 | 可变 | 特定数据拉取 |

**同步数据域：**
```
学生资料 (Student Data)
├── 基本资料 ←→ 双向同步
├── 学籍资料 ←→ 双向同步
├── 出席记录 ←→ 学校 → WebSAMS
├── 成绩资料 ←→ 学校 → WebSAMS
└── 健康记录 ←→ 学校 → WebSAMS
```

---

### Function F-INT-002: eClass系统集成

**eClass API端点（消费）：**
```
GET  /api/attendance/students/{class_id}/{date}
GET  /api/attendance/student/{student_id}/history
POST /api/attendance/record
GET  /api/homework/class/{class_id}
GET  /api/homework/student/{student_id}
POST /api/homework
GET  /api/communication/messages/{user_type}
POST /api/communication/send
```

---

### Function F-COMP-001: 隐私条例合规检查

**数据分类：**
| 级别 | 描述 | 示例 | 处理规则 |
|------|------|------|----------|
| P1 | 高度敏感 | 健康资料, 身份证号, 家庭状况 | 加密, 双重授权, 完整审计 |
| P2 | 中度敏感 | 成绩, 奖惩记录, 联络方式 | 加密, 用途限制 |
| P3 | 一般资料 | 姓名, 班别, 出席率 | 标准保护 |

**合规检查：**
```python
def pdpo_compliance_check(action, data_class, user_role, purpose):
    checks = []
    
    # 目的限制 (Purpose Limitation)
    allowed_purposes = {
        "P1": ["education_administration", "healthcare", "emergency"],
        "P2": ["education_administration", "communication", "reporting"],
        "P3": ["education_administration", "communication", "public"]
    }
    checks.append(check_purpose_legitimate(...))
    
    # 资料最小化 (Data Minimization)
    checks.append(check_data_minimization(...))
    
    # 存取控制 (Access Control)
    checks.append(check_access_permission(...))
    
    # 保留期限 (Retention Period)
    checks.append(check_retention_compliance(...))
    
    return all(checks)
```

---

### Function F-COMP-002: 双人见证流程

**触发条件：**
| 条件 | 阈值 | 所需见证人 |
|------|------|-----------|
| 现金收取 | 任何金额 | 1名员工+1名见证人 |
| 现金支付 >HK$500 | >HK$500 | 2名授权员工 |
| 备用金补充 | 任何 | 2名授权员工 |
| 保险箱开启 | 任何 | 2名授权员工 |
| 支票签署 | 任何 | 2名授权签署人 |

---

### Function F-COMP-003: 审计日志管理

**记录的事件：**
| 类别 | 事件 | 保存期 |
|------|------|--------|
| 资料存取 | 查询、下载、打印个人资料 | 7年 |
| 资料修改 | 新增、更新、删除记录 | 7年 |
| 系统操作 | 登入、登出、权限变更 | 5年 |
| 财务交易 | 收款、付款、报销 | 7年 |
| 合规事件 | 同意书查阅、销毁记录 | 7年 |

---

### Function F-BACK-001: 自动备份管理

**备份计划：**
| 备份类型 | 频率 | 保存期 | 存储 |
|---------|------|--------|------|
| 每日增量 | 每日01:00 | 7天 | 本地+云端 |
| 每周完整 | 周日02:00 | 4周 | 本地+云端 |
| 每月完整 | 每月1日 | 12个月 | 云端（加密） |
| 学年结束 | 8月31日 | 7年 | 异地存档 |

---

## 第九部分：Module 8 — 多语言支持系统

### 8.1 模块概述

**Module 8 (Multilingual Support / i18n)** 是系统的横向基础设施模块，为所有功能模块提供多语言服务能力，确保系统能够服务香港多元语言环境下的所有用户。

**支持语言（评审报告修正 — 用户模拟反馈D-08）：**
- 🇭🇰 **繁体中文 (zh-HK)** — 香港本地默认语言，符合香港教育局官方文件标准
- 🇭🇰 **粤语口语 (zh-HK-Cantonese)** — **新增：** 家长/学生门户界面支持粤语口语表达（非正式书面语），系统自动识别粤语口语关键词并以口语语气回复，如「幾點到？」、「今日 lunch 係咩？」
- 🇨🇳 **简体中文 (zh-CN)** — 内地用户常用语言
- 🇬🇧 **英语 (en)** — 国际学生、外籍教师及跨境沟通场景

> **评审报告修正说明：** 原有 zh-HK 仅定义为繁体中文，现补充粤语口语（zh-HK-Cantonese）作为独立界面模式，适用于家长/学生微信门户。粤语口语模式不改变数据存储语言（所有数据以标准繁体中文存储），仅在展示层使用口语语气和常用粤语词汇。

**核心价值：**
- **一致性：** 统一的翻译管理，避免同一术语在不同模块出现不同翻译
- **可扩展性：** 模块化架构，支持未来新增语言（如粤语拼音、葡语）
- **上下文感知：** 根据用户角色和场景自动推荐语言
- **AI 增强：** 利用 LLM 提供上下文感知的智能翻译建议

**涉及函数：**
| 函数ID | 函数名称 | 优先级 |
|--------|---------|--------|
| F-I18N-001 | 多语言框架与翻译管理 | P0 |
| F-I18N-002 | 语言检测与自动切换 | P0 |
| F-I18N-003 | 实时内容翻译 (LLM) | P1 |
| F-I18N-004 | 区域化与格式本地化 (Locale) | P1 |

---

### Function F-I18N-001: 多语言框架与翻译管理

**函数ID：** F-I18N-001
**函数名称：** 多语言框架与翻译管理
**模块：** MOD-I18N-001 (Module 8)
**子模块：** 基础框架
**优先级：** P0
**责任人：** 系统架构团队

#### 功能描述

建立统一的多语言框架（i18n Framework），集中管理系统中所有用户可见文本的翻译，支持繁体中文、简体中文、英语三种语言，并为未来扩展新语言预留架构。

#### 用户故事

> **作为** 校务处同工，**我想要** 在系统中切换界面语言，**以便** 我能用自己熟悉的语言（繁体/简体/英文）进行日常工作，提升操作效率。

#### 业务规则

1. **翻译资源结构**
   - 每种语言对应一个独立的翻译资源文件（JSON / i18next 格式）
   - 翻译键（Translation Key）统一采用 `模块.子模块.概念` 命名规范，例如：`dashboard.attendance.title = "学生出勤概览"`
   - 翻译值不得包含占位符之外的任何业务逻辑

2. **语言覆盖要求**
   - 所有 P0 和 P1 优先级的用户界面文本必须有三种语言（zh-HK, zh-CN, en）的完整翻译
   - P2 及以下优先级的文本优先保证 zh-HK 和 zh-CN，可选 en

3. **翻译一致性**
   - 建立核心术语表（Glossary），确保相同概念在不同模块使用一致翻译
   - 关键业务术语（如学生状态、费用类型）须经专人审核

4. **翻译工作流**
   - 初始翻译 → AI 辅助审校 → 人工审核 → 发布上线
   - 翻译更新须记录版本历史，支持回滚

#### 用户界面与交互

| 场景 | 行为描述 |
|------|----------|
| **语言切换入口** | 页面右上角语言选择器，显示为国旗/地区图标（如 🇭🇰 / 🇨🇳 / 🇬🇧） |
| **切换响应** | 页面刷新后立即显示目标语言，无延迟加载翻译文件 |
| **默认语言** | 未登录用户默认显示 zh-HK；已登录用户使用其个人偏好语言 |
| **语言偏好保存** | 用户选择语言后，系统自动保存至用户配置，后续访问自动应用 |

#### 数据结构

**翻译资源文件结构：**
```json
{
  "zh-HK": {
    "dashboard": {
      "title": "智能校务仪表板",
      "attendance": {
        "title": "學生出勤概覽",
        "present": "已出席",
        "absent": "缺席",
        "late": "遲到",
        "early_leave": "早退"
      },
      "lunch": {
        "title": "午膳訂購彙總",
        "ordered": "已訂購",
        "pending": "待確認"
      }
    },
    "common": {
      "save": "儲存",
      "cancel": "取消",
      "confirm": "確認",
      "search": "搜尋"
    }
  },
  "zh-CN": {
    "dashboard": {
      "title": "智能校务仪表板",
      "attendance": {
        "title": "学生出勤概览",
        "present": "已出席",
        "absent": "缺席",
        "late": "迟到",
        "early_leave": "早退"
      },
      "lunch": {
        "title": "午膳订购汇总",
        "ordered": "已订购",
        "pending": "待确认"
      }
    },
    "common": {
      "save": "保存",
      "cancel": "取消",
      "confirm": "确认",
      "search": "搜索"
    }
  },
  "en": {
    "dashboard": {
      "title": "Smart School Admin Dashboard",
      "attendance": {
        "title": "Student Attendance Overview",
        "present": "Present",
        "absent": "Absent",
        "late": "Late",
        "early_leave": "Early Leave"
      },
      "lunch": {
        "title": "Lunch Order Summary",
        "ordered": "Ordered",
        "pending": "Pending Confirmation"
      }
    },
    "common": {
      "save": "Save",
      "cancel": "Cancel",
      "confirm": "Confirm",
      "search": "Search"
    }
  }
}
```

**翻译键注册表：**
```sql
CREATE TABLE translation_keys (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  key_path VARCHAR(200) NOT NULL UNIQUE,
  module VARCHAR(50) NOT NULL,
  description TEXT,
  priority VARCHAR(10) DEFAULT 'P1',  -- P0 | P1 | P2
  status VARCHAR(20) DEFAULT 'ACTIVE', -- ACTIVE | DEPRECATED | PENDING
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);

CREATE TABLE translations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  key_id UUID NOT NULL REFERENCES translation_keys(id) ON DELETE CASCADE,
  locale VARCHAR(10) NOT NULL,  -- zh-HK | zh-CN | en
  value TEXT NOT NULL,
  is_approved BOOLEAN DEFAULT FALSE,
  approved_by UUID REFERENCES users(id),
  translator_id UUID REFERENCES users(id),
  version INTEGER DEFAULT 1,
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(key_id, locale)
);

CREATE TABLE glossary_terms (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  term_en VARCHAR(100) NOT NULL,
  term_zh_hk VARCHAR(100),
  term_zh_cn VARCHAR(100),
  definition TEXT,
  domain VARCHAR(50),           -- education, finance, attendance
  is_verified BOOLEAN DEFAULT FALSE,
  created_at TIMESTAMP DEFAULT NOW()
);
```

#### API 接口

| 方法 | 路径 | 描述 |
|------|------|------|
| GET | `/api/v1/i18n/translations` | 获取指定语言的翻译资源 |
| GET | `/api/v1/i18n/translations/:locale` | 获取特定语言的完整翻译文件 |
| GET | `/api/v1/i18n/glossary` | 获取术语表 |
| POST | `/api/v1/i18n/translations` | 提交新翻译或更新（需审核） |
| PUT | `/api/v1/i18n/translations/:id/approve` | 审核通过翻译 |
| GET | `/api/v1/i18n/keys` | 列出所有翻译键及翻译状态 |
| GET | `/api/v1/i18n/keys/missing` | 获取缺失翻译的键列表 |

```typescript
// GET /api/v1/i18n/translations/:locale
interface GetTranslationsResponse {
  locale: 'zh-HK' | 'zh-CN' | 'en';
  version: string;        // 翻译文件版本号（基于最后更新时间）
  translations: Record<string, any>;  // 嵌套翻译对象
  missing_keys?: string[];  // 该语言缺失的键（用于调试）
}

// GET /api/v1/i18n/translations
interface GetAllTranslationsResponse {
  'zh-HK': Record<string, any>;
  'zh-CN': Record<string, any>;
  'en': Record<string, any>;
  generated_at: string;
}
```

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 用户可在界面右上角切换三种语言 | 手动切换并验证所有可见文本 |
| 2 | 语言偏好保存至用户配置 | 切换语言后登出再登录，验证默认语言 |
| 3 | 翻译文件按模块组织，无重复键 | 代码审查 + 自动化检测 |
| 4 | 翻译更新不需重新部署 | 通过管理界面更新翻译后立即生效 |
| 5 | 术语表覆盖所有核心业务术语（≥ 100条） | 术语表条目数量验证 |

---

### Function F-I18N-002: 语言检测与自动切换

**函数ID：** F-I18N-002
**函数名称：** 语言检测与自动切换
**模块：** MOD-I18N-001 (Module 8)
**子模块：** 语言检测
**优先级：** P0
**责任人：** 系统架构团队

#### 功能描述

根据用户的浏览器语言设置、IP 地理位置和历史偏好，自动检测并设置最佳界面语言，减少用户手动切换成本。

#### 业务规则

1. **语言检测优先级**
   - 第1优先：用户手动选择并保存的语言偏好（`user.preferred_locale`）
   - 第2优先：URL 参数 `?lang=zh-HK|zh-CN|en`
   - 第3优先：浏览器 `Accept-Language` 请求头
   - 第4优先：IP 地理位置（香港 → zh-HK，中国大陆 → zh-CN，其他 → en）
   - 第5优先：系统默认 zh-HK

2. **语言代码规范**
   - 支持：`zh-HK`（默认）、`zh-CN`、`en`
   - 旧版兼容：`zh-TW` → 自动映射至 `zh-HK`；`zh` → 根据检测结果映射

3. **自动切换提示**
   - 当系统自动检测到的语言与用户当前语言不同时，可选择显示切换提示横幅
   - 用户可选择"切换"或"保持当前语言"

#### 用户界面与交互

| 场景 | 行为描述 |
|------|----------|
| **新用户首次访问** | 根据浏览器语言自动设置界面语言，显示切换提示横幅 |
| **URL 带语言参数** | 优先使用 URL 参数，覆盖所有自动检测逻辑 |
| **切换提示横幅** | 显示"我们检测到您可能偏好 [语言]，是否切换？" |
| **语言不一致告警** | 管理后台显示语言使用分布统计 |

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 浏览器 Accept-Language 为 zh-CN 时自动显示简体中文 | 修改浏览器语言设置后访问 |
| 2 | URL `?lang=en` 参数优先于浏览器语言 | 携带参数访问并验证 |
| 3 | 用户手动切换后偏好被保存 | 切换语言 → 刷新页面 → 验证保持 |
| 4 | IP 位于内地时默认 zh-CN | 使用 VPN 模拟内地 IP 访问 |

---

### Function F-I18N-003: 实时内容翻译 (LLM)

**函数ID：** F-I18N-003
**函数名称：** 实时内容翻译 (LLM)
**模块：** MOD-I18N-001 (Module 8)
**子模块：** AI 翻译
**优先级：** P1
**责任人：** AI 团队

#### 功能描述

利用 LLM（Coze/OpenAI）提供实时智能翻译，不仅翻译文本，更根据上下文保持语气、专业术语一致性。适用于用户生成的动态内容（如家长留言、AI 回复）翻译。

#### 业务规则

1. **翻译场景**
   - 用户生成内容（UGC）的实时翻译
   - AI 助手的双语回复
   - 通知消息的多语言推送
   - 文档附件的文字提取后翻译

2. **质量要求**
   - 使用专门针对香港教育场景微调的 LLM
   - 翻译后须保留原文语义，不可遗漏关键信息
   - 专业术语须与系统术语表保持一致

3. **性能要求**
   - 实时翻译（< 3 秒）用于聊天场景
   - 批量翻译用于文档场景（< 30 秒/页）

4. **翻译缓存**
   - 相同内容相同语言对仅翻译一次，结果缓存
   - 缓存有效期：24 小时
   - 缓存键：`SHA256(original_text + source_locale + target_locale)`

#### 技术实现

```typescript
// 实时翻译服务
async function translateContent(
  text: string,
  sourceLocale: 'zh-HK' | 'zh-CN' | 'en',
  targetLocale: 'zh-HK' | 'zh-CN' | 'en'
): Promise<TranslationResult> {
  // 1. 检查缓存
  const cacheKey = generateCacheKey(text, sourceLocale, targetLocale);
  const cached = await redis.get(`i18n:cache:${cacheKey}`);
  if (cached) return JSON.parse(cached);

  // 2. 调用 LLM 翻译
  const result = await llm.translate({
    text,
    source: sourceLocale,
    target: targetLocale,
    context: 'school_admin_hk',  // 香港学校管理场景
    glossary: await getGlossary(sourceLocale, targetLocale)
  });

  // 3. 缓存结果
  await redis.setex(`i18n:cache:${cacheKey}`, 86400, JSON.stringify(result));

  return result;
}
```

#### API 接口

```typescript
// POST /api/v1/i18n/translate
interface TranslateRequest {
  text: string;
  source_locale: 'zh-HK' | 'zh-CN' | 'en';
  target_locale: 'zh-HK' | 'zh-CN' | 'en';
  use_cache?: boolean;  // default: true
}

interface TranslationResult {
  original: string;
  translated: string;
  source_locale: string;
  target_locale: string;
  confidence: number;
  cached: boolean;
  glossary_applied: number;  // 应用了多少条术语表条目
}

// POST /api/v1/i18n/translate/batch
interface BatchTranslateRequest {
  texts: string[];
  source_locale: string;
  target_locale: string;
}
```

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 相同内容第二次翻译命中缓存 | 日志验证 `cached: true` |
| 2 | 翻译应用术语表，专业术语一致 | 测试"旷课"→"Absence without leave" |
| 3 | 批量翻译支持最多 50 条/请求 | API 测试 |
| 4 | 翻译错误率 < 1%（人工抽检） | 随机抽样 100 条进行人工审核 |

---

### Function F-I18N-004: 区域化与格式本地化 (Locale)

**函数ID：** F-I18N-004
**函数名称：** 区域化与格式本地化 (Locale)
**模块：** MOD-I18N-001 (Module 8)
**子模块：** 格式本地化
**优先级：** P1
**责任人：** 系统架构团队

#### 功能描述

除文本翻译外，对数字、货币、日期、时间、文件大小等数据的显示格式进行本地化适配，确保符合各地区用户的阅读习惯。

#### 业务规则

| 数据类型 | zh-HK 格式 | zh-CN 格式 | en 格式 |
|----------|------------|------------|---------|
| **日期** | 2026年5月25日 / 25/05/2026 | 2026年5月25日 | May 25, 2026 |
| **时间** | 上午10:30 / 10:30 am | 上午10:30 / 10:30 AM | 10:30 AM |
| **货币** | HK$1,500 | ¥1,500 / RMB ¥1,500 | HK$1,500.00 |
| **数字** | 1,234.56 | 1,234.56 | 1,234.56 |
| **百分比** | 85.5% | 85.5% | 85.5% |
| **学号** | 保留原始格式（数字+字母） | 保留原始格式 | 保留原始格式 |
| **文件大小** | 1.5 MB / 2.3 GB | 1.5 MB / 2.3 GB | 1.5 MB / 2.3 GB |

#### 技术实现

```typescript
// 本地化格式化工具
import { format } from 'date-fns';
import { zhHK, zhCN, enUS } from 'date-fns/locale';

const localeConfigs = {
  'zh-HK': {
    dateFormat: 'yyyy年M月d日',
    timeFormat: 'a h:mm',
    currency: 'HKD',
    currencySymbol: 'HK$',
    locale: zhHK,
  },
  'zh-CN': {
    dateFormat: 'yyyy年M月d日',
    timeFormat: 'A h:mm',
    currency: 'CNY',
    currencySymbol: '¥',
    locale: zhCN,
  },
  'en': {
    dateFormat: 'MMMM d, yyyy',
    timeFormat: 'h:mm a',
    currency: 'HKD',
    currencySymbol: 'HK$',
    locale: enUS,
  }
};

// 使用示例
formatDate(date: Date, locale: string): string {
  const config = localeConfigs[locale];
  return format(date, config.dateFormat, { locale: config.locale });
}

formatCurrency(amount: number, locale: string): string {
  const config = localeConfigs[locale];
  return `${config.currencySymbol}${amount.toLocaleString(locale)}`;
}
```

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 日期格式符合各语言/地区习惯 | 切换语言后验证"2026年5月25日"等 |
| 2 | 货币符号正确显示 | 财务模块切换语言后验证 HK$ / ¥ |
| 3 | 数字格式统一（千位分隔符） | 切换语言后验证大数字显示 |
| 4 | PDF 导出内容与界面语言一致 | 生成报表验证语言和格式 |

---

## 第十部分：Module 9 — 新增功能 (v1.6.0 评审报告补全)

> **Module 9 (New Functions / v1.6.0)** 是基于功能评审报告（2026-06-06）的缺漏需求清单，新增的6个核心功能模块。其余9项缺漏需求（P2/P3优先级）列入未来版本计划。

### 10.1 模块概述

| 属性 | 描述 |
|------|------|
| 模块名称 | New Functions — v1.6.0 评审报告补全 (New Functions from Functional Review Report v1.6.0) |
| 模块ID | MOD-NEW-001 |
| 优先级 | P0/P1 |
| 用户 | 全体校务人员 |
| 评审依据 | 功能评审报告 REVIEW-FUNC-001（2026-06-06） |

---

### Function F-NEW-001: 一键灾难恢复与业务连续性

**函数ID：** F-NEW-001
**函数名称：** 一键灾难恢复与业务连续性
**模块：** MOD-NEW-001 (Module 9)
**子模块：** 灾难恢复
**优先级：** P0（评审报告P0优先级清单第15项）
**责任人：** 系统架构团队

#### 功能描述

提供一键灾难恢复能力，确保系统在遭遇硬件故障、数据损坏、网络攻击或人为误操作后，能够快速恢复业务运营，将RTO（恢复时间目标）控制在4小时以内，RPO（恢复点目标）控制在1小时以内。

#### 业务连续性目标

| 指标 | 目标值 | 说明 |
|------|--------|------|
| RTO（恢复时间目标）| ≤4小时 | 系统从故障到恢复可用的最长时间 |
| RPO（恢复点目标）| ≤1小时 | 数据丢失的最大时间窗口 |
| 备份成功率 | ≥99.9% | 每日备份任务成功率 |
| 恢复演练频率 | 每季度1次 | 季度灾难恢复演练验证 |
| 恢复验证时间 | ≤30分钟 | 恢复后数据完整性验证时间 |

#### 灾难恢复策略

| 层级 | 策略 | 触发条件 |
|------|------|---------|
| **L1 — 数据层** | PostgreSQL WAL（预写日志）+ PITR（时间点恢复） | 数据库损坏 |
| **L2 — 应用层** | Docker镜像快照 + 配置分离 | 应用崩溃或配置错误 |
| **L3 — 系统层** | 云快照（整机备份）+ 异地容灾 | 整机故障或硬件损坏 |
| **L4 — 网络层** | CDN + 多线路BGP failover | 网络攻击或线路中断 |

#### 一键恢复操作流程

```
灾难发生 → 告警触发（<1分钟）
    ↓
系统自动评估故障类型 → 选择恢复策略
    ↓
一键恢复执行：
  ├─ 确认恢复点（最近可用备份）→ 用户/管理员确认
  ├─ 自动执行恢复脚本
  ├─ 数据完整性校验（MD5/SHA-256）
  ├─ 服务可用性探测
  └─ 恢复完成通知（App + 短信）
    ↓
恢复报告自动生成 → 存档至审计日志
```

#### 数据恢复类型

| 恢复类型 | 适用场景 | 操作复杂度 | 预计耗时 |
|----------|---------|-----------|---------|
| 全量恢复（Full Restore）| 整机故障、勒索软件 | 高 | 2-4小时 |
| 数据库PITR | 误删数据、数据损坏 | 中 | 15-30分钟 |
| 文件级恢复 | 单个文件误删 | 低 | 5-15分钟 |
| 定时点回滚 | 错误批量操作 | 中 | 10-30分钟 |

#### 输出

```json
{
  "disaster_recovery_session": {
    "session_id": "DR-2026-06-06-001",
    "incident_type": "data_corruption",
    "triggered_at": "2026-06-06T14:30:00+08:00",
    "recovery_point": {
      "type": "pitr",
      "timestamp": "2026-06-06T13:30:00+08:00",
      "data_loss_minutes": 60,
      "backup_source": "cloud_backup_06_06_13:00"
    },
    "recovery_steps": [
      {"step": 1, "action": "validate_backup_integrity", "status": "completed", "duration_sec": 45},
      {"step": 2, "action": "restore_postgresql_pitr", "status": "completed", "duration_sec": 720},
      {"step": 3, "action": "validate_data_integrity", "status": "completed", "duration_sec": 180},
      {"step": 4, "action": "restart_application_services", "status": "completed", "duration_sec": 60}
    ],
    "total_duration_minutes": 17,
    "rto_achieved": true,
    "rpo_achieved": true,
    "notified_users": ["SCHOOL_ADMIN", "SYSTEM_ADMIN"],
    "recovery_report_url": "https://..."
  }
}
```

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 模拟数据库损坏后，30分钟内恢复至最近备份点 | 季度演练 |
| 2 | 恢复后数据完整性校验通过（MD5） | 自动执行 |
| 3 | 恢复完成后自动发送短信/App通知校务主任 | 功能验证 |
| 4 | 恢复日志完整存档至F-USER-005审计日志 | 日志审查 |

---

### Function F-NEW-002: 多渠道通知模板管理

**函数ID：** F-NEW-002
**函数名称：** 多渠道通知模板管理
**模块：** MOD-NEW-001 (Module 9)
**子模块：** 通知管理
**优先级：** P1（评审报告缺漏需求第12项）
**责任人：** AI 团队

#### 功能描述

集中管理学校向家长、学生、教师发送的所有通知模板，统一管理微信模板消息、短信（SMS）和邮件的内容格式、发送规则和变量替换，支持按通知类型、紧急程度、接收角色进行差异化模板配置。

#### 通知渠道

| 渠道 | 适用场景 | 字符限制 | 成本 |
|------|---------|---------|------|
| 微信模板消息 | 家长微信/App通知 | ≤600字 | 低 |
| 短信 (SMS) | 紧急通知、无App家长 | ≤160字（英文）/ 70字（中文）| 较高 |
| 邮件 | 正式文件、附件传输 | 无限制 | 低 |
| 飞书消息 | 内部教职员通知 | 无限制 | 免费 |
| App推送 | 有App的用户 | ≤300字 | 极低 |

#### 模板结构

```json
{
  "template_id": "TPL-BUS-DELAY-001",
  "name": "校车延误通知",
  "category": "bus",
  "urgency": "high",
  "channels": ["wechat", "sms"],
  "wechat": {
    "template_id": "wx_template_bus_delay",
    "title": "校车延误通知",
    "content": "亲爱的家长，您的孩子乘坐的校车（线路：{{route_name}}）预计延误{{delay_minutes}}分钟。\n延误原因：{{delay_reason}}\n预计到校时间：{{eta_time}}\n如有任何疑问，请联系校务处。",
    "variables": ["route_name", "delay_minutes", "delay_reason", "eta_time"],
    "fallback_channel": "sms"
  },
  "sms": {
    "content": "【学校通知】校车{{route_name}}延误{{delay_minutes}}分钟，预计{{eta_time}}到校。如需帮助请致电学校。",
    "variables": ["route_name", "delay_minutes", "eta_time"],
    "priority": "high"
  },
  "delivery_rules": {
    "min_interval_minutes": 30,
    "max_daily_per_parent": 5,
    "quiet_hours": "21:00-07:00"
  },
  "version": 3,
  "last_modified": "2026-05-20",
  "modified_by": "USR-2026-00001"
}
```

#### 模板分类

| 类别 | 示例模板 | 优先级 | 备用渠道 |
|------|---------|--------|---------|
| 校车通知 | 校车延误、到校确认、路线变更 | HIGH | SMS |
| 出勤通知 | 迟到提醒、缺席确认、异常预警 | HIGH | SMS |
| 成绩通知 | 成绩发布、成绩异常 | NORMAL | Email |
| 缴费通知 | 账单生成、逾期提醒、已缴确认 | NORMAL | Email |
| 活动通知 | 活动报名、活动取消、活动提醒 | NORMAL | Wechat |
| 紧急通知 | 台风停课、紧急疏散、食品安全 | CRITICAL | SMS + Call |
| 日常通知 | 午膳变更、校车开通、开学通知 | INFO | Wechat |

#### API 接口

```typescript
// GET /api/v1/notifications/templates
// POST /api/v1/notifications/templates
// PUT /api/v1/notifications/templates/:id
// POST /api/v1/notifications/send
interface SendNotificationRequest {
  template_id: string;
  recipient_group: 'parents' | 'teachers' | 'all' | 'specific_users';
  recipient_user_ids?: string[];
  variables: Record<string, string>;
  channel_override?: ('wechat' | 'sms' | 'email' | 'app')[];
  scheduled_at?: string;
}
```

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 同一模板可配置微信/短信/邮件三个渠道 | 手动创建模板并发送测试 |
| 2 | 模板变量正确替换，无遗漏 | 使用不同变量值发送测试通知 |
| 3 | 紧急通知自动切换SMS备用渠道 | 模拟推送失败场景 |
| 4 | 通知历史可在F-USER-005中查询 | 日志审查 |

---

### Function F-NEW-003: DSE放榜成绩追踪

**函数ID：** F-NEW-003
**函数名称：** DSE放榜成绩追踪
**模块：** MOD-NEW-001 (Module 9)
**子模块：** 考试管理
**优先级：** P1（评审报告缺漏需求第1项，周期性校务模块补充）
**责任人：** 教务团队

#### 功能描述

对接香港考试及评核局（HKEAA）DSE放榜系统，自动接收中六学生DSE考试成绩，追踪学生升学去向，生成统计分析报告，支持JUPAS申请状态追踪。

#### HKEAA数据对接

| 数据类型 | 对接方式 | 频率 | 说明 |
|----------|---------|------|------|
| DSE考试成绩 | HKEAA Secure Data Portal (SDP) | 放榜日（8月） | 加密CSV/JSON格式，学校凭授权码下载 |
| 成绩更正申请 | Email + 电话 | 按需 | 成绩公布后14天内可申请覆核 |
| 放榜指南 | HKEAA官网API | 每年更新 | 获取放榜流程、申请截止日期 |

#### 追踪功能

| 功能 | 描述 |
|------|------|
| 成绩自动入库 | HKEAA放榜后自动导入学生成绩，生成个人成绩单 |
| 成绩分布分析 | 全级/各班DSE成绩分布柱状图、平均分、标准差 |
| 大学联招追踪 | JUPAS申请状态（Band A/B/C志愿状态）自动更新 |
| 升学去向统计 | 学生最终就读大学/课程统计，匿名化处理 |
| 升学顾问报告 | 自动生成年度升学去向报告（供校务主任参考）|

#### 输出

```json
{
  "dse_result_tracking": {
    "academic_year": "2025-2026",
    "release_date": "2026-08-12",
    "total_s6_students": 180,
    "results_received": 175,
    "results_pending": 5,
    "by_subject_analysis": [
      {
        "subject": "中國語文",
        "candidates": 175,
        "level_5_plus_pct": "12.5%",
        "pass_rate": "98.3%",
        "school_avg": 4.2,
        "hkeaa_avg": 4.1
      }
    ],
    "jupas_status": {
      "applications_submitted": 165,
      "first_round_offers": 142,
      "acceptance_rate": "86.1%"
    },
    "offer_tracking": [
      {
        "student_id": "2023S10601",
        "student_name": "陳同學",
        "jupas_status": "confirmed_offer",
        "institution": "香港大學",
        "program": "內外全科醫學士",
        "offer_date": "2026-08-10"
      }
    ]
  }
}
```

---

### Function F-NEW-004: 年度预算编制与执行追踪

**函数ID：** F-NEW-004
**函数名称：** 年度预算编制与执行追踪
**模块：** MOD-NEW-001 (Module 9)
**子模块：** 财务管理
**优先级：** P1（评审报告缺漏需求第2项，财务资产模块补充）
**责任人：** 校务主任

#### 功能描述

为学校提供完整的年度预算编制、执行追踪和差异分析功能，支持多部门预算分配、实际支出实时比对、超支预警和预算调整审批流程。

#### 预算编制流程

```
校务主任发起预算 → 各部门提交预算申请（按类别）
    ↓
财务专员初审 → 校务主任/校董会审批
    ↓
预算分配确认 → 录入系统（按月/按季度拆分）
    ↓
执行中追踪 → 实际支出与预算比对
    ↓
季度/年度决算 → 预算执行报告
```


#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 新学年开始，校务主任发起年度预算编制 | 校务主任进入预算管理，点击「新建年度预算」| 系统创建空白预算框架，显示8大预算科目模板，各部门可开始填写申请 |
| AC-02 | IT部门申请HK$300,000设备采购预算 | IT主管填写预算申请，提交审批 | 申请进入审批流程，校务主任审批通过后，HK$300,000分配至IT科目 |
| AC-03 | 10月份，IT科目实际支出HK$280,000 | 财务专员查看IT科目执行率 | 系统显示IT科目执行率=93.3%，status=warning（黄灯告警）|
| AC-04 | IT科目超支预警（>80%预算使用）| 系统例行检查 | 自动生成预警通知，推送至校务主任，提示「IT科目执行率达93%，建议控制支出」|
| AC-05 | 学期末，需科目间预算调整（ACTIVITY省HK$50,000调至IT）| 校务主任发起预算调整申请 | 系统记录调整明细（从ACTIVITY科目HK$50,000→IT科目），生成调整审批记录 |


#### 预算科目分类

| 类别代码 | 描述 | 示例 |
|---------|------|------|
| STAFF | 人事费用 | 教职员薪金、津贴 |
| FACILITY | 设施维护 | 冷气、维修、水电 |
| ACADEMIC | 教务支出 | 试卷印刷、图书采购 |
| IT | 资讯科技 | 软件许可、设备更新 |
| ACTIVITY | 活动费用 | 课外活动、旅行 |
| SCHOLARSHIP | 奖助学金 | 奖学金、助学金 |
| ADMIN | 行政费用 | 文具、邮费、印刷 |
| CONTINGENCY | 应急储备 | 未预见支出 |

#### 预算执行追踪

| 功能 | 描述 |
|------|------|
| 实时执行率 | 各科目已用预算 / 分配预算，按月/季度可视化 |
| 超支预警 | 实际支出达到预算80%时黄色预警，100%时红色告警 |
| 预算调整 | 科目间调整须审批，不可超出总预算 |
| 执行报告 | 每月自动生成预算执行报告（PDF）|

#### 输出

```json
{
  "annual_budget": {
    "fiscal_year": "2026-2027",
    "total_allocated": 8500000.00,
    "total_spent": 2150000.00,
    "execution_rate": "25.3%",
    "by_category": [
      {
        "category": "STAFF",
        "allocated": 5800000.00,
        "spent": 1450000.00,
        "execution_rate": "25.0%",
        "monthly_forecast": 483333.00,
        "variance": 8333.00,
        "status": "on_track"
      },
      {
        "category": "IT",
        "allocated": 300000.00,
        "spent": 280000.00,
        "execution_rate": "93.3%",
        "status": "warning"
      }
    ],
    "adjustments": [
      {"date": "2026-10-15", "from_category": "ACTIVITY", "to_category": "IT", "amount": 50000.00, "approved_by": "校務主任"}
    ]
  }
}
```

---

### Function F-NEW-005: 自定义报表生成与定时推送

**函数ID：** F-NEW-005
**函数名称：** 自定义报表生成与定时推送
**模块：** MOD-NEW-001 (Module 9)
**子模块：** 报表中心
**优先级：** P1（评审报告缺漏需求第4项，AI助理模块补充）
**责任人：** AI 团队

#### 功能描述

为校务主任和各级管理人员提供自助式报表生成工具，支持拖拽式字段选择、多维度筛选和定时自动推送，减少人工报表制作工作量。

#### 报表生成器

| 功能 | 说明 |
|------|------|
| 字段选择 | 拖拽选择所需字段，支持多表关联（学生×成绩、出勤×班级）|
| 筛选条件 | 等值/范围/模糊匹配，支持AND/OR组合 |
| 排序规则 | 多字段排序，显示顺序可调 |
| 分组聚合 | 按班级/年级/月份/教师等维度聚合统计 |
| 图表类型 | 柱状图、饼图、折线图、数字仪表盘 |
| 导出格式 | PDF、Excel、CSV |

#### 预置报表模板

| 报表名称 | 周期 | 接收人 |
|---------|------|--------|
| 每日出勤摘要 | 每日 08:00 | 校务主任 |
| 每周出勤周报 | 每周一 09:00 | 校务主任、班主任 |
| 每月收费报告 | 每月1日 | 校务主任 |
| 学期成绩汇总 | 学期末 | 教务主任、校务主任 |
| 年度财务摘要 | 每学年末 | 校务主任、校董会 |
| DSE成绩分析 | 放榜后 | 教务主任、校长 |

#### 定时推送配置

| 配置项 | 说明 |
|--------|------|
| 推送频率 | 每日/每周/每月/每学期 |
| 推送时间 | 精确到小时（默认工作日09:00）|
| 推送格式 | PDF附件 + 正文摘要 |
| 推送渠道 | App通知 + 邮件 |
| 订阅管理 | 用户可订阅/退订各类型报表 |

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 用户可在30分钟内创建自定义出勤报表 | 用户测试 |
| 2 | 定时报表准时推送成功 | 连续运行30天验证 |
| 3 | 报表数据与系统数据一致 | 人工抽查比对 |

---

### Function F-NEW-006: 系统健康检查与自动诊断

**函数ID：** F-NEW-006
**函数名称：** 系统健康检查与自动诊断
**模块：** MOD-NEW-001 (Module 9)
**子模块：** 系统运维
**优先级：** P2（评审报告缺漏需求第3项，整合合规模块补充）
**责任人：** 系统管理员

#### 功能描述

对系统各组件（数据库、Web服务、AI引擎、第三方接口）进行持续健康监测，自动发现异常并触发告警，提供一键诊断报告，降低运维成本。

#### 健康检查维度

| 检查项 | 检查内容 | 健康阈值 | 告警级别 |
|--------|---------|---------|---------|
| 数据库连接 | PostgreSQL连接池状态 | 连接成功率 ≥ 99% | ERROR |
| 数据库性能 | 查询响应时间、慢查询数 | P95响应 < 500ms | WARNING |
| Web服务 | HTTP响应时间、错误率 | 5xx错误率 < 0.1% | ERROR |
| 磁盘空间 | 各挂载点使用率 | 使用率 < 85% | WARNING |
| 内存使用 | 各服务内存占用 | 使用率 < 90% | WARNING |
| AI服务 | LLM API响应时间 | < 5秒 | WARNING |
| 第三方接口 | WebSAMS/eClass API可用性 | 可用率 ≥ 98% | ERROR |
| 备份任务 | 每日备份是否成功完成 | 成功率 100% | ERROR |
| Token有效性 | 微信/飞书Access Token状态 | 有效期 > 24h | WARNING |

#### 自动诊断报告

```json
{
  "health_report": {
    "generated_at": "2026-06-06T08:00:00+08:00",
    "overall_status": "healthy",
    "checks": [
      {
        "component": "postgresql",
        "status": "healthy",
        "response_time_ms": 23,
        "connection_pool_used": "45/100",
        "slow_queries": 0
      },
      {
        "component": "wechat_api",
        "status": "warning",
        "access_token_expires_in_hours": 18,
        "last_successful_call": "2026-06-06T07:58:00+08:00",
        "recommendation": "Token将于6小时后过期，建议立即刷新"
      },
      {
        "component": "feishu_api",
        "status": "healthy",
        "last_successful_call": "2026-06-06T07:59:30+08:00"
      }
    ],
    "diagnostic_report_url": "https://..."
  }
}
```

#### 告警升级规则

| 告警级别 | 触发条件 | 通知对象 | 通知方式 | 升级条件 |
|----------|---------|---------|---------|---------|
| CRITICAL | 服务不可用/数据丢失 | 校务主任 + 系统管理员 | 短信 + 电话 | 无升级，立即响应 |
| ERROR | 单组件故障 | 系统管理员 | App + 短信 | 30分钟未响应升级至校务主任 |
| WARNING | 性能下降/资源紧张 | 系统管理员 | App推送 | 2小时未响应升级至校务主任 |
| INFO | 例行检查结果 | 系统管理员 | App推送 | 无升级 |

---

---

## 第十二部分：Module 11 — 运维自动化与监控体系 (v1.7.0 运维催生需求)

> **Module 11 (Ops Automation & Monitoring / v1.7.0)** 是基于运维实践分析（运维催生功能优化需求清单 v1.0.0，2026-06-06）产出的运维自动化与监控增强模块。涵盖10项P0优先级需求：5项自动化脚本（A-01/A-05/A-15/A-19/A-21）和5项监控增强（M-05/M-10/M-11/L-02/G-01）。

### 12.1 模块概述

| 属性 | 描述 |
|------|------|
| 模块名称 | Ops Automation & Monitoring System — 运维自动化与监控体系 |
| 模块ID | MOD-OPS-001 |
| 优先级 | P0/P1 |
| 用户 | 系统管理员、校务主任 |
| 评审依据 | 运维催生功能优化需求清单 v1.0.0（2026-06-06）P0优先级清单 |

---

### Function F-OPS-001: 数据库健康检查与 WAL 积压自动处理

**函数ID：** F-OPS-001
**函数名称：** 数据库健康检查与 WAL 积压自动处理
**模块：** MOD-OPS-001 (Module 11)
**子模块：** 自动化运维脚本
**优先级：** P0（对应运维需求 A-01 / A-05）
**责任人：** 系统管理员

#### 功能描述

提供一键数据库健康检查脚本（`db-health-check.sh`），自动化执行每日数据库运维检查任务（D-01），涵盖连接数、PgBouncer 状态、WAL 积压等核心指标。同时实现 WAL 积压自动处理机制，当积压超过阈值时自动触发恢复动作，降低数据层 RPO 风险。

#### 数据库健康检查脚本（`db-health-check.sh`）

**检查项目：**

| 检查项 | 阈值 | 告警级别 | 说明 |
|--------|------|---------|------|
| 数据库连接数 | > 80% 最大连接数 | ERROR | PostgreSQL max_connections 使用率 |
| PgBouncer 连接池 | 连接数 > 90% 池上限 | ERROR | pgbouncer pool usage |
| WAL 积压 | > 500MB | ERROR | WAL 积压量，自动触发处理 |
| 复制延迟 | > 30 秒 | WARNING | 主从复制延迟 |
| 慢查询数 | > 10 条（P95 > 1s）| WARNING | pg_stat_statements Top N |
| 磁盘使用率 | > 85% | WARNING | 数据目录所在磁盘 |
| 备份任务 | 过去 24h 无成功备份 | ERROR | 备份状态检查 |
| 表膨胀率 | > 20% 膨胀率 | WARNING | check-table-bloat.sh 每周检查 |

**输出格式：** 结构化 JSON 报告，集成至 Prometheus/Grafana：
```json
{
  "script": "db-health-check.sh",
  "executed_at": "2026-06-06T08:00:00+08:00",
  "overall_status": "healthy",
  "checks": [
    {"check": "postgres_connection_usage", "status": "healthy", "current": "45/100", "threshold": "80/100"},
    {"check": "pgbouncer_pool_usage", "status": "healthy", "current": "30/50"},
    {"check": "wal_archive_backlog", "status": "warning", "current_mb": 320, "threshold_mb": 500, "auto_action": "triggered"},
    {"check": "replication_lag", "status": "healthy", "lag_seconds": 2},
    {"check": "slow_queries", "status": "healthy", "count_p95_gt_1s": 3},
    {"check": "disk_usage", "status": "healthy", "usage_percent": 62},
    {"check": "backup_status", "status": "healthy", "last_successful_backup": "2026-06-05T01:00:00+08:00"}
  ],
  "prometheus_metrics": {
    "db_health_status": 1,
    "db_connection_usage_percent": 45,
    "pgbouncer_pool_usage_percent": 60,
    "wal_archive_backlog_mb": 320,
    "replication_lag_seconds": 2
  }
}
```

**执行策略：**

| 执行频率 | 触发方式 | 输出目标 |
|----------|---------|---------|
| 每日 07:00 | Cron 自动执行 | Prometheus + Slack |
| 手动触发 | 运维人员执行 | 标准输出 + 日志文件 |
| 告警时 | 监控系统触发 | 立即告警 + 诊断报告 |

#### WAL 积压自动处理

**触发条件：** WAL 积压 > 500MB

**自动处理流程：**
```
WAL 积压告警触发 → 自动执行 wal-archive-speedup.sh
    ├─ 临时增加 archive_compression 并发数（×2）
    ├─ 触发 checkpoint 加速 WAL 刷新
    └─ 持续监控积压变化
    ↓
积压 < 200MB → 恢复默认配置
积压持续 > 500MB（> 10 分钟）→ 升级告警 + 人工介入
    ↓
处理完成记录至审计日志
```

**输出：**
```json
{
  "wal_auto_action": {
    "triggered_at": "2026-06-06T09:00:00+08:00",
    "backlog_mb_before": 520,
    "action_taken": "increase_archive_concurrency",
    "backlog_mb_after_5min": 380,
    "backlog_mb_after_10min": 150,
    "resolved": true,
    "resolved_at": "2026-06-06T09:10:00+08:00",
    "audit_logged": true
  }
}
```

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | `db-health-check.sh` 输出结构化 JSON 报告 | 手动执行并验证 JSON 格式 |
| 2 | 健康检查结果自动推送至 Prometheus | 验证 Prometheus 指标入库 |
| 3 | WAL 积压 > 500MB 时自动触发处理脚本 | 模拟 WAL 积压场景 |
| 4 | WAL 积压处理后自动恢复默认配置 | 日志验证 |
| 5 | 所有检查结果记录至审计日志 | 日志审查 |

---

### Function F-OPS-002: SSL 证书到期自动续期

**函数ID：** F-OPS-002
**函数名称：** SSL 证书到期自动续期
**模块：** MOD-OPS-001 (Module 11)
**子模块：** 自动化运维脚本
**优先级：** P0（对应运维需求 A-15）
**责任人：** 系统管理员

#### 功能描述

集成 Let's Encrypt `certbot renew` 至自动化 Cron，实现证书到期前自动续期，并在 Vault 中统一管理证书状态，解决 R-05 证书过期导致服务中断的风险。

#### 自动续期机制

**Cron 配置：**
```yaml
certbot_renew_check:
  schedule: "0 2 * * *"
  script: "/opt/ops/scripts/certbot-renew-check.sh"
  output: prometheus + slack
  alert_on_failure: true

cert_expiry_alert:
  schedule: "0 9 * * *"
  check_ahead_days: [30, 7, 1]
  alert_channels: ["slack", "email", "sms"]
  notify_roles: ["SYSTEM_ADMIN", "SCHOOL_ADMIN"]
```

**证书状态管理（Vault）：**
```json
{
  "certificate_status": {
    "domain": "school-admin.internal",
    "issued_at": "2026-03-15T00:00:00+08:00",
    "expires_at": "2026-06-13T23:59:59+08:00",
    "days_until_expiry": 7,
    "auto_renew_enabled": true,
    "auto_renew_attempts": 0,
    "last_renewal_result": null,
    "vault_path": "secret/certs/school-admin-internal"
  }
}
```

#### 告警规则

| 告警级别 | 触发条件 | 通知对象 | 通知方式 |
|----------|---------|---------|---------|
| INFO | 证书到期前 30 天 | 系统管理员 | Slack |
| WARNING | 证书到期前 7 天 | 系统管理员 | Slack + 邮件 |
| CRITICAL | 证书到期前 1 天 | 系统管理员 + 校务主任 | Slack + 邮件 + 短信 |
| ERROR | 证书已过期 | 系统管理员 + 校务主任 | 短信 + 电话 |

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | `certbot renew` 集成至 Cron 并正常执行 | 检查 Cron 日志 |
| 2 | 证书到期前 30/7/1 天自动告警 | 模拟到期测试 |
| 3 | 证书状态存储于 Vault 并可查询 | Vault API 验证 |
| 4 | 证书续期后自动重新加载服务（Nginx/Traefik）| 手动续期测试 |

---

### Function F-OPS-003: WebSAMS Token 自动刷新

**函数ID：** F-OPS-003
**函数名称：** WebSAMS Token 自动刷新
**模块：** MOD-OPS-001 (Module 11)
**子模块：** 自动化运维脚本
**优先级：** P0（对应运维需求 A-21）
**责任人：** 系统管理员

#### 功能描述

提供 `websams-token-refresh.sh` 脚本，每小时自动检查 WebSAMS API Token 有效期，当 Token 剩余有效期 < 24 小时时自动触发刷新，并将新 Token 安全存储至 Vault，解决 Token 过期导致 D-09 数据同步中断的问题。

#### Token 健康检查与自动刷新

**执行频率：** 每小时 Cron

**处理流程：**
```
每小时触发 → 检查 Token 剩余有效期
    ↓
剩余有效期 > 24h → 记录状态，正常退出
    ↓（≤ 24h）
剩余有效期 > 4h → 记录 WARNING 告警
    ↓（≤ 4h）
触发自动刷新 → 调用 WebSAMS Token API → 获取新 Token
    ↓
验证新 Token 可用 → 存储至 Vault（secret/websams/access_token）
    ↓
通知系统管理员（Slack）→ 记录审计日志
```

**Vault 存储结构：**
```json
{
  "websams_token": {
    "access_token": "eyJhbGciOiJIUzI1NiIs...",
    "token_type": "Bearer",
    "expires_at": "2026-06-07T10:00:00+08:00",
    "remaining_hours": 23.5,
    "refreshed_at": "2026-06-06T10:00:00+08:00",
    "refresh_reason": "scheduled_check",
    "vault_path": "secret/websams/access_token"
  }
}
```

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 每小时执行 Token 有效期检查 | 检查 Cron 执行日志 |
| 2 | Token < 24h 时自动刷新 | 模拟过期场景 |
| 3 | 新 Token 正确存储至 Vault | Vault API 验证 |
| 4 | Token 刷新后自动通知系统管理员 | Slack 消息验证 |
| 5 | Token 刷新操作记录至审计日志 | 审计日志审查 |

---

### Function F-OPS-004: 一键灾难恢复脚本（F-NEW-001 落地增强）

**函数ID：** F-OPS-004
**函数名称：** 一键灾难恢复脚本
**模块：** MOD-OPS-001 (Module 11)
**子模块：** 灾难恢复
**优先级：** P0（对应运维需求 A-19）
**责任人：** 系统管理员

#### 功能描述

基于 F-NEW-001 定义的灾难恢复框架，实现 `disaster-recovery.sh` 一键执行脚本，将 L1–L3 恢复步骤自动化串联，将 RTO 从手动操作的 4+ 小时缩短至 30 分钟以内，解决 DR 恢复步骤手动串联导致的 RTO 风险。

#### 一键恢复脚本功能

**脚本参数：**
```bash
./disaster-recovery.sh --type=pitr           # 数据库 PITR 恢复
./disaster-recovery.sh --type=full           # 全量恢复
./disaster-recovery.sh --type=file --path=   # 文件级恢复
./disaster-recovery.sh --dry-run             # 演练模式（不实际执行）
./disaster-recovery.sh --verify              # 仅验证备份完整性
```

**L1–L3 恢复步骤自动串联：**

| 层级 | 步骤 | 自动化程度 | 预计耗时 |
|------|------|-----------|---------|
| **L1 数据层** | 1. 验证备份完整性（MD5/SHA-256）| 全自动 | 2-5 分钟 |
| | 2. 确定恢复点（最新可用备份）| 半自动（显示选项）| 1 分钟 |
| | 3. 执行 PostgreSQL PITR 恢复 | 全自动 | 10-20 分钟 |
| | 4. 数据完整性校验（记录数抽样）| 全自动 | 5-10 分钟 |
| **L2 应用层** | 5. 拉取最新 Docker 镜像 | 全自动 | 3-5 分钟 |
| | 6. 应用配置（Vault Secret 注入）| 全自动 | 1 分钟 |
| | 7. 重启应用服务 | 全自动 | 2-3 分钟 |
| **L3 系统层** | 8. 验证服务可用性（健康检查）| 全自动 | 1-2 分钟 |
| | 9. 发送恢复完成通知 | 全自动 | 即时 |

**输出报告：**
```json
{
  "dr_session": {
    "session_id": "DR-2026-06-06-AUTO-001",
    "trigger_type": "manual_one_click",
    "triggered_by": "SYSTEM_ADMIN",
    "triggered_at": "2026-06-06T14:30:00+08:00",
    "recovery_type": "pitr",
    "recovery_point": {
      "backup_id": "BACKUP-20260606-130000",
      "timestamp": "2026-06-06T13:00:00+08:00",
      "data_loss_minutes": 90,
      "verified": true
    },
    "steps": [
      {"step": 1, "action": "verify_backup_integrity", "duration_sec": 45, "status": "completed"},
      {"step": 2, "action": "confirm_recovery_point", "duration_sec": 60, "status": "completed"},
      {"step": 3, "action": "execute_pitr_restore", "duration_sec": 720, "status": "completed"},
      {"step": 4, "action": "validate_data_integrity", "duration_sec": 180, "status": "completed"},
      {"step": 5, "action": "pull_docker_images", "duration_sec": 180, "status": "completed"},
      {"step": 6, "action": "inject_vault_secrets", "duration_sec": 60, "status": "completed"},
      {"step": 7, "action": "restart_services", "duration_sec": 120, "status": "completed"},
      {"step": 8, "action": "health_check", "duration_sec": 90, "status": "completed"},
      {"step": 9, "action": "notify_completion", "duration_sec": 5, "status": "completed"}
    ],
    "total_duration_minutes": 25,
    "rto_target_minutes": 30,
    "rto_achieved": true,
    "rpo_achieved": true,
    "recovery_report_url": "https://..."
  }
}
```

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 一键脚本完整执行 L1–L3 所有步骤 | 季度 DR 演练 |
| 2 | RTO ≤ 30 分钟 | 演练计时验证 |
| 3 | 恢复后自动生成标准格式演练报告 | 报告内容审查 |
| 4 | 演练报告存档至审计日志（F-USER-005）| 审计日志审查 |
| 5 | 恢复完成自动通知校务主任（App + 短信）| 通知验证 |

---

### Function F-OPS-005: 审计日志写入完整性监控

**函数ID：** F-OPS-005
**函数名称：** 审计日志写入完整性监控
**模块：** MOD-OPS-001 (Module 11)
**子模块：** 监控增强
**优先级：** P0（对应运维需求 M-05）
**责任人：** 系统管理员

#### 功能描述

实现审计日志写入完整性监控机制，解决 `audit_logs` 记录可能因程序异常而丢失的合规盲区。通过 Prometheus 指标 `audit_log_writer_errors_total` 和定期抽样校验，确保写入成功率持续 > 99.9%。

#### 监控指标

**Prometheus 指标定义：**

| 指标名称 | 类型 | 说明 |
|----------|------|------|
| `audit_log_writer_errors_total` | Counter | 审计日志写入失败总次数（按错误类型标签）|
| `audit_log_writer_success_total` | Counter | 审计日志写入成功总次数 |
| `audit_log_write_latency_seconds` | Histogram | 写入延迟分布 |
| `audit_log_writer_queue_size` | Gauge | 待写入队列长度 |
| `audit_log_sample_verify_errors_total` | Counter | 抽样校验发现的丢失记录数 |

**告警规则：**

| 告警级别 | 触发条件 | 通知对象 | 动作 |
|----------|---------|---------|------|
| ERROR | 写入成功率 < 99.9%（5分钟窗口）| 系统管理员 | 自动重试 + 告警 |
| CRITICAL | 连续 3 次写入失败 | 系统管理员 + 校务主任 | 服务降级告警 |
| WARNING | 抽样校验发现记录丢失 | 系统管理员 | 补充写入 + 告警 |

#### 抽样校验机制

| 校验频率 | 抽样比例 | 校验内容 | 处理方式 |
|----------|---------|---------|---------|
| 每 5 分钟 | 随机 1% | 写入记录数 vs 预期记录数 | 丢失记录补充写入 |
| 每日 | 全量对比 | 关键表（attendance_records, audit_logs）记录数 | 生成完整性报告 |

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | Prometheus 指标正常暴露（`audit_log_writer_errors_total`）| Prometheus UI 验证 |
| 2 | 写入成功率 < 99.9% 时触发告警 | 模拟失败场景 |
| 3 | 抽样校验每 5 分钟执行 | Cron 日志验证 |
| 4 | 校验报告存档至审计日志 | 报告审查 |

---

### Function F-OPS-006: Coze API 配额实时监控

**函数ID：** F-OPS-006
**函数名称：** Coze API 配额实时监控
**模块：** MOD-OPS-001 (Module 11)
**子模块：** 监控增强
**优先级：** P0（对应运维需求 M-10）
**责任人：** 系统管理员

#### 功能描述

实现 Coze API 配额使用量实时监控，解决 AI 服务配额耗尽时无预警导致服务突然中断的问题。通过配额使用率监控实现三级告警，并在 > 95% 时自动触发限流保护。

#### 监控配置

**Prometheus 指标定义：**

| 指标名称 | 类型 | 说明 |
|----------|------|------|
| `coze_api_quota_used` | Gauge | 已使用配额数量 |
| `coze_api_quota_limit` | Gauge | 配额上限 |
| `coze_api_quota_usage_percent` | Gauge | 配额使用率（%）|
| `coze_api_call_total` | Counter | API 调用总次数 |
| `coze_api_call_errors_total` | Counter | API 调用失败次数 |
| `coze_api_latency_seconds` | Histogram | API 响应延迟 |

**告警规则：**

| 告警级别 | 触发条件 | 通知对象 | 自动动作 |
|----------|---------|---------|---------|
| WARNING | 配额使用 > 80% | 系统管理员 | Slack 预警 |
| ERROR | 配额使用 > 95% | 系统管理员 | **自动触发限流保护** |
| CRITICAL | 配额使用 = 100% | 系统管理员 + 校务主任 | 禁用非关键 AI 功能，保留核心功能 |

#### 限流保护机制

```yaml
coze_rate_limiting:
  enabled: true
  trigger_threshold: 95
  limits:
    high_priority:      # 核心功能：无限制
      - 出勤异常通知
      - 请假申请 AI 核验
    medium_priority:    # 中等：10 req/min
      - FAQ 智能匹配
      - OCR 文档识别
    low_priority:       # 低优先级：暂停
      - AI 报表生成
      - 成绩单评语辅助
```

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 配额使用率指标正常暴露至 Prometheus | Prometheus 查询验证 |
| 2 | 配额使用 > 80% 时自动告警 | 模拟配额消耗场景 |
| 3 | 配额使用 > 95% 时自动触发限流 | 功能测试验证限流生效 |
| 4 | 限流后核心功能仍可用 | 功能测试 |

---

### Function F-OPS-007: 敏感字段查看频率告警

**函数ID：** F-OPS-007
**函数名称：** 敏感字段查看频率告警
**模块：** MOD-OPS-001 (Module 11)
**子模块：** 安全监控
**优先级：** P0（对应运维需求 M-11）
**责任人：** 系统管理员

#### 功能描述

实现 `sensitive-field-view-monitor.sh`（每 5 分钟执行），统计各用户查看身份证号、电话号码、完整地址等敏感字段的次数，超越阈值时自动告警至 Slack #security-alerts，配合 PDPO 合规要求。

#### 监控范围

**敏感字段列表：**

| 字段类型 | 表/字段 | 告警阈值（5分钟窗口）| 告警级别 |
|---------|---------|---------------------|---------|
| 香港身份证号 | `students.hkid`、`parents.hkid` | > 5 次/用户/5分钟 | WARNING |
| 电话号码 | `users.phone`、`parents.contact_phone` | > 10 次/用户/5分钟 | WARNING |
| 完整地址 | `students.address`、`parents.address` | > 3 次/用户/5分钟 | ERROR |
| 医疗记录 | `students.medical_info` | > 2 次/用户/5分钟 | CRITICAL |

#### 执行流程

```
每 5 分钟执行 sensitive-field-view-monitor.sh
    ↓
查询 audit_logs 表（过去 5 分钟内）
    ↓
统计各用户查看敏感字段次数
    ↓
超出阈值？
    ↓（是）
  生成告警 → Slack #security-alerts + 短信至系统管理员
  记录至 audit_logs（event_type: SENSITIVE_FIELD_EXCESSIVE_ACCESS）
    ↓
持续异常（3 次/小时）→ 临时禁用该用户访问敏感字段权限
```

#### 输出

```json
{
  "sensitive_field_monitor": {
    "executed_at": "2026-06-06T14:25:00+08:00",
    "window_minutes": 5,
    "alerts_triggered": [
      {
        "user_id": "USR-2026-00015",
        "user_name": "李老師",
        "field_type": "hkid",
        "access_count": 12,
        "threshold": 5,
        "alert_level": "warning",
        "notified_channels": ["slack"],
        "action_taken": "alert_sent"
      }
    ],
    "critical_alerts": 0,
    "warning_alerts": 1
  }
}
```

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 每 5 分钟自动执行监控脚本 | Cron 日志验证 |
| 2 | 超阈值时告警至 Slack #security-alerts | 模拟超阈值场景 |
| 3 | 告警记录存档至审计日志 | 审计日志审查 |
| 4 | 持续异常用户自动临时禁用敏感字段访问 | 功能测试 |

---

### Function F-OPS-008: 数据库 DDL 操作审计

**函数ID：** F-OPS-008
**函数名称：** 数据库 DDL 操作审计
**模块：** MOD-OPS-001 (Module 11)
**子模块：** 审计增强
**优先级：** P0（对应运维需求 L-02）
**责任人：** 系统管理员

#### 功能描述

启用 PostgreSQL `pgaudit` 扩展，对 `ALTER TABLE`、`DROP TABLE`、`CREATE INDEX` 等 DDL 操作进行专项审计，所有 DDL 操作记录至专用审计日志表，满足安全合规要求。

#### pgaudit 配置

**PostgreSQL 配置（postgresql.conf）：**
```sql
-- 启用 pgaudit
shared_preload_libraries = 'pgaudit'

-- 审计日志输出到专用表
pgaudit.log = 'ddl'          -- 记录所有 DDL 操作
pgaudit.log_catalog = 'on'   -- 同时记录系统表变更
pgaudit.log_level = 'notice'
pgaudit.log_parameter = 'on' -- 记录操作参数
pgaudit.role = 'audit_user'  -- 专用审计角色
```

**DDL 操作审计表：**
```sql
CREATE TABLE ddl_audit_logs (
  id BIGSERIAL PRIMARY KEY,
  audit_timestamp TIMESTAMP NOT NULL DEFAULT NOW(),
  username TEXT NOT NULL,
  database_name TEXT NOT NULL,
  command_tag TEXT NOT NULL,
  object_type TEXT NOT NULL,
  object_identity TEXT NOT NULL,
  statement TEXT NOT NULL,
  parameters TEXT,
  client_addr INET,
  application_name TEXT,
  session_id UUID NOT NULL
);

CREATE INDEX idx_ddl_audit_timestamp ON ddl_audit_logs(audit_timestamp);
CREATE INDEX idx_ddl_audit_username ON ddl_audit_logs(username);
CREATE INDEX idx_ddl_audit_object ON ddl_audit_logs(object_type, object_identity);
```

#### 告警规则

| 告警级别 | 触发条件 | 通知对象 | 说明 |
|----------|---------|---------|------|
| ERROR | DROP TABLE / TRUNCATE TABLE | 系统管理员 + 校务主任 | 重大结构变更 |
| WARNING | ALTER TABLE（在生产库）| 系统管理员 | 表结构变更 |
| INFO | CREATE INDEX / CREATE TABLE | 系统管理员 | 正常审计记录 |

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | pgaudit 正确记录所有 DDL 操作 | 执行 DDL 测试 |
| 2 | DDL 日志记录到 `ddl_audit_logs` 表 | SQL 查询验证 |
| 3 | DROP/TRUNCATE 操作触发即时告警 | 模拟 DROP TABLE 场景 |
| 4 | DDL 审计日志保留 7 年 | 保留策略验证 |

---

### Function F-OPS-009: 运维健康仪表板（F-NEW-006 增强）

**函数ID：** F-OPS-009
**函数名称：** 运维健康仪表板
**模块：** MOD-OPS-001 (Module 11)
**子模块：** 监控体系
**优先级：** P0（对应运维需求 G-01，整合 F-NEW-006）
**责任人：** 系统管理员

#### 功能描述

基于 F-NEW-006 的系统健康检查维度，建设统一的运维健康仪表板（Ops Health Dashboard），将所有运维关键指标（Grafana）汇总展示，支持一键诊断，并将所有新增监控点（M-05/M-10/M-11/L-02）纳入统一监控体系。

#### 仪表板集成指标清单

| 监控领域 | 指标来源 | 对应需求 | 面板名称 |
|----------|---------|---------|---------|
| 数据库健康 | F-OPS-001 | A-01 | 数据库健康检查 |
| WAL 积压 | F-OPS-001 | A-05 | WAL 积压状态 |
| 证书状态 | F-OPS-002 | A-15 | SSL 证书管理 |
| WebSAMS Token | F-OPS-003 | A-21 | 第三方 Token 健康 |
| 审计日志完整性 | F-OPS-005 | M-05 | 审计监控 |
| Coze API 配额 | F-OPS-006 | M-10 | AI 服务配额 |
| 敏感字段访问 | F-OPS-007 | M-11 | 安全监控 |
| DDL 审计 | F-OPS-008 | L-02 | 数据库审计 |
| 备份状态 | F-NEW-001 | — | 备份与恢复 |
| 服务可用性 | F-NEW-006 | — | SLA 监控 |

#### 一键诊断功能

| 诊断类型 | 执行内容 | 输出 |
|----------|---------|------|
| **全面诊断** | 执行所有健康检查 → 生成汇总报告 → 推送 Slack | 诊断报告 URL |
| **数据库诊断** | 执行 db-health-check.sh → WAL 状态 → 慢查询分析 → 连接池诊断 | JSON 诊断报告 |
| **AI 服务诊断** | Coze API 健康 → 配额剩余 → 响应延迟 → 错误率分析 | AI 服务状态报告 |
| **安全诊断** | 敏感字段访问审计 → DDL 变更审计 → 异常登录审计 | 安全告警汇总 |
| **DR 演练** | 执行 disaster-recovery.sh --dry-run | 演练报告 |

#### 验收标准

| # | 验收条件 | 测试方法 |
|---|----------|----------|
| 1 | 仪表板聚合所有 10 个 P0 运维监控指标 | 逐项验证指标展示 |
| 2 | 一键诊断生成完整报告（< 2 分钟）| 功能测试 |
| 3 | 仪表板数据延迟 < 1 分钟（Prometheus 拉取间隔）| 指标时效验证 |
| 4 | 告警状态实时同步（CRITICAL/ERROR/WARNING）| 告警触发验证 |
| 5 | 诊断报告存档至审计日志 | 报告审查 |

---

### 12.2 监控指标汇总

**新增 Prometheus 指标定义（v1.7.0）：**

| 指标名称 | 类型 | 标签 | 说明 | 对应函数 |
|----------|------|------|------|---------|
| `db_health_status` | Gauge | - | 数据库整体健康状态（1=健康）| F-OPS-001 |
| `db_connection_usage_percent` | Gauge | - | 数据库连接使用率 | F-OPS-001 |
| `pgbouncer_pool_usage_percent` | Gauge | pool_name | PgBouncer 连接池使用率 | F-OPS-001 |
| `wal_archive_backlog_mb` | Gauge | - | WAL 积压量（MB）| F-OPS-001 |
| `cert_days_until_expiry` | Gauge | domain | 证书到期剩余天数 | F-OPS-002 |
| `websams_token_remaining_hours` | Gauge | - | WebSAMS Token 剩余小时数 | F-OPS-003 |
| `audit_log_writer_errors_total` | Counter | error_type | 审计日志写入失败次数 | F-OPS-005 |
| `audit_log_writer_success_total` | Counter | - | 审计日志写入成功次数 | F-OPS-005 |
| `audit_log_write_latency_seconds` | Histogram | - | 审计日志写入延迟 | F-OPS-005 |
| `audit_log_sample_verify_errors_total` | Counter | table_name | 抽样校验发现丢失记录数 | F-OPS-005 |
| `coze_api_quota_used` | Gauge | - | Coze API 已使用配额 | F-OPS-006 |
| `coze_api_quota_limit` | Gauge | - | Coze API 配额上限 | F-OPS-006 |
| `coze_api_quota_usage_percent` | Gauge | - | Coze API 配额使用率 | F-OPS-006 |
| `coze_api_rate_limited` | Gauge | priority | AI 请求是否被限流（1=限流）| F-OPS-006 |
| `sensitive_field_access_count` | Counter | user_id, field_type | 敏感字段访问次数 | F-OPS-007 |
| `sensitive_field_access_alerts_total` | Counter | user_id, alert_level | 敏感字段访问告警次数 | F-OPS-007 |
| `ddl_operations_total` | Counter | command_tag, object_type | DDL 操作总次数 | F-OPS-008 |
| `ddl_critical_alerts_total` | Counter | command_tag | DDL 告警次数（DROP/TRUNCATE）| F-OPS-008 |
| `dr_recovery_duration_seconds` | Gauge | recovery_type | 灾难恢复耗时（秒）| F-OPS-004 |
| `dr_rto_achieved` | Gauge | recovery_type | RTO 是否达标（1=达标）| F-OPS-004 |

---

## 第十三部分：Module 12 — 长期规划功能（v1.7+）

> 以下功能在运维催生功能优化需求清单中标记为 P1/P2 优先级，本次不纳入正式规格，但记录于此以便后续规划参考。

| 序号 | 功能 | 优先级 | 评审依据 | 说明 |
|------|------|--------|---------|------|
| 1 | 慢查询自动分析与优化建议 | P1 | A-02 | slow-query-analyzer.sh + 索引建议生成 |
| 2 | 数据库连接泄漏自动检测 | P1 | A-03 | 连接增长率告警 + 诊断报告 |
| 3 | 表膨胀（bloat）自动检测 | P1 | A-04 | check-table-bloat.sh（cron 每周）|
| 4 | Redis 集群健康检查脚本 | P1 | A-07 | redis-health-check.sh 一键检查 |
| 5 | Kafka 消费者 Lag 自动恢复 | P1 | A-10 | kafka-lag-recovery.sh |
| 6 | 系统配置备份自动化 | P1 | A-13 | backup-system-configs.sh（Webhook 触发）|
| 7 | 用户账户生命周期自动化 | P1 | A-14 | account-lifecycle-manager.sh |
| 8 | 磁盘空间自动清理 | P1 | A-17 | auto-disk-cleaner.sh |
| 9 | 备份任务失败自动重试 | P1 | A-18 | 备份脚本重试逻辑（3次重试）|
| 10 | 数据保留自动清理脚本 | P1 | A-20 | data-retention-cleanup.sh（PDPO 合规）|
| 11 | eClass 数据同步自动补偿 | P1 | A-22 | 同步失败自动等待 15 分钟重试 |
| 12 | 考勤数据完整性监控 | P1 | M-01 | attendance_records_daily_count 指标 |
| 13 | 系统配置集中管理界面 | P1 | C-01 | 系统配置管理页面（SYSTEM 角色）|
| 14 | 敏感配置变更审批流程 | P1 | C-02 | 敏感配置变更需校务主任审批 |
| 15 | system_configs 表变更历史 | P1 | C-05 | system_config_history 表 + 回滚 API |
| 16 | 定时任务执行审计 | P1 | L-01 | scheduled_task_executions 表 |

---

## 第十四部分：Module 13 — 会议管理模块

> **Module 13 (Meeting Management / v1.8.0)** 是基于会议管理模块需求文档（Issue #171）新增的功能模块，涵盖4项P2优先级需求：会议安排、会议记录、任务分派及到期提醒。

### 14.1 模块概述

| 属性 | 描述 |
|------|------|
| 模块名称 | Meeting Management Module — 会议管理模块 |
| 模块ID | MOD-MEET-001 |
| 优先级 | P2 |
| 用户 | 校务处同工, 校务主任, 教师 |
| 评审依据 | Issue #171 — 会议管理模块需求文档（2026-06-28）|

### 14.2 数据模型设计

#### 核心实体关系

```
┌─────────────┐       ┌─────────────────────┐       ┌─────────────┐
│   meetings   │◄─────►│ meeting_participants │◄─────►│    users    │
│   (会议)     │       │     (参与者)         │       │   (用户)    │
└──────┬──────┘       └─────────────────────┘       └─────────────┘
       │
       ▼
┌─────────────┐       ┌─────────────┐
│ meeting_    │◄─────►│    tasks    │
│  records    │       │   (任务)    │
│  (纪要)     │       └─────────────┘
└─────────────┘
```

---

### Function F-MEET-001: 会议安排

**函数ID：** F-MEET-001
**函数名称：** 会议安排
**模块：** MOD-MEET-001 (Module 13)
**子模块：** 会议安排
**优先级：** P2
**责任人：** 校务处同工

#### 功能描述

支持创建、编辑、取消校务会议，设置会议时间、地点、参与者等信息，并自动发送会议通知给所有参与者。

#### 输入字段

| 字段 | 类型 | 必填 | 来源 | 描述 |
|------|------|------|------|------|
| meeting_id | String | 自动 | 系统生成 | 会议唯一ID，格式：`MTG-{年份}{月份}{日}-{序号}` |
| meeting_title | String | 是 | 用户输入 | 会议标题/名称 |
| meeting_type | Enum | 是 | 用户选择 | `management`（校务会议）/ `academic`（教学会议）/ `parents`（家长会）/ `emergency`（紧急会议）/ `committee`（委员会）/ `other`（其他）|
| organizer_id | String | 是 | 系统/用户选择 | 主持人/组织者用户ID |
| start_time | DateTime | 是 | 用户选择 | 会议开始时间（Asia/Hong_Kong时区）|
| end_time | DateTime | 是 | 用户选择 | 会议结束时间 |
| location | String | 是 | 用户输入 | 会议地点（如：校务处会议室 / Google Meet链接）|
| is_online | Boolean | 自动 | 系统推断 | 是否为线上会议（根据location格式判断）|
| participants | Array[Participant] | 是 | 用户选择 | 参与者列表 |
| agenda | Text | 可选 | 用户输入 | 会议议程 |
| description | Text | 可选 | 用户输入 | 会议说明/备注 |
| related_students | Array[String] | 可选 | 用户选择 | 关联学生（仅家长会等场景）|
| recurring_id | String | 可选 | 系统 | 周期性会议关联ID |
| status | Enum | 自动 | 系统 | `scheduled`（已安排）/ `in_progress`（进行中）/ `completed`（已完成）/ `cancelled`（已取消）|
| created_at | DateTime | 自动 | 系统 | 创建时间 |
| updated_at | DateTime | 自动 | 系统 | 更新时间 |

#### 处理流程

```
Step 1: 会议信息录入 (Meeting Information Entry)
Step 2: 参与者确认 (Participant Confirmation)
Step 3: 时间冲突检测 (Time Conflict Detection)
Step 4: 会议通知发送 (Notification Dispatch)
Step 5: 周期性会议处理 (Recurring Meeting Processing)
```

**Step 1: 会议信息录入 (Meeting Information Entry)**

| 处理规则 | 说明 | 示例 |
|----------|------|------|
| ID自动生成 | 系统按格式自动生成唯一ID | `MTG-20260628-001` |
| 线上会议识别 | location含 http/https 或特定关键词时自动标记 | Google Meet URL → is_online=true |
| 最低时长 | 会议时长不得少于15分钟 | end_time - start_time >= 15min |
| 会议类型预设 | 不同类型会议有默认议程模板 | 家长会预设「家长反馈」议程 |

**Step 2: 参与者确认 (Participant Confirmation)**

| 处理规则 | 说明 |
|----------|------|
| 用户选择 | 从用户列表中选择参与者 |
| 角色标记 | 可为参与者标记角色：`chair`（主持）/ `secretary`（记录）/ `required`（必须出席）/ `optional`（可选出席）|
| 外聘人员 | 支持添加外部人员（邮箱/手机号），生成一次性邀请链接 |
| 回复状态 | 参与者收到通知后可选「确认出席」/「婉拒」/「待定」|

**Step 3: 时间冲突检测 (Time Conflict Detection)**

| 处理规则 | 说明 |
|----------|------|
| 参与者冲突 | 检测参与者是否已有其他会议时间重叠 |
| 场地冲突 | 检测同一场地同时段是否已有其他会议 |
| 冲突处理 | 冲突时提示用户，可选择「忽略并继续」或「调整时间」|
| 递归检测 | 周期性会议需检测未来N场是否存在冲突 |

**Step 4: 会议通知发送 (Notification Dispatch)**

| 处理规则 | 说明 |
|----------|------|
| 即时通知 | 创建会议后立即发送通知至所有参与者 |
| 渠道 | App推送 + 邮件（根据用户偏好设置）|
| 提前提醒 | 会议开始前24小时、1小时自动发送提醒 |
| 更新通知 | 会议信息变更时重新通知受影响参与者 |
| 取消通知 | 取消会议时通知所有参与者 |

**Step 5: 周期性会议处理 (Recurring Meeting Processing)**

| 处理规则 | 说明 |
|----------|------|
| 重复模式 | 支持每日/每周/每月/每学期重复 |
| 结束条件 | 支持设定结束日期或重复次数 |
| 自动生成 | 系统提前生成未来N场周期性会议（如：未来3个月）|
| 例外日期 | 支持标记单场会议为节假日/学校活动等例外 |

#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 校务主任登录系统，进入会议安排页面 | 点击「新建会议」，填写会议信息（标题=校务处周会，时间=2026-06-30 14:00，地点=会议室A，参与者=张主任/李同工）| 系统生成会议记录，status=scheduled，自动发送App/邮件通知至张主任和李同工 |
| AC-02 | 校务处同工尝试安排某时间段会议，该时段张主任已有另一会议 | 填写会议时间并添加张主任 | 系统显示冲突警告：「张主任在此时段已有『教师例会』」，提供「忽略并继续」或「调整时间」选项 |
| AC-03 | 已安排的会议信息需变更（地点改为会议室B）| 修改会议地点，点击保存 | 系统更新会议记录，所有参与者收到会议更新通知 |
| AC-04 | 某会议需临时取消 | 点击「取消会议」，填写取消原因 | status更新为cancelled，所有参与者收到取消通知 |
| AC-05 | 设置每周一上午9:00的校务处例会（周期性）| 创建会议时勾选「重复」，设置重复模式=每周一 | 系统自动生成未来12周的会议记录，每周一自动发送会议通知 |

#### 输出

```json
{
  "meeting_id": "MTG-20260628-001",
  "meeting_title": "校务处周会",
  "meeting_type": "management",
  "organizer": {
    "id": "USR-001",
    "name": "张主任",
    "role": "校务主任"
  },
  "start_time": "2026-06-30T14:00:00+08:00",
  "end_time": "2026-06-30T15:00:00+08:00",
  "location": "会议室A",
  "is_online": false,
  "participants": [
    {
      "user_id": "USR-001",
      "name": "张主任",
      "role": "chair",
      "rsvp_status": "accepted"
    },
    {
      "user_id": "USR-002",
      "name": "李同工",
      "role": "required",
      "rsvp_status": "pending"
    }
  ],
  "agenda": "1. 上周工作回顾\n2. 本周重点任务\n3. 事项通报",
  "status": "scheduled",
  "notifications_sent": {
    "app_push": 2,
    "email": 2
  },
  "created_at": "2026-06-28T15:30:00+08:00"
}
```

---

### Function F-MEET-002: 会议记录

**函数ID：** F-MEET-002
**函数名称：** 会议记录
**模块：** MOD-MEET-001 (Module 13)
**子模块：** 会议记录
**优先级：** P2
**责任人：** 记录员（由主持人指定或自动分配）

#### 功能描述

在会议进行中或结束后，记录会议纪要、决策事项、行动项目等关键信息，支持AI辅助生成纪要草稿。

#### 输入字段

| 字段 | 类型 | 必填 | 来源 | 描述 |
|------|------|------|------|------|
| record_id | String | 自动 | 系统生成 | 记录唯一ID，格式：`RECMTG-{meeting_id}-{序号}` |
| meeting_id | String | 是 | 系统/关联 | 所属会议ID |
| recorder_id | String | 是 | 系统/用户选择 | 记录员用户ID |
| record_type | Enum | 是 | 用户选择 | `minutes`（会议纪要）/ `decisions`（决策事项）/ `action_items`（行动项目）/ `attachments`（附件上传）|
| content | Text | 是 | 用户输入/AI生成 | 记录内容 |
| decisions | Array[Decision] | 可选 | 用户输入 | 会议决策事项列表 |
| key_points | Array[String] | 可选 | 用户输入 | 关键讨论要点 |
| next_meeting | Object | 可选 | 用户输入 | 下次会议安排建议 |
| attachments | Array[Attachment] | 可选 | 用户上传 | 附件（PDF/图片/文档）|
| is_draft | Boolean | 自动 | 系统 | 是否为草稿 |
| approved_by | String | 可选 | 用户审批 | 审批人ID（纪要审批）|
| approved_at | DateTime | 可选 | 系统 | 审批时间 |
| created_at | DateTime | 自动 | 系统 | 创建时间 |
| updated_at | DateTime | 自动 | 系统 | 更新时间 |

#### 处理流程

```
Step 1: 会议纪要录入 (Minutes Entry)
Step 2: AI辅助生成草稿 (AI Draft Generation)
Step 3: 决策事项提取 (Decision Extraction)
Step 4: 行动项目关联 (Action Item Linking)
Step 5: 审批流程 (Approval Workflow)
Step 6: 定稿归档 (Finalization & Archive)
```

**Step 1: 会议纪要录入 (Minutes Entry)**

| 处理规则 | 说明 |
|----------|------|
| 实时录入 | 会议进行中可实时更新纪要 |
| 离线支持 | 支持离线录入后同步 |
| 历史回溯 | 同一会议可有多份纪要（不同记录员）|

**Step 2: AI辅助生成草稿 (AI Draft Generation)**

| 处理规则 | 说明 |
|----------|------|
| 触发条件 | 会议结束后自动提示是否需要AI生成 |
| 输入来源 | 基于议程（agenda）+ 会议讨论记录（如有）|
| 输出内容 | 自动生成纪要草稿、决策事项建议 |
| 人工审核 | AI草稿须经记录员审核修改后方可定稿 |

**Step 3: 决策事项提取 (Decision Extraction)**

| 处理规则 | 说明 |
|----------|------|
| 格式要求 | 每项决策须包含：决策内容、决策理由、负责人、截止日期 |
| 状态追踪 | 决策可标记状态：`pending`（待执行）/ `in_progress`（执行中）/ `completed`（已完成）/ `cancelled`（已取消）|

**Step 4: 行动项目关联 (Action Item Linking)**

| 处理规则 | 说明 |
|----------|------|
| 快速分派 | 纪要中的行动项目可直接分派为系统任务（见 F-MEET-003）|
| 关联决策 | 行动项目可关联至对应决策事项 |

**Step 5: 审批流程 (Approval Workflow)**

| 处理规则 | 说明 |
|----------|------|
| 审批人 | 默认为会议主持人（organizer）|
| 审批流程 | 记录员提交草稿 → 主持人审批 → 定稿 |
| 通知 | 审批结果通知记录员 |

**Step 6: 定稿归档 (Finalization & Archive)**

| 处理规则 | 说明 |
|----------|------|
| 定稿后锁定 | 定稿后内容不可直接修改，须通过「修订」流程 |
| 归档 | 定稿后自动归档，保留完整版本历史 |

#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 校务处周会结束后，记录员进入会议记录页面 | 点击「生成纪要」，AI根据议程生成草稿 | 系统显示AI生成的纪要草稿，包含讨论要点、决策事项建议 |
| AC-02 | AI生成纪要草稿后 | 记录员修改审核后点击「提交审批」| 纪要状态为pending_approval，通知推送至主持人张主任 |
| AC-03 | 主持人张主任收到审批通知 | 点击「批准」，纪要定稿 | 纪要状态更新为approved，内容锁定，所有参与者可查看 |
| AC-04 | 会议中讨论了「下学期午膳供应商更换」并形成决议 | 记录员录入决策事项：决策=更换供应商，负责人=李同工，截止日期=2026-07-15 | 系统生成决策记录，状态=pending，届时自动发送提醒至李同工 |
| AC-05 | 定稿后需补充遗漏内容 | 记录员申请修订，填写修订原因 | 纪要解锁修改，修改后重新审批，原版本保留为历史记录 |

#### 输出

```json
{
  "record_id": "RECMTG-MTG-20260628-001-001",
  "meeting_id": "MTG-20260628-001",
  "meeting_title": "校务处周会",
  "recorder": {
    "id": "USR-002",
    "name": "李同工"
  },
  "record_type": "minutes",
  "content": "会议时间：2026-06-28 14:00-15:00\n出席：张主任（主持）、李同工、王老师\n\n一、上周工作回顾\n- 课本分发完成率98%，剩余2人待补发\n\n二、本周重点任务\n- 6月30日前完成DSE成绩录入\n- 7月1日家长信息说明会\n\n三、决策事项\n- 决定更换下学期午膳供应商（详见决策事项列表）",
  "decisions": [
    {
      "decision_id": "DEC-001",
      "content": "更换下学期午膳供应商为新膳食公司",
      "reason": "原供应商合同到期，新供应商报价低15%",
      "responsible_user": {"id": "USR-002", "name": "李同工"},
      "deadline": "2026-07-15",
      "status": "pending"
    }
  ],
  "key_points": [
    "课本分发本周内全部完成",
    "DSE成绩录入须在6月30日前完成",
    "家长说明会将于7月1日举行"
  ],
  "is_draft": false,
  "approved_by": {
    "id": "USR-001",
    "name": "张主任"
  },
  "approved_at": "2026-06-28T16:00:00+08:00",
  "created_at": "2026-06-28T15:30:00+08:00",
  "updated_at": "2026-06-28T16:00:00+08:00"
}
```

---

### Function F-MEET-003: 任务分派（从会议生成任务）

**函数ID：** F-MEET-003
**函数名称：** 任务分派（从会议生成任务）
**模块：** MOD-MEET-001 (Module 13)
**子模块：** 任务分派
**优先级：** P2
**责任人：** 记录员或主持人

#### 功能描述

从会议纪要中提取行动项目，一键分派为系统任务，自动关联至会议记录，支持任务跟踪和到期提醒。

#### 输入字段

| 字段 | 类型 | 必填 | 来源 | 描述 |
|------|------|------|------|------|
| task_id | String | 自动 | 系统生成 | 任务唯一ID |
| meeting_id | String | 是 | 关联会议 | 所属会议ID |
| record_id | String | 可选 | 关联纪要 | 关联会议记录ID |
| task_title | String | 是 | 用户输入/AI提取 | 任务标题 |
| task_description | Text | 可选 | 用户输入 | 任务描述 |
| responsible_user_id | String | 是 | 用户选择 | 负责人用户ID |
| assigned_by | String | 自动 | 系统 | 分派人（默认为分派操作者）|
| due_date | Date | 是 | 用户选择 | 截止日期 |
| priority | Enum | 是 | 用户选择 | `low`（低）/ `medium`（中）/ `high`（高）/ `urgent`（紧急）|
| related_students | Array[String] | 可选 | 用户选择 | 关联学生 |
| status | Enum | 自动 | 系统 | `pending`（待接受）/ `in_progress`（进行中）/ `completed`（已完成）/ `cancelled`（已取消）|
| created_at | DateTime | 自动 | 系统 | 创建时间 |

#### 处理流程

```
Step 1: 行动项目提取 (Action Item Extraction)
Step 2: 任务信息补全 (Task Information Completion)
Step 3: 负责人确认 (Responsible User Confirmation)
Step 4: 任务通知 (Task Notification)
Step 5: 到期提醒设置 (Due Date Reminder Setup)
```

**Step 1: 行动项目提取 (Action Item Extraction)**

| 处理规则 | 说明 |
|----------|------|
| AI辅助提取 | 系统从纪要内容中自动识别行动项目建议 |
| 手动添加 | 支持手动添加行动项目 |
| 批量分派 | 可一次性将多个行动项目分派为任务 |

**Step 2: 任务信息补全 (Task Information Completion)**

| 处理规则 | 说明 |
|----------|------|
| 必填字段 | 任务标题、负责人、截止日期 |
| 优先级预设 | 根据来源决策的紧急程度自动预设，可人工调整 |
| 关联学生 | 如任务涉及学生，可关联学生记录 |

**Step 3: 负责人确认 (Responsible User Confirmation)**

| 处理规则 | 说明 |
|----------|------|
| 通知负责人 | 分派后自动通知负责人 |
| 接受/拒绝 | 负责人可接受或拒绝任务，拒绝须填写原因 |
| 重新分派 | 主持人可将任务转派给其他人员 |

**Step 4: 任务通知 (Task Notification)**

| 处理规则 | 说明 |
|----------|------|
| 通知内容 | 任务标题、来源会议、截止日期、描述 |
| 渠道 | App推送 + 邮件 |

**Step 5: 到期提醒设置 (Due Date Reminder Setup)**

| 处理规则 | 说明（详见 F-MEET-004）|
|----------|------|
| 提醒规则 | 支持多级提醒（见 F-MEET-004）|
| 升级机制 | 未完成时自动升级通知（见 F-MEET-004）|

#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 会议纪要已定稿，其中包含3个行动项目 | 记录员点击「分派任务」，勾选3个行动项目，选择对应负责人和截止日期 | 系统生成3条任务记录，关联会议ID和纪要ID，自动发送通知至各负责人 |
| AC-02 | 分派的任务截止日期为2026-07-15 | 系统检测任务未完成（截止当天）| 负责人李同工收到App+邮件到期提醒 |
| AC-03 | 负责人李同工完成任务 | 点击「标记完成」，上传完成说明 | 任务状态更新为completed，关联的会议决策状态同步更新为completed |
| AC-04 | 负责人无法完成任务，申请延期 | 点击「申请延期」，填写原因和新截止日期 | 系统通知分派人审批，审批通过后更新截止日期 |

#### 输出

```json
{
  "task_id": "TASK-MTG-20260628-001-001",
  "meeting_id": "MTG-20260628-001",
  "record_id": "RECMTG-MTG-20260628-001-001",
  "decision_id": "DEC-001",
  "task_title": "跟进更换下学期午膳供应商",
  "task_description": "联系新膳食公司，谈判合同条款，于7月15日前完成",
  "responsible_user": {
    "id": "USR-002",
    "name": "李同工"
  },
  "assigned_by": {
    "id": "USR-001",
    "name": "张主任"
  },
  "due_date": "2026-07-15",
  "priority": "high",
  "status": "pending",
  "reminders": [
    {"type": "due_date", "sent_at": "2026-07-15T09:00:00+08:00"},
    {"type": "overdue", "sent_at": null}
  ],
  "created_at": "2026-06-28T16:30:00+08:00"
}
```

---

### Function F-MEET-004: 到期提醒（自动提醒负责人）

**函数ID：** F-MEET-004
**函数名称：** 到期提醒（自动提醒负责人）
**模块：** MOD-MEET-001 (Module 13)
**子模块：** 提醒管理
**优先级：** P2
**责任人：** 系统自动执行

#### 功能描述

对会议相关的任务、决策事项、周期性会议等自动发送到期提醒，支持多级提醒机制和升级通知。

#### 提醒类型与规则

| 提醒类型 | 触发时机 | 通知对象 | 渠道 | 说明 |
|----------|---------|---------|------|------|
| 会议前24h提醒 | 会议开始前24h | 全体参与者 | App+邮件 | 会议即将开始提醒 |
| 会议前1h提醒 | 会议开始前1h | 全体参与者 | App+短信 | 紧急提醒 |
| 任务到期提醒 | 任务截止当天09:00 | 负责人 | App+邮件 | 任务今日到期 |
| 任务逾期提醒 | 任务逾期当天 | 负责人 | App+短信 | 任务已逾期 |
| 任务升级提醒 | 逾期超过48h | 负责人+分派人 | App+短信+邮件 | 未完成时升级通知 |
| 任务二次升级 | 逾期超过96h | 负责人+分派人+主持人 | App+短信+邮件 | 仍未完成再次升级 |
| 决策待跟进提醒 | 决策截止前3天 | 负责人 | App | 决策即将到期 |
| 决策逾期提醒 | 决策逾期 | 负责人+主持人 | App+短信 | 决策已逾期 |
| 周期性会议生成提醒 | 周期性会议创建后 | 全体参与者 | App+邮件 | 提醒未来会议已自动生成 |

#### 升级机制

| 阶段 | 触发条件 | 通知对象扩展 | 告警级别 |
|------|---------|-------------|---------|
| Level 1 | 截止当天未完成 | 负责人 | 普通 |
| Level 2 | 逾期超过48h | 负责人 + 分派人 | 中等 |
| Level 3 | 逾期超过96h | 负责人 + 分派人 + 会议主持人 | 高 |
| Level 4 | 逾期超过168h（1周）| 负责人 + 分派人 + 主持人 + 校务主任 | 紧急 |

#### 处理流程

```
Step 1: 提醒任务扫描 (Reminder Scan)
Step 2: 提醒规则匹配 (Rule Matching)
Step 3: 多渠道通知发送 (Multi-channel Notification)
Step 4: 升级判定 (Escalation Check)
Step 5: 提醒状态记录 (Reminder Status Logging)
```

**Step 1: 提醒任务扫描 (Reminder Scan)**

| 处理规则 | 说明 |
|----------|------|
| 扫描频率 | 每小时执行一次全量扫描 |
| 提前提醒 | 支持提前N天/小时提醒（可配置）|
| 去重机制 | 同一提醒不重复发送（基于提醒类型+目标对象+触发时间唯一键）|

**Step 2: 提醒规则匹配 (Rule Matching)**

| 处理规则 | 说明 |
|----------|------|
| 优先级继承 | 任务优先级影响提醒频率（高优先级加密提醒）|
| 免打扰时段 | 支持用户设置免打扰时段，提醒在时段结束后发送 |
| 时区处理 | 所有时间统一使用 Asia/Hong_Kong 时区 |

**Step 3: 多渠道通知发送 (Multi-channel Notification)**

| 处理规则 | 说明 |
|----------|------|
| 用户偏好 | 根据用户通知偏好设置选择渠道 |
| 失败重试 | 发送失败自动重试（最多3次，间隔5分钟）|
| 模板管理 | 通知内容使用模板（见 F-NEW-002 多渠道通知模板管理）|

**Step 4: 升级判定 (Escalation Check)**

| 处理规则 | 说明 |
|----------|------|
| 升级触发 | 任务/决策逾期后按时间节点自动升级 |
| 升级通知 | 每次升级通知当前级别所有对象 |
| 升级记录 | 所有升级事件记录至审计日志 |

**Step 5: 提醒状态记录 (Reminder Status Logging)**

| 处理规则 | 说明 |
|----------|------|
| 发送记录 | 每次发送记录：发送时间、渠道、状态 |
| 未送达处理 | 未送达时标记，24小时后重新尝试 |
| 统计报表 | 支持按月/季度生成提醒统计报告 |

#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 校务处周会安排在2026-06-30 14:00 | 会议开始前24小时，系统扫描提醒任务 | 全体参与者收到App+邮件提醒：「校务处周会将于明天14:00在会议室A举行」|
| AC-02 | 「跟进更换下学期午膳供应商」任务截止日期为2026-07-15 | 截止当天09:00，系统扫描到期任务 | 负责人李同工收到App+邮件提醒：「您负责的任务『跟进更换下学期午膳供应商』今日到期」|
| AC-03 | 李同工忘记完成任务，截止日期已过2天 | 系统扫描逾期任务（逾期>48h）| Level 2升级：除李同工外，分派人张主任也收到提醒 |
| AC-04 | 任务逾期超过1周仍未完成 | 系统扫描（逾期>168h）| Level 4升级：张主任+校务主任均收到紧急提醒 |
| AC-05 | 负责人设置了免打扰时段（22:00-08:00）| 系统本应在23:00发送提醒 | 提醒顺延至08:00后发送，不在免打扰时段内 |

#### 输出

```json
{
  "reminder_id": "REM-TASK-20260715-001",
  "source_type": "task",
  "source_id": "TASK-MTG-20260628-001-001",
  "source_title": "跟进更换下学期午膳供应商",
  "trigger_type": "due_date",
  "trigger_time": "2026-07-15T09:00:00+08:00",
  "recipient": {
    "id": "USR-002",
    "name": "李同工"
  },
  "channels": ["app_push", "email"],
  "notification_content": {
    "title": "任务到期提醒",
    "body": "您负责的任务『跟进更换下学期午膳供应商』今日到期，请及时处理。",
    "meeting_context": "来源会议：校务处周会（2026-06-28）"
  },
  "status": "sent",
  "sent_at": "2026-07-15T09:00:05+08:00",
  "escalation_level": 1,
  "escalation_notified": []
}
```

---

## 跨模块总结表.6 | 缺漏需求#11 | 校内文件共享与版本控制 |
| 7 | 采购供应链管理 | v1.7 | 缺漏需求#8 | 课本/文具采购订单管理 |
| 8 | 历史数据批量导入 | v1.6 | 缺漏需求#5 | 批量导入历史数据 |
| 9 | 第三方API网关 | v1.6 | 缺漏需求#14 | 统一管理第三方API接入 |

---

## 跨模块总结表

### 功能总表

| 模块 | 子模块 | 功能ID | 功能名称 | 优先级 |
|------|--------|--------|---------|--------|
| MOD-USER-001 | 用户管理 | F-USER-001 | 用户生命周期管理 | P0 |
| MOD-USER-001 | 认证 | F-USER-002 | 身份认证 | P0 |
| MOD-USER-001 | 授权 | F-USER-003 | 功能授权 (RBAC+ABAC) | P0 |
| MOD-USER-001 | 会话 | F-USER-004 | 会话与Token管理 | P0 |
| MOD-USER-001 | 审计 | F-USER-005 | 审计日志与登录记录 | P0 |
| MOD-USER-001 | 凭证 | F-USER-006 | 密码与凭证重置 | P0 |
| MOD-USER-001 | 审批 | F-USER-007 | 权限变更审批流程 | P1 |
| MOD-DAILY-001 | 出勤 | F-ATT-001 | 出勤统计 | P0 |
| MOD-DAILY-001 | 出勤 | F-ATT-002 | 迟到/早退记录 | P0 |
| MOD-DAILY-001 | 查询 | F-INQ-001 | 家长查询队列 | P0 |
| MOD-DAILY-001 | 查询 | F-INQ-002 | 快速回复模板 | P1 |
| MOD-DAILY-001 | 午膳 | F-LUNCH-001 | 午膳订购汇总 | P0 |
| MOD-DAILY-001 | 校车 | F-BUS-001 | 校车实时追踪 | P0 |
| MOD-DAILY-001 | 校车 | F-BUS-002 | 校车点大名记录 | P0 |
| MOD-DAILY-001 | 请假 | F-LEAVE-001 | 请假申请处理 | P0 |
| MOD-DAILY-001 | 收费 | F-FEE-001 | 费用收取追踪 | P0 |
| MOD-CYCL-001 | 注册 | F-ENRL-001 | 新生注册 | P0 |
| MOD-CYCL-001 | 注册 | F-ENRL-002 | AI辅助编班 | P0 |
| MOD-CYCL-001 | 注册 | F-ENRL-003 | 课本分发 | P1 |
| MOD-CYCL-001 | 考试 | F-EXAM-001 | DSE报考管理 | P0 |
| MOD-CYCL-001 | 考试 | F-EXAM-002 | 试卷管理 | P0 |
| MOD-CYCL-001 | 考试 | F-EXAM-003 | 特别考试安排 | P0 |
| MOD-CYCL-001 | 考试 | F-EXAM-004 | 成绩单生成发布 | P0 |
| MOD-CYCL-001 | 收生 | F-ADM-001 | 中一自行分配 | P0 |
| MOD-CYCL-001 | 收生 | F-ADM-002 | JUPAS联招管理 | P0 |
| MOD-CYCL-001 | 年终 | F-YREND-001 | 档案清理销毁 | P1 |
| MOD-CYCL-001 | 年终 | F-YREND-002 | 学年财务结算 | P0 |
| MOD-FIN-001 | 收费 | F-FIN-001 | 学费管理 | P0 |
| MOD-FIN-001 | 收费 | F-FIN-002 | 零用现金报销 | P0 |
| MOD-FIN-001 | 收费 | F-FIN-003 | 奖学金津贴申请 | P0 |
| MOD-FIN-001 | 资产 | F-ASSET-001 | 校产条码盘点 | P1 |
| MOD-FIN-001 | 资产 | F-ASSET-002 | 场地租借管理 | P1 |
| MOD-FIN-001 | 资产 | F-ASSET-003 | 设备保养管理 | P1 |
| MOD-FIN-001 | 供应商 | F-VEND-001 | 供应商注册评估 | P1 |
| MOD-AI-001 | NLP | F-AI-001 | 自然语言查询理解 | P0 |
| MOD-AI-001 | NLP | F-AI-002 | FAQ智能匹配 | P0 |
| MOD-AI-001 | 自动化 | F-AUTO-001 | 周期性任务触发器 | P0 |
| MOD-AI-001 | 自动化 | F-AUTO-002 | 智能提醒系统 | P0 |
| MOD-AI-001 | 文档 | F-AI-003 | OCR文档识别 | P1 |
| MOD-INT-001 | WebSAMS | F-INT-001 | WebSAMS数据同步 | P0 |
| MOD-INT-001 | eClass | F-INT-002 | eClass系统集成 | P0 |
| MOD-INT-001 | 合规 | F-COMP-001 | 隐私条例合规检查 | P0 |
| MOD-INT-001 | 合规 | F-COMP-002 | 双人见证流程 | P0 |
| MOD-INT-001 | 合规 | F-COMP-003 | 审计日志管理 | P0 |
| MOD-INT-001 | 备份 | F-BACK-001 | 自动备份管理 | P0 |

| MOD-INT-001 | 备份 | F-BACK-001 | 自动备份管理 | P0 |
| MOD-I18N-001 | 框架 | F-I18N-001 | 多语言框架与翻译管理 | P0 |
| MOD-I18N-001 | 检测 | F-I18N-002 | 语言检测与自动切换 | P0 |
| MOD-I18N-001 | 翻译 | F-I18N-003 | 实时内容翻译 (LLM) | P1 |
| MOD-I18N-001 | 格式 | F-I18N-004 | 区域化与格式本地化 | P1 |
| MOD-NEW-001 | 灾难恢复 | F-NEW-001 | 一键灾难恢复与业务连续性 | P0 |
| MOD-NEW-001 | 通知管理 | F-NEW-002 | 多渠道通知模板管理 | P1 |
| MOD-NEW-001 | 考试管理 | F-NEW-003 | DSE放榜成绩追踪 | P1 |
| MOD-NEW-001 | 财务管理 | F-NEW-004 | 年度预算编制与执行追踪 | P1 |
| MOD-NEW-001 | 报表中心 | F-NEW-005 | 自定义报表生成与定时推送 | P1 |
| MOD-NEW-001 | 系统运维 | F-NEW-006 | 系统健康检查与自动诊断 | P2 |
| MOD-OPS-001 | 自动化脚本 | F-OPS-001 | 数据库健康检查与 WAL 积压自动处理 | P0 |
| MOD-OPS-001 | 自动化脚本 | F-OPS-002 | SSL 证书到期自动续期 | P0 |
| MOD-OPS-001 | 自动化脚本 | F-OPS-003 | WebSAMS Token 自动刷新 | P0 |
| MOD-OPS-001 | 灾难恢复 | F-OPS-004 | 一键灾难恢复脚本 | P0 |
| MOD-OPS-001 | 监控增强 | F-OPS-005 | 审计日志写入完整性监控 | P0 |
| MOD-OPS-001 | 监控增强 | F-OPS-006 | Coze API 配额实时监控 | P0 |
| MOD-OPS-001 | 安全监控 | F-OPS-007 | 敏感字段查看频率告警 | P0 |
| MOD-OPS-001 | 审计增强 | F-OPS-008 | 数据库 DDL 操作审计 | P0 |
| MOD-OPS-001 | 监控体系 | F-OPS-009 | 运维健康仪表板 | P0 |
| MOD-MEET-001 | 会议安排 | F-MEET-001 | 会议安排 | P2 |
| MOD-MEET-001 | 会议记录 | F-MEET-002 | 会议记录 | P2 |
| MOD-MEET-001 | 任务分派 | F-MEET-003 | 任务分派（从会议生成任务）| P2 |
| MOD-MEET-001 | 提醒管理 | F-MEET-004 | 到期提醒（自动提醒负责人）| P2 |
| MOD-RECRUIT-001 | 职位管理 | F-RECRUIT-001 | 职位发布与管理 | P2 |
| MOD-RECRUIT-001 | 简历管理 | F-RECRUIT-002 | 简历收集与筛选 | P2 |
| MOD-RECRUIT-001 | 面试安排 | F-RECRUIT-003 | 面试安排 | P2 |
| MOD-RECRUIT-001 | 录用审批 | F-RECRUIT-004 | 录用审批流程 | P2 |
| MOD-RECRUIT-001 | 入职管理 | F-RECRUIT-005 | 入职手续办理 | P2 |
| MOD-STU-001 | 学生档案 | F-STU-001 | 学生档案创建与管理 | P0 |
| MOD-STU-001 | 学号管理 | F-STU-002 | 学号自动生成 | P0 |
| MOD-STU-001 | 班级分配 | F-STU-003 | 班级按学年动态分配 | P0 |
| MOD-ATTQR-001 | QR签到 | F-ATTQR-001 | 动态QR码生成与展示 | P1 |
| MOD-ATTQR-001 | QR签到 | F-ATTQR-002 | 扫码签到记录 | P1 |
| MOD-ATTQR-001 | QR签到 | F-ATTQR-003 | 日报表推送 | P1 |
| MOD-ATTQR-001 | QR签到 | F-ATTQR-004 | 家长考勤查询 | P2 |
| MOD-PORTAL-AC-001 | 档案管理 | F-PORTAL-001 | 个人档案查看与有限自修改 | P1 |
| MOD-PORTAL-AC-001 | 请假管理 | F-PORTAL-002 | 电子请假 | P1 |
| MOD-PORTAL-AC-001 | 数据安全 | F-PORTAL-003 | 数据隔离规则 | P0 |

## 第十五部分：Module 14 — 教师招聘管理模块 (v1.8.1 新增)

### 模块概述

| 属性 | 描述 |
|------|------|
| 模块名称 | 教师招聘管理 (Teacher Recruitment Management) |
| 模块ID | MOD-RECRUIT-001 |
| 优先级 | P2 |
| 用户 | 校务主任、人事管理员 |
| 使用频率 | 按需（每学期招聘季） |

---

### Function F-RECRUIT-001: 职位发布与管理

**函数ID：** F-RECRUIT-001
**函数名称：** 职位发布与管理
**模块：** MOD-RECRUIT-001 (Module 14)
**子模块：** 职位管理
**优先级：** P2
**责任人：** 校务主任

#### 功能描述

支持学校发布教师招聘职位，管理职位信息（职位名称、学科、薪资范围、工作地点、要求条件等），并支持职位的编辑、暂停、关闭等生命周期管理。

#### 职位信息

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| title | String | ✅ | 职位名称，如"中文科教师" |
| subject | String | ✅ | 教授学科 |
| employment_type | Enum | ✅ | FULL_TIME/PART_TIME/CONTRACT |
| salary_range | Object | ✅ | {min, max, currency} |
| location | String | ✅ | 工作地点 |
| requirements | Text[] | ✅ | 任职要求列表 |
| responsibilities | Text[] | ✅ | 工作职责列表 |
| benefits | Text[] | ❌ | 福利待遇 |
| application_deadline | Date | ✅ | 申请截止日期 |
| status | Enum | ✅ | DRAFT/PUBLISHED/CLOSED |

#### 职位状态

| 状态 | 说明 | 可执行操作 |
|------|------|-----------|
| DRAFT | 草稿 | 编辑、发布 |
| PUBLISHED | 已发布 | 暂停、关闭 |
| PAUSED | 已暂停 | 重新发布、关闭 |
| CLOSED | 已关闭 | 仅查看 |

---

### Function F-RECRUIT-002: 简历收集与筛选

**函数ID：** F-RECRUIT-002
**函数名称：** 简历收集与筛选
**模块：** MOD-RECRUIT-001 (Module 14)
**子模块：** 简历管理
**优先级：** P2
**责任人：** 人事管理员

#### 功能描述

收集求职者的申请资料，支持简历上传、在线申请表格填写，并对收到的申请进行筛选、分类、标记等管理操作。

#### 申请信息

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| applicant_name | String | ✅ | 申请人姓名 |
| email | String | ✅ | 邮箱（登录账号） |
| phone | String | ✅ | 联系电话 |
| position_id | UUID | ✅ | 申请职位 |
| cv_url | String | ✅ | 简历文件URL |
| cover_letter | Text | ❌ | 求职信 |
| education | Object[] | ✅ | 教育背景 [{degree, school, major, year}] |
| experience | Object[] | ❌ | 工作经历 [{company, position, duration, description}] |
| status | Enum | ✅ | NEW/SCREENING/SHORTLISTED/INTERVIEW/REJECTED/OFFER |

#### 筛选状态

| 状态 | 说明 | 可执行操作 |
|------|------|-----------|
| NEW | 新申请 | 标记为筛选中 |
| SCREENING | 筛选中 | 进入候选、淘汰 |
| SHORTLISTED | 候选 | 安排面试 |
| INTERVIEW | 面试中 | 录用、淘汰 |
| REJECTED | 已淘汰 | 查看记录 |
| OFFER | 已发Offer | 入职 |

---

### Function F-RECRUIT-003: 面试安排

**函数ID：** F-RECRUIT-003
**函数名称：** 面试安排
**模块：** MOD-RECRUIT-001 (Module 14)
**子模块：** 面试管理
**优先级：** P2
**责任人：** 人事管理员

#### 功能描述

管理面试流程，包括面试时间安排、面试官指派、面试形式设定（线上/线下）、面试评分及面试结果记录。

#### 面试信息

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| application_id | UUID | ✅ | 关联申请 |
| interview_date | DateTime | ✅ | 面试时间 |
| interviewers | UUID[] | ✅ | 面试官列表 |
| interview_type | Enum | ✅ | ONLINE/ONSITE |
| meeting_link | String | ❌ | 线上会议链接 |
| location | String | ❌ | 线下地点 |
| duration_minutes | Number | ✅ | 面试时长 |
| notes | Text | ❌ | 备注 |
| status | Enum | ✅ | SCHEDULED/COMPLETED/CANCELLED |

#### 面试评分

| 字段 | 类型 | 说明 |
|------|------|------|
| criterion | String | 评分维度（如"教学能力"、"沟通表达"） |
| score | Number(1-5) | 评分 |
| comment | String | 评语 |
| interviewer_id | UUID | 评分人 |

---

### Function F-RECRUIT-004: 录用审批流程

**函数ID：** F-RECRUIT-004
**函数名称：** 录用审批流程
**模块：** MOD-RECRUIT-001 (Module 14)
**子模块：** 审批管理
**优先级：** P2
**责任人：** 校务主任

#### 功能描述

支持录用决定的审批流程，包括Offer发送、Offer接受/拒绝、签约确认等环节的管理。

#### Offer信息

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| application_id | UUID | ✅ | 关联申请 |
| salary | Number | ✅ | 薪资 |
| start_date | Date | ✅ | 预计到职日期 |
| position | String | ✅ | 录用职位 |
| benefits_package | Object | ❌ | 福利套餐 |
| status | Enum | ✅ | PENDING/ACCEPTED/DECLINED/SIGNED |
| sent_at | DateTime | ✅ | 发送时间 |
| responded_at | DateTime | ❌ | 回应时间 |

#### 审批状态

| 状态 | 说明 |
|------|------|
| PENDING | 待回应 |
| ACCEPTED | 已接受 |
| DECLINED | 已拒绝 |
| SIGNED | 已签约 |

---

### Function F-RECRUIT-005: 入职手续办理

**函数ID：** F-RECRUIT-005
**函数名称：** 入职手续办理
**模块：** MOD-RECRUIT-001 (Module 14)
**子模块：** 入职管理
**优先级：** P2
**责任人：** 人事管理员

#### 功能描述

管理新教师的入职流程，包括入职清单、资料收集、账户开通、系统权限分配等。

#### 入职清单

| 字段 | 类型 | 说明 |
|------|------|------|
| item | String | 清单项目 |
| required | Boolean | 是否必填 |
| status | Enum | PENDING/COMPLETED |
| completed_at | DateTime | 完成时间 |
| document_url | String | 文档URL |

#### 系统账户

| 字段 | 类型 | 说明 |
|------|------|------|
| teacher_profile_id | UUID | 教师档案ID |
| user_account | Object | 系统账户信息 |
| role | Enum | TEACHER |
| default_permissions | String[] | 默认权限 |
| onboarding_status | Enum | PENDING/IN_PROGRESS/COMPLETED |

---


**总计：81个功能函数，涵盖16大模块**（v1.8.1新增Module 14：5项功能；v2.0.0-draft.1新增Module 16 QR签到考勤4项+Module 17学生门户3项）

---

## 模块依赖关系

```
MOD-I18N-001 (多语言支持)
  └── 被所有业务模块依赖（基础层，横向支持）

MOD-USER-001 (用户与权限管理)
  └── 被所有业务模块依赖（基础层）

MOD-DAILY-001 (每日仪表板)
  ├── 依赖 → MOD-USER-001 (用户身份验证)
  ├── 依赖 → MOD-INT-001 (WebSAMS, eClass 数据)
  ├── 依赖 → MOD-AI-001 (AI查询理解)
  └── 输出 → MOD-FIN-001 (费用收取数据)

MOD-CYCL-001 (周期性校务)
  ├── 依赖 → MOD-USER-001 (用户身份验证)
  ├── 依赖 → MOD-DAILY-001 (出勤数据用于编班)
  ├── 依赖 → MOD-INT-001 (WebSAMS 学籍)
  ├── 依赖 → MOD-AI-001 (AI文档识别)
  └── 输出 → MOD-FIN-001 (考试费、奖学金)

MOD-FIN-001 (财务资产)
  ├── 依赖 → MOD-USER-001 (用户身份 + 审批流程)
  ├── 依赖 → MOD-CYCL-001 (周期性收费)
  ├── 依赖 → MOD-DAILY-001 (日常收款)
  └── 强合规 → MOD-INT-001 (PDPO, 双人见证)

MOD-AI-001 (AI助理)
  └── 被所有模块调用

MOD-INT-001 (整合合规)
  └── 被所有业务模块依赖

MOD-NEW-001 (新增功能/v1.6.0)
  ├── 依赖 → MOD-USER-001 (身份验证)
  ├── 依赖 → MOD-AI-001 (报表生成)
  ├── 依赖 → MOD-INT-001 (系统监控接口)
  └── 输出 → 通知给所有业务模块用户

MOD-OPS-001 (运维自动化与监控/v1.7.0)
  ├── 依赖 → MOD-USER-001 (身份验证 + 审计日志)
  ├── 依赖 → MOD-INT-001 (数据库监控接口、备份状态)
  ├── 依赖 → MOD-AI-001 (AI 服务配额监控)
  └── 输出 → 告警通知给系统管理员 + 校务主任

MOD-MEET-001 (会议管理/v1.8.0)
  ├── 依赖 → MOD-USER-001 (用户身份验证 + 参与者管理)
  ├── 依赖 → MOD-AI-001 (AI辅助纪要生成)
  ├── 依赖 → MOD-INT-001 (通知渠道集成)
  └── 输出 → 会议任务 → MOD-CYCL-001 (周期性事务跟进)

模块依赖关系说明：
- MOD-USER-001 是所有模块的基础依赖，所有涉及用户身份的操作
  （登录、权限校验、数据归属判断）均依赖本模块
- MOD-I18N-001 为横向支持层，所有涉及用户界面文本、格式本地化的模块均依赖本模块
- MOD-AI-001 与 MOD-INT-001 为横向支撑层，被各业务模块调用
```

---



# 附录 A：文档版本管理 (Document Version Management)

## A.1 变更日志 (Changelog)

> 所有正式版本的变更必须记录于此。变更按发布日期倒序排列。

| 版本 | 发布日期 | 发布者 | 变更类型 | 变更摘要 | 审批人 |
|------|----------|--------|----------|----------|--------|
| **v2.0.0-draft.1** | 2026-07-14 | REQ | Draft | **新增 Module 16 — QR签到考勤(4项功能) 和 Module 17 — 学生/家长门户权限控制(3项功能)**：CR-20260714-001；新增F-ATTQR-001~004及F-PORTAL-001~003；功能函数总数74→81，模块14→16 |
| **v1.9.0** | 2026-07-02 | PM | Minor | **新增 Module 15 — 学生档案管理模块**：Issue #194；学生档案与系统用户分离，学号自动生成，班级按学年动态分配；F-STU-001~003；功能函数总数71→74，模块14→15 |
| **v1.8.1** | 2026-06-28 | PM | Minor | **新增 Module 14 — 教师招聘管理模块**：Issue #175；新增5项P2优先级功能（F-RECRUIT-001至F-RECRUIT-005）：职位发布与管理、简历收集与筛选、面试安排、录用审批流程、入职手续办理；功能函数总数69→74，模块13→14 |
| **v1.8.0** | 2026-06-28 | 系统架构团队 | Minor | **新增 Module 13 — 会议管理模块**：Issue #171；新增4项P2优先级功能（F-MEET-001至F-MEET-004）：会议安排（时间、地点、参与者、时间冲突检测、周期性会议）、会议记录（纪要、决策事项、AI辅助生成、审批流程）、任务分派（从会议生成任务、关联决策事项、状态追踪）、到期提醒（多级提醒机制、升级通知、免打扰时段）；功能函数总数65→69，模块12→13 |
| **v1.7.1** | 2026-06-09 | 系统架构团队 | Minor | **需求文档补充**：依据GitHub Issues优先级清单（P0/P1/P2共8项缺失）全面补充：① **UI文档补充（SPEC-UI-PROTO.md v1.3.0）**：新增家长门户完整UI设计（10个页面）、午膳管理独立页面（4个页面）、财务报销管理页面、灾难恢复操作界面、运维仪表板详细设计、家长自助变更微信端页面；② **SPEC-COMPLETE.md AC补充**：F-INQ-001新增8条Given/When/Then格式AC验收标准、F-FIN-002新增10条完整AC验收标准+8条测试用例+6类异常处理、F-USER-003新增ABAC规则详细示例（7个OPA/Rego代码示例+11条测试用例）；③ 新增MOD-PARENT-001/MOD-LUNCH-001功能编号对照；④ 文档版本同步更新 | （待填）|

| **v1.7.0-Patch1** | 2026-06-06 | 系统架构团队 | Patch | **v1.7.0质检修复**：① **F-ENRL-003补全**（🔴致命）：新增完整输入字段定义（16个字段）、6步处理流程、8条Given/When/Then验收标准AC；② **RBAC权限矛盾修正**（🟡重要）：拆分「费用收取」与「费用查询」，明确TEACHER角色对「费用收取」无操作权限（❌），对「费用查询」有本班学生费用状态查看权限（✅），并增加安全澄清注释；③ **新增7个关键函数AC验收标准**：F-ATT-001（8条）、F-LEAVE-001（6条）、F-BUS-001（6条）、F-FIN-001（4条）、F-USER-001（7条）、F-AUTO-002（5条）、F-NEW-003/004/006（各5条）；④ **跨模块数据流转自洽**：验证MOD-DAILY-001→MOD-FIN-001费用数据流（每日收费→学费管理）、MOD-CYCL→MOD-USER-001（编班→班级归属→出勤范围）、MOD-OPS→MOD-USER-001（运维告警→审计日志→审计完整性），无矛盾；⑤ **归档**：archive/SPEC-SCHOOL-ADMIN-v1.7.0.md 已归档 |

| **v1.7.0** | 2026-06-06 | 系统架构团队 | Minor | 依据运维催生功能优化需求清单（v1.0.0，2026-06-06）P0优先级清单全面修订：**新增 Module 11（MOD-OPS-001）**，新增9项运维自动化与监控功能：F-OPS-001数据库健康检查与WAL积压自动处理(P0)、F-OPS-002 SSL证书到期自动续期(P0)、F-OPS-003 WebSAMS Token自动刷新(P0)、F-OPS-004一键灾难恢复脚本(P0)、F-OPS-005审计日志写入完整性监控(P0)、F-OPS-006 Coze API配额实时监控(P0)、F-OPS-007敏感字段查看频率告警(P0)、F-OPS-008数据库DDL操作审计(P0)、F-OPS-009运维健康仪表板(P0)；**新增20项Prometheus监控指标**；**增强F-NEW-006**（运维健康仪表板覆盖全部10项P0运维监控点）；F-NEW-006优先级由P2升级为P0；功能函数总数55→65，模块数9→12（新增Module 11）；F-OPS-004对F-NEW-001实现脚本落地增强（RTO≤30分钟）| （待填）|

| **v1.6.0** | 2026-06-06 | 系统架构团队 | Minor | 依据功能评审报告（REVIEW-FUNC-001，2026-06-06）全面修订：**新增6项功能**（F-NEW-001至F-NEW-006）：一键灾难恢复与业务连续性(P0)、多渠道通知模板管理(P1)、DSE放榜成绩追踪(P1)、年度预算编制与执行追踪(P1)、自定义报表生成与定时推送(P1)、系统健康检查与自动诊断(P2)；**修正7项不合理业务点**：F-ATT-001批量撤销时限5分钟→15分钟、F-LEAVE-001病假自动批准增加AI核验机制、F-FIN-002零用现金限额改为动态CPI调整、F-BUS-001延误通知增加阈值配置(>10分钟)、F-USER-003 ABAC升级为OPA规则引擎、F-USER-007权限审批增加二次认证(OTP)、F-EXAM-004成绩撤回增加审计告警；**新增粤语口语界面**(zh-HK-Cantonese)；功能函数总数49→55，模块数8→9（新增Module 9）| （待填）|

| **v1.5.0** | 2026-06-05 | 系统架构团队 | Minor | 用户模拟反馈整合：基于 USER-SIMULATION-FEEDBACK.md v1.0.0（48条反馈）；增强F-USER-003敏感字段脱敏策略；F-USER-005新增家长通话记录规范；F-USER-007新增权限审批证明文件要求；F-USER-001新增家长密码强制设置；F-DASH-001新增仪表板自定义功能；F-ATT-001新增数据源独立状态、受影响学生列表、批量录入确认预览+撤销；F-INQ-001新增通话质量和满意度字段；F-LUNCH-001新增家长自助变更和截止时间规则；F-BUS-001新增乘搭学生列表、延误通知意外类型标签；F-LEAVE-001新增代课老师课时数显示、跟进提醒、审批进度条；F-FEE-001新增电子收据推送和缴费延迟提示；F-FIN-001新增欠费sub_status；F-FIN-002新增OCR金额视觉区分和双人见证实时推送；F-EXAM-004新增成绩自撤回、PDF导出、班级成绩分布图；F-ADM-002新增推荐信AI字数统计和写作辅助；F-AUTO-002新增消息送达回执、短信备用渠道、Token健康检查 | （待填）|
| **v1.4.0** | 2026-06-03 | 系统架构团队 | Minor | 新增数据库架构设计文档 (DB-SCHEMA.md) 和数据字典 (DATA-DICTIONARY.md)；定义60+数据表，涵盖7大模块；包含ER关系图、表结构定义、索引设计、外键关系、枚举值规范；数据字典定义数据项、业务规则、数据质量规则、敏感数据处理、数据保留政策 | （待填）|
| **v1.3.0** | 2026-05-25 | 系统架构团队 | Minor | 新增 Module 8 — 多语言支持系统：F-I18N-001 至 F-I18N-004（4个函数）；支持繁体中文(zh-HK)、简体中文(zh-CN)、英语(en)；涵盖翻译框架、语言检测、AI实时翻译、区域化格式本地化；功能函数总数 45→49 | （待填）|
| **v1.2.0** | 2026-05-24 | 系统架构团队 | Minor | 新增文档版本管理系统：SemVer 规范、版本修订规则、附录 A 变更记录；Module 4→7、Module 5→8 重新编号；功能函数总数 38→45 | （待填）|
| **v1.1.0** | 2026-05-24 | 系统架构团队 | Minor | 新增 Module 6 — 用户与权限管理系统：F-USER-001 至 F-USER-007（7个函数）；用户生命周期、身份认证、RBAC+ABAC授权、会话Token、审计日志、凭证重置、权限变更审批；MOD-USER-001 纳入模块依赖图 | （待填）|
| **v1.0.0** | 2026-05-23 | 系统架构团队 | Major | 初始版本发布。5大核心模块（Module 1–5），共 38 个功能函数，涵盖每日晨检、周期性校务、财务资产、AI助理、整合合规全部核心功能 | （待填）|

---

## A.2 版本历史详情 (Version History Detail)

### v1.7.0-Patch1 — 2026-06-06 | 质检报告关键问题修复

**变更类型：** Patch

**变更者：** 系统架构团队

**评审依据：** 功能规格质检报告（v1.7.0，2026-06-06）— 5个致命问题中可自行修复的部分

**一、F-ENRL-003课本分发管理补全（🔴致命修复）：**

| 序号 | 变更类型 | 描述 |
|------|----------|------|
| 1 | 新增输入字段 | 新增16个完整输入字段定义（必填/可选/来源/描述）|
| 2 | 新增处理流程 | 新增6步处理流程：批次准备→分发清单生成→分发登记→费用结算→退换处理→汇总归档 |
| 3 | 新增验收标准 | 新增8条Given/When/Then格式AC验收标准 |
| 4 | 新增业务规则 | 新增4类业务规则（豁免识别、退换限制、30天审批、电子收据联动）|

**二、RBAC权限矛盾修正（🟡重要修复）：**

| 序号 | 变更类型 | 描述 |
|------|----------|------|
| 1 | 拆分权限行 | 将「费用收取」拆分为「费用收取」+「费用查询」两行 |
| 2 | 明确TEACHER权限 | 费用收取：❌（无操作权限）；费用查询：✅（本班学生费用状态，仅状态标签不显示金额）|
| 3 | 明确PARENT权限 | 费用查询：✅（子女缴费记录，含完整明细）|
| 4 | 新增安全注释 | 增加🔒RBAC权限澄清说明，明确教师不可操作收费界面，仅可查看学生费用状态标签 |

**三、新增AC验收标准（7个关键函数）：**

| 函数 | 新增AC数量 | 说明 |
|------|-----------|------|
| F-ATT-001 | 8条 | 出勤数据拉取/同步失败/异常检测/批量撤销时限 |
| F-LEAVE-001 | 6条 | 请假提交/OCR核验/审批进度/跟进提醒 |
| F-BUS-001 | 6条 | GPS追踪/延误通知阈值/到校确认/家长通知 |
| F-FIN-001 | 4条 | 资助豁免/逾期升级/申诉处理/对账报表 |
| F-USER-001 | 7条 | 账户生命周期/家长密码/多子女绑定 |
| F-AUTO-002 | 5条 | 消息送达回执/短信备用/token健康检查/升级机制 |
| F-NEW-003 | 5条 | DSE成绩入库/覆核/家长查询/升学追踪 |
| F-NEW-004 | 5条 | 预算编制/审批/超支预警/科目调整 |
| F-NEW-006 | 5条 | 健康检查/告警/第三方接口/备份状态 |

**四、跨模块数据流转自洽验证：**

| 验证项 | 来源模块 | 目标模块 | 验证结论 |
|--------|---------|---------|---------|
| 每日收费→学费管理 | MOD-DAILY-001 (F-FEE-001) | MOD-FIN-001 (F-FIN-001) | ✅ 数据流清晰：每日收款记录→月度汇总→学费管理年度对账 |
| 编班→班级归属→出勤范围 | MOD-CYCL-001 (F-ENRL-002) | MOD-USER-001 (F-USER-003 ABAC) | ✅ 编班结果写入学生主档→ABAC按class_id控制教师数据范围 |
| 运维告警→审计日志→合规 | MOD-OPS-001 (F-OPS-005~009) | MOD-USER-001 (F-USER-005) | ✅ 全部运维告警事件→审计日志记录→F-OPS-005完整性监控 |

**文档更新：**
- SPEC-COMPLETE.md 版本保持：v1.7.0（版本号不变，Changelog记录本次Patch修复）
- archive/SPEC-SCHOOL-ADMIN-v1.7.0.md 已归档（修复前版本快照）

---

### v1.7.0 — 2026-06-06 | 运维催生P0功能优化需求纳入

**变更类型：** Minor

**变更者：** 系统架构团队

**评审依据：** 运维催生功能优化需求清单 v1.0.0（2026-06-06）P0优先级清单（10项）

**一、新增功能（9项）：**

| 序号 | 变更类型 | 位置 | 描述 |
|------|----------|------|------|
| 1 | 新增 (Add) | Module 11（新增章节） | F-OPS-001 数据库健康检查与 WAL 积压自动处理（P0）：db-health-check.sh，8项检查项，结构化 JSON 输出，集成 Prometheus；WAL 积压 > 500MB 自动触发 wal-archive-speedup.sh |
| 2 | 新增 (Add) | Module 11（新增章节） | F-OPS-002 SSL 证书到期自动续期（P0）：certbot renew 集成 Cron，Vault 证书状态管理，30/7/1 天分级告警 |
| 3 | 新增 (Add) | Module 11（新增章节） | F-OPS-003 WebSAMS Token 自动刷新（P0）：websams-token-refresh.sh，每小时检查 Token，< 24h 自动刷新，Vault 存储 |
| 4 | 新增 (Add) | Module 11（新增章节） | F-OPS-004 一键灾难恢复脚本（P0）：disaster-recovery.sh，L1–L3 九步自动化串联，RTO ≤ 30 分钟，标准格式演练报告 |
| 5 | 新增 (Add) | Module 11（新增章节） | F-OPS-005 审计日志写入完整性监控（P0）：audit_log_writer_errors_total 指标，5分钟抽样校验，写入成功率 > 99.9% 保障 |
| 6 | 新增 (Add) | Module 11（新增章节） | F-OPS-006 Coze API 配额实时监控（P0）：coze_api_quota_usage_percent 指标，三级告警，> 95% 自动限流保护 |
| 7 | 新增 (Add) | Module 11（新增章节） | F-OPS-007 敏感字段查看频率告警（P0）：sensitive-field-view-monitor.sh，每5分钟执行，四类敏感字段分级阈值，PDPO 合规 |
| 8 | 新增 (Add) | Module 11（新增章节） | F-OPS-008 数据库 DDL 操作审计（P0）：pgaudit 扩展启用，ddl_audit_logs 表，DROP/TRUNCATE 实时告警 |
| 9 | 新增 (Add) | Module 11（新增章节） | F-OPS-009 运维健康仪表板（P0）：Ops Health Dashboard，整合全部10项 P0 运维监控点，一键诊断功能；F-NEW-006 由 P2 升级为 P0 |

**二、新增监控指标（20项 Prometheus 指标）：**

| 指标名称 | 类型 | 说明 | 对应函数 |
|----------|------|------|---------|
| `db_health_status` | Gauge | 数据库整体健康状态 | F-OPS-001 |
| `db_connection_usage_percent` | Gauge | 数据库连接使用率 | F-OPS-001 |
| `pgbouncer_pool_usage_percent` | Gauge | PgBouncer 连接池使用率 | F-OPS-001 |
| `wal_archive_backlog_mb` | Gauge | WAL 积压量（MB）| F-OPS-001 |
| `cert_days_until_expiry` | Gauge | 证书到期剩余天数 | F-OPS-002 |
| `websams_token_remaining_hours` | Gauge | WebSAMS Token 剩余小时数 | F-OPS-003 |
| `audit_log_writer_errors_total` | Counter | 审计日志写入失败次数 | F-OPS-005 |
| `audit_log_writer_success_total` | Counter | 审计日志写入成功次数 | F-OPS-005 |
| `audit_log_write_latency_seconds` | Histogram | 审计日志写入延迟 | F-OPS-005 |
| `audit_log_sample_verify_errors_total` | Counter | 抽样校验发现丢失记录数 | F-OPS-005 |
| `coze_api_quota_used` | Gauge | Coze API 已使用配额 | F-OPS-006 |
| `coze_api_quota_limit` | Gauge | Coze API 配额上限 | F-OPS-006 |
| `coze_api_quota_usage_percent` | Gauge | Coze API 配额使用率 | F-OPS-006 |
| `coze_api_rate_limited` | Gauge | AI 请求限流状态 | F-OPS-006 |
| `sensitive_field_access_count` | Counter | 敏感字段访问次数 | F-OPS-007 |
| `sensitive_field_access_alerts_total` | Counter | 敏感字段访问告警次数 | F-OPS-007 |
| `ddl_operations_total` | Counter | DDL 操作总次数 | F-OPS-008 |
| `ddl_critical_alerts_total` | Counter | DDL 告警次数 | F-OPS-008 |
| `dr_recovery_duration_seconds` | Gauge | 灾难恢复耗时 | F-OPS-004 |
| `dr_rto_achieved` | Gauge | RTO 是否达标 | F-OPS-004 |

**三、模块与函数统计更新：**

| 指标 | v1.6.0 | v1.7.0 | 变化 |
|------|--------|--------|------|
| 功能函数总数 | 55 | **65** | +10 |
| 模块总数 | 9 | **12** | +3（Module 11 + Module 12）|
| P0功能数 | 约36 | **约45** | +9 |

**四、附录更新：**

- 新增 Module 11（第十二部分）：运维自动化与监控体系（F-OPS-001 至 F-OPS-009，9个函数）
- 新增 Module 12（第十三部分）：长期规划功能清单（v1.7+，P1/P2优先级16项）
- 更新跨模块总结表：功能总数55→65，模块9→12
- 更新模块依赖关系图：新增 MOD-OPS-001 依赖关系
- 新增 Prometheus 监控指标汇总表（12.2节）
- archive/SPEC-SCHOOL-ADMIN-v1.6.0.md 已归档

**文档更新：**
- SPEC-COMPLETE.md 版本：v1.6.0 → **v1.7.0**
- 运维需求来源：ops/运维催生功能优化需求.md v1.0.0

---

### v1.6.0 — 2026-06-06 | 评审报告全面修订

**变更类型：** Minor

**变更者：** 系统架构团队

**评审依据：** `REVIEW-FUNC-001` 功能评审报告（2026-06-06，15项缺漏需求 + 7项不合理业务点）

**一、新增功能（6项）：**

| 序号 | 变更类型 | 位置 | 描述 |
|------|----------|------|------|
| 1 | 新增 (Add) | Module 9（新增章节） | F-NEW-001 一键灾难恢复与业务连续性（P0）：RTO≤4h、RPO≤1h，L1-L4四级恢复策略，自动诊断与短信通知 |
| 2 | 新增 (Add) | Module 9（新增章节） | F-NEW-002 多渠道通知模板管理（P1）：微信/短信/邮件统一模板，变量替换，差异化渠道配置 |
| 3 | 新增 (Add) | Module 9（新增章节） | F-NEW-003 DSE放榜成绩追踪（P1）：HKEAA SDP对接，放榜自动入库，升学去向统计 |
| 4 | 新增 (Add) | Module 9（新增章节） | F-NEW-004 年度预算编制与执行追踪（P1）：8类预算科目，实时执行率，超支预警 |
| 5 | 新增 (Add) | Module 9（新增章节） | F-NEW-005 自定义报表生成与定时推送（P1）：拖拽式报表生成，6类预置模板，定时推送 |
| 6 | 新增 (Add) | Module 9（新增章节） | F-NEW-006 系统健康检查与自动诊断（P2）：9维度健康检查，CRITICAL/ERROR/WARNING三级告警，自动诊断报告 |

**二、业务逻辑修正（7项）：**

| 序号 | 变更类型 | 位置 | 描述 |
|------|----------|------|------|
| 1 | 修正 (Modify) | F-ATT-001 | 批量撤销时限：5分钟 → 15分钟（评审报告业务逻辑不合理#1） |
| 2 | 修正 (Modify) | F-LEAVE-001 | 病假自动批准：增加AI辅助核验+随机抽查机制（评审报告业务逻辑不合理#2） |
| 3 | 修正 (Modify) | F-FIN-002 | 零用现金限额：由固定HK$3,000改为动态限额（CPI挂钩，公式调整）（评审报告业务逻辑不合理#3） |
| 4 | 修正 (Modify) | F-BUS-001 | 延误通知阈值：增加可配置阈值（默认>10分钟触发通知，>20分钟SMS备用）（评审报告业务逻辑不合理#4） |
| 5 | 修正 (Modify) | F-USER-003 | ABAC规则：由概念性描述升级为Open Policy Agent (OPA)规则引擎实现（评审报告权限设计缺陷#1） |
| 6 | 修正 (Modify) | F-USER-007 | 权限审批：所有敏感权限变更须二次认证（短信OTP或硬件Token）（评审报告权限设计缺陷#2） |
| 7 | 修正 (Modify) | F-EXAM-004 | 成绩撤回：撤回后自动触发审计告警推送给校务主任（评审报告业务逻辑不合理#3） |

**三、语言支持修正（1项）：**

| 序号 | 变更类型 | 位置 | 描述 |
|------|----------|------|------|
| 1 | 修正 (Modify) | F-I18N-001 | 新增粤语口语界面(zh-HK-Cantonese)：家长/学生门户支持粤语口语语气（用户模拟反馈D-08） |

**四、模块与函数统计更新：**

| 指标 | v1.5.0 | v1.6.0 | 变化 |
|------|--------|--------|------|
| 功能函数总数 | 49 | **55** | +6 |
| 模块总数 | 8 | **9** | +1（新增Module 9）|
| P0功能数 | 约35 | **36** | +1（F-NEW-001）|

**五、附录更新：**

- 新增 Module 9（第十部分）：新增功能规格（F-NEW-001 至 F-NEW-006）
- 新增 Module 10（第十一部分）：长期规划功能清单（v1.6+）
- 更新跨模块总结表：功能总数49→55，模块8→9
- 更新模块依赖关系图：新增 MOD-NEW-001 依赖关系

**文档更新：**
- SPEC-COMPLETE.md 版本：v1.5.0 → **v1.6.0**
- archive/SPEC-SCHOOL-ADMIN-v1.5.0.md 已归档

---

### v1.5.0 — 2026-06-05 | 用户模拟反馈整合

**变更类型：** Minor
**变更者：** 系统架构团队
**参考文档：** `USER-SIMULATION-FEEDBACK.md` v1.0.0（48条反馈，2026-06-05审阅）

**新增/增强内容：**

| 序号 | 变更类型 | 位置 | 描述 |
|------|----------|------|------|
| 1 | 增强 (Enhance) | F-USER-003 | 新增「敏感字段界面展示策略」：身份证号/电话/地址脱敏规则，默认A123456(X)格式，点击需二次确认；新增「家校通讯功能」：教师-家长系统内消息通道 |
| 2 | 增强 (Enhance) | F-USER-005 | 新增「家长通话记录规范」：必填字段（通话时长、下次跟进日期、家长情绪）；新增「敏感字段查看审计」类型 |
| 3 | 增强 (Enhance) | F-USER-007 | 新增权限审批「证明文件」要求：5类敏感权限操作均须上传支持文档（PDF/JPG/PNG，≤10MB）|
| 4 | 增强 (Enhance) | F-USER-001 | 新增家长首次注册强制设置「家长密码」规则，用于多子女账号关联验证 |
| 5 | 增强 (Enhance) | F-USER-006 | 新增家长密码（独立于登录密码）支持短信OTP找回 |
| 6 | 增强 (Enhance) | F-DASH-001 | 新增仪表板自定义功能：组件显示/隐藏、拖拽排序、紧急程度筛选 |
| 7 | 增强 (Enhance) | F-ATT-001 | 新增数据源独立状态展示（不合并为"部分成功"）；新增「受影响学生智能列表」；新增批量录入「确认预览页」+「5分钟内批量撤销」 |
| 8 | 增强 (Enhance) | F-INQ-001 | 新增通话时长、家长满意度等可选字段（电话渠道）|
| 9 | 增强 (Enhance) | F-LUNCH-001 | 新增家长自助午膳变更功能（微信门户）；新增更改截止时间规则（默认14:00）；新增截止前1小时提醒通知 |
| 10 | 增强 (Enhance) | F-BUS-001 | 新增「乘搭学生列表」+ 一键通知；新增延误通知「意外类型」标签；新增下一站预计到达时间 |
| 11 | 增强 (Enhance) | F-LEAVE-001 | 新增代课老师推荐显示当日已有课时数；新增跟进提醒设置；新增OCR进度提示；新增请假申请「审批进度状态条」 |
| 12 | 增强 (Enhance) | F-FEE-001 | 新增电子收据自动推送（App/邮件）；新增缴费延迟提示（显示「预计5分钟内更新」+上次缴费时间）|
| 13 | 增强 (Enhance) | F-FIN-001 | 新增欠费 sub_status 枚举（installment_plan / overdue / disputed）|
| 14 | 增强 (Enhance) | F-FIN-002 | 新增OCR金额视觉区分（黄色高亮 + 粗体红色复核提示 + 收据侧栏缩略图）；新增双人见证实时任务推送（见证人1完成自动通知见证人2）|
| 15 | 增强 (Enhance) | F-EXAM-004 | 新增教师成绩提交后48小时内可自行撤回修改；新增家长成绩单PDF导出；新增班级成绩分布柱状图 |
| 16 | 增强 (Enhance) | F-ADM-002 | 新增推荐信AI字数统计（建议300-500字）+ AI写作大纲建议 |
| 17 | 增强 (Enhance) | F-AUTO-002 | 新增消息送达回执机制；新增高优先级通知短信备用渠道；新增Token健康检查（每24小时）|

**合规更新：**
- PDPO合规：敏感字段脱敏策略（身份证号、电话、地址）已纳入RBAC+界面展示双重保护
- 财务合规：双人见证流程增加实时协同推送，避免操作不同步
- 审计合规：家长通话记录规范化、通话时长必填

**文档更新：**
- SPEC-COMPLETE.md 版本：v1.4.0 → v1.5.0
- USER-SIMULATION-FEEDBACK.md 版本：v1.0.0 → v1.0.0（已审阅）

---

### v1.3.0 — 2026-05-25 | 多语言支持系统添加

**变更类型：** Minor
**变更者：** 系统架构团队

**新增内容：**

- **Module 8: 多语言支持系统 (MOD-I18N-001)**
  - F-I18N-001: 多语言框架与翻译管理（P0）
    - 统一 i18n 框架，支持 zh-HK / zh-CN / en 三种语言
    - 翻译资源文件结构、翻译键注册表、术语表
  - F-I18N-002: 语言检测与自动切换（P0）
    - 5 级语言检测优先级（用户偏好 → URL参数 → 浏览器语言 → IP地理 → 默认）
  - F-I18N-003: 实时内容翻译 LLM（P1）
    - 基于 Coze/OpenAI 的上下文感知翻译，术语表集成，24h 缓存
  - F-I18N-004: 区域化与格式本地化（P1）
    - 日期/货币/数字格式本地化，香港/内地/英文格式标准

**依赖更新：**
- 新增 MOD-I18N-001 → 所有业务模块（横向依赖）
- 更新模块依赖图

**文档更新：**
- 功能函数总数：45 → 49
- 涵盖模块：7 → 8

---

### v1.4.0 — 2026-06-03 | 数据库架构设计与数据字典

**变更类型：** Minor
**变更者：** 系统架构团队

**新增内容：**

- **数据库架构设计文档 (DB-SCHEMA.md)**
  - 设计原则：3NF规范化、Audit First、Soft Delete、多租户、UUID主键、JSONB扩展
  - 命名规范：表/字段/索引/约束命名规则
  - ER实体关系图：7大模块60+表关系总览
  - 表结构定义：
    - 核心管理表 (3表)：schools, departments, academic_years
    - 用户权限表 (11表)：users, user_roles, permissions, sessions, audit_logs等
    - 学生班级表 (8表)：students, classes, teachers, parents等
    - 每日操作表 (12表)：attendance_records, lunch_orders, bus_tracking等
    - 周期性校务表 (4表)：enrollment_records, textbooks, exam_registrations等
    - 财务资产表 (12表)：fee_assessments, petty_cash, assets, vendors等
    - 系统表 (7表)：reminders, ai_knowledge_base, translations等
  - 索引设计：针对高频查询的索引策略
  - 外键关系：完整的实体关系映射

- **数据字典 (DATA-DICTIONARY.md)**
  - 核心实体数据定义：学校、用户、学生、班级字段详细说明
  - 枚举值规范：user_type, status, fee_type等所有枚举定义
  - 业务规则：格式校验、范围校验、唯一性、引用完整性
  - 数据质量规则：香港身份证格式、邮箱格式、电话格式等
  - 敏感数据处理：加密策略（AES-256）、哈希策略（bcrypt/argon2）
  - 数据保留政策：审计日志7年、学生记录离校后7年、会话30天等

**文档更新：**
- 新增配套文档：DB-SCHEMA.md（~37KB）、DATA-DICTIONARY.md（~10KB）
- SPEC-COMPLETE.md 版本更新：v1.3.0 → v1.4.0

---

### v1.2.0 — 2026-05-24 | 文档版本管理系统添加

**变更类型：** Minor
**变更者：** 系统架构团队

| 序号 | 变更类型 | 位置 | 描述 |
|------|----------|------|------|
| 1 | 新增 (Add) | 文档头部 | 新增「文档版本信息」元数据表：版本号 v1.2.0、文档状态、维护人、审批人 |
| 2 | 新增 (Add) | 文档头部 | 新增「版本修订规则」：SemVer 格式（Major.Minor.Patch）、变更粒度规则、审批要求、分支策略 |
| 3 | 新增 (Add) | 附录 A | 新增「文档版本管理」附录：变更日志总表（A.1）、版本历史详情（A.2）、变更类型说明（A.3）、评审审批记录（A.4）、文件管理规范（A.5）|
| 4 | 新增 (Add) | 附录 A.2 | 新增「版本历史详情」：每个版本附带具体变更序号、类型、位置和描述 |
| 5 | 修正 (Patch) | 整体结构 | Module 4（AI助理）→ Module 7，Module 5（整合合规）→ Module 8；相应章节重新编号 |
| 6 | 修正 (Patch) | 跨模块总结表 | 功能函数总数：38 → 45；模块依赖图中 MOD-USER-001 作为基础依赖层 |

**新增功能函数：** 无（纯文档管理改进）

---

### v1.1.0 — 2026-05-24 | 用户与权限管理系统添加

**变更类型：** Minor
**变更者：** 系统架构团队

| 序号 | 变更类型 | 位置 | 描述 |
|------|----------|------|------|
| 1 | 新增 (Add) | 第六部分（全新章节）| 新增 Module 6 — 用户与权限管理系统（MOD-USER-001，P0），作为所有业务模块的基础依赖层 |
| 2 | 新增 (Add) | Module 6 | F-USER-001 用户生命周期管理：用户注册/停用/角色分配/账户过期/离职自动处理/家长多子女绑定 |
| 3 | 新增 (Add) | Module 6 | F-USER-002 身份认证：密码/SSO/OTP/飞书OAuth/MFA，含登录风险检测与密码策略 |
| 4 | 新增 (Add) | Module 6 | F-USER-003 功能授权（RBAC+ABAC）：6种角色×功能模块权限矩阵、班级/子女/字段级数据范围控制 |
| 5 | 新增 (Add) | Module 6 | F-USER-004 会话与Token管理：Access/Refresh Token、并发会话限制（最多3个）、异地登录告警、空闲超时、Token主动作废 |
| 6 | 新增 (Add) | Module 6 | F-USER-005 审计日志与登录记录：8类登录事件、4类敏感操作、3–7年保留策略、SIEM集成 |
| 7 | 新增 (Add) | Module 6 | F-USER-006 密码与凭证重置：邮箱/短信OTP自助重置（15分钟有效）、管理员代重置（双验证）|
| 8 | 新增 (Add) | Module 6 | F-USER-007 权限变更审批流程：5类高风险权限操作需审批链，含有效期限制 |
| 9 | 更新 (Modify) | 跨模块总结表 | 新增7行功能记录；功能函数总数：38 → 45 |
| 10 | 更新 (Modify) | 模块依赖关系图 | 所有业务模块新增对 MOD-USER-001 的依赖 |

**新增功能函数：**
```
F-USER-001  用户生命周期管理       P0
F-USER-002  身份认证               P0
F-USER-003  功能授权 (RBAC+ABAC)   P0
F-USER-004  会话与Token管理         P0
F-USER-005  审计日志与登录记录      P0
F-USER-006  密码与凭证重置          P0
F-USER-007  权限变更审批流程        P1
```

---

### v1.0.0 — 2026-05-23 | 初始版本发布

**变更类型：** Major
**变更者：** 系统架构团队

| 序号 | 变更类型 | 位置 | 描述 |
|------|----------|------|------|
| 1 | 新增 (Add) | 第一部分 | 需求规格说明书：系统概述、4种用户角色（校务主任/校务处同工/教师/家长学生）、三大工作分类 |
| 2 | 新增 (Add) | 第二部分 | 系统架构设计：三层技术架构（用户界面/AI核心/集成数据）、核心实体关系（ER图）|
| 3 | 新增 (Add) | 第三部分 | Module 1 — 每日晨检仪表板（9个功能）：出勤统计、迟到早退记录、家长查询队列、快速回复模板（40+模板）、午膳订单汇总、校车实时追踪（GPS）、校车点名记录、请假申请处理、每日收费追踪 |
| 4 | 新增 (Add) | 第四部分 | Module 2 — 周期性校务管理（11个功能）：新生注册、AI辅助编班（6因素权重）、课本分发、DSE报考管理、试卷管理（6子流程）、特别考试安排（6类类型）、成绩单生成发布、中一自行分配学位、JUPAS联招管理、档案清理销毁（EDB保存期限）、学年财务结算 |
| 5 | 新增 (Add) | 第五部分 | Module 3 — 财务及资产管理（7个功能）：学费管理（7类费用）、零用现金报销（双人见证）、奖学金津贴申请（4类）、校产条码盘点（10类资产）、场地租借管理（5类场地）、设备保养管理（4类保养）、供应商注册评估（10类）|
| 6 | 新增 (Add) | 第七部分 | Module 4 — AI助理及自动化（5个功能）：NLU意图分类（12类意图树）、FAQ智能匹配（4维评分算法）、周期性任务触发器（YAML配置）、智能提醒系统（4级升级）、OCR文档识别（5类证件，准确率目标）|
| 7 | 新增 (Add) | 第八部分 | Module 5 — 整合及合规（6个功能）：WebSAMS数据同步（4种模式）、eClass API集成（7端点）、PDPO隐私合规检查（P1/P2/P3分类）、双人见证流程（5类触发）、审计日志管理（4类事件）、自动备份管理（4级策略）|
| 8 | 新增 (Add) | 跨模块总结 | 功能总表（38个函数）、模块依赖关系图（5模块依赖链）|

**初始功能函数：**
```
MOD-DAILY-001  [9]  F-ATT-001/002, F-INQ-001/002, F-LUNCH-001,
                   F-BUS-001/002, F-LEAVE-001, F-FEE-001
MOD-CYCL-001   [11] F-ENRL-001/002/003, F-EXAM-001/002/003/004,
                   F-ADM-001/002, F-YREND-001/002
MOD-FIN-001    [7]  F-FIN-001/002/003, F-ASSET-001/002/003, F-VEND-001
MOD-AI-001     [5]  F-AI-001/002/003, F-AUTO-001/002
MOD-INT-001    [6]  F-INT-001/002, F-COMP-001/002/003, F-BACK-001
───────────────────────────────────────────────────────────────────
Total: 38 functions across 5 modules
```

---

## A.3 变更类型说明 (Change Type Definitions)

| 变更类型代码 | 中文名称 | 说明 |
|------------|----------|------|
| `Major` | 重大版本 | 架构性重构、模块级变更，与前一版本不兼容 |
| `Minor` | 次要版本 | 新增功能、模块扩展，向后兼容 |
| `Patch` | 修订版本 | 文档修正、格式调整，不影响功能内容 |
| `Add` | 新增 | 在文档中添加新的内容（函数、模块、章节）|
| `Modify` | 修改 | 对现有内容进行更新、补充或调整 |
| `Delete` | 删除 | 从文档中移除内容（标记为废弃）|
| `Deprecate` | 弃用 | 标记某功能/模块为过时，保留记录但不推荐使用 |

---

## A.4 文档评审与审批记录 (Review & Approval Records)

| 版本 | 评审日期 | 评审人 | 角色 | 评审结果 | 备注 |
|------|----------|--------|------|----------|------|
| **v1.7.1** | 2026-06-09 | 系统架构团队 | 系统架构团队 | 需求文档补充 | UI设计+AC验收标准+ABAC示例补充 |
| v1.7.0-Patch1 | 2026-06-06 | 系统架构团队 | 系统架构团队 | 质检报告修复 | F-ENRL-003补全/RBAC修正/7个函数AC新增/跨模块数据流验证 |
| v1.7.0 | 2026-06-06 | 系统架构团队 | 系统架构团队 | 依据运维催生功能优化需求清单 v1.0.0 修订 | 新增9项运维自动化功能/新增20项监控指标 |
| v1.6.0 | 2026-06-06 | 系统架构团队 | 系统架构团队 | 依据功能评审报告 REVIEW-FUNC-001 修订 | 新增6项功能/修正7项业务点 |
| v1.5.0 | 2026-06-05 | （待填）| 项目经理 | （待审批）| 用户模拟反馈整合 |
| v1.0.0 | 2026-05-23 | （待填）| 项目经理 | （待审批）| 初始版本提交 |

> **注：** 实际使用时，请将「（待填）/（待审批）」替换为实际评审人姓名及结果。

---

## A.5 文件管理规范 (File Management)

| 规范项 | 说明 |
|--------|------|
| **工作文件** | `SPEC-COMPLETE.md` 始终为最新可用版本（**可编辑**）|
| **归档位置** | `/docs/school-admin-system/archive/`（**只读快照**，禁止直接编辑）|
| **归档命名** | `SPEC-<项目>-v<Major>.<Minor>.<Patch>.md`，如 `SPEC-SCHOOL-ADMIN-v1.2.0.md` |
| **版本快照** | 每次正式发布须将当前文件复制一份至 `archive/`，文件名含完整版本号 |
| **保留策略** | 保留最近 **3 个正式版本**（Major + Minor）；超出移至 `archive/legacy/`；Patch 可覆盖删除；Major **永久保留** |
| **Git 集成** | 工作区受 `.gitignore` 保护，Git Tag 流程待后续集成（如迁移至独立代码仓库）|
| **分支策略** | 长期变更使用 `spec/<功能描述>` 命名分支（如 `spec/user-auth-module`），完成后合并至 `SPEC-COMPLETE.md` |
| **PR 要求** | 跨模块或 Minor 及以上变更须附带 `附录 A.2` 格式的变更说明，评审通过后方可合并 |
| **审阅要求** | Minor 及以上须 1 名评审人；Major 须 2 名评审人 |
| **争议处理** | 同一模块有冲突变更时，以最新 Changelog 记录为准，并通知相关评审人 |
| **发布检查清单** | 每次正式发布前须确认：Changelog 更新 ✓、版本号更新 ✓、快照已存至 archive ✓、审批记录已填 ✓ |

### 操作流程摘要

> 📝 编辑变更  →  更新 `SPEC-COMPLETE.md`
> 📋 记录变更  →  填写 `附录 A.1` + `A.2` + `A.4`
> 💾 归档快照  →  复制为 `archive/SPEC-...-v<X.Y.Z>.md`
> 🔢 更新版本号 →  文档头部 + `A.1` 表头
> ✅ 评审审批   →  `A.4` 填写评审人及结果
> 📢 通知相关方  →  校务主任 / 项目经理

---

**文档结束**


#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | DSE放榜日（8月12日），HKEAA发布成绩 | 系统自动接收HKEAA SDP数据文件 | 系统自动解析并入库，175名学生的DSE成绩记录入库，5名未到账学生标记为pending |
| AC-02 | 某学生成绩有误，申请覆核 | 学校收到HKEAA覆核通知 | 系统更新该生成绩记录，生成新的个人成绩单，注明「覆核后成绩」|
| AC-03 | 放榜后，家长在微信门户查看成绩 | 家长登录微信，点击「DSE成绩」| 显示该生各科成绩、等级、与全校平均分对比、升学建议 |
| AC-04 | JUPAS申请状态自动更新 | HKEAA发布各轮联招结果 | 系统自动接收更新，142名学生状态从submitted更新为first_round_offers |
| AC-05 | 学年结束，需生成年度升学去向报告 | 校务主任请求生成升学报告 | 系统生成匿名化统计报告：各大学录取人数、各专业分布，学校留存备查 |



#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 每日08:00定时健康检查执行 | 无人为操作，系统自动执行 | 系统生成健康报告，显示：overall_status=healthy，各组件（数据库/API/AI）状态正常 |
| AC-02 | 数据库连接池使用率达92%（告警阈值85%）| 健康检查发现异常 | 状态=warning，建议「数据库连接池接近上限，建议检查慢查询」|
| AC-03 | 微信Access Token剩余有效期6小时（告警阈值24小时）| Token健康检查执行 | 状态=warning，recommendation=「Token即将过期，建议立即刷新」，自动触发刷新流程 |
| AC-04 | WebSAMS API可用率跌至96%（告警阈值98%）| 第三方接口检查 | 状态=error，告警推送至系统管理员（App+短信）|
| AC-05 | 备份任务昨日未成功完成 | 每日01:00备份任务执行后检查 | 状态=error，告警立即推送至系统管理员和校务主任 |

---

## 第十六部分：Module 15 — 学生档案管理模块 (v1.9.0 新增，P0)

> **Module 15 (Student Profile Management / v1.9.0)** 是基于 Issue #194 用户反馈根本性设计错误而新增的核心模块。
> 核心变更：学生档案（`students` 表）与系统用户（`users` 表）完全分离，学号按 `YYYYNNNN` 格式自动生成，班级分配按学年动态管理。

### 16.1 模块概述

| 属性 | 描述 |
|------|------|
| 模块名称 | Student Profile Management — 学生档案管理模块 |
| 模块ID | MOD-STU-001 |
| 优先级 | **P0（根本性设计错误，必须立即修复）** |
| 用户 | 校务处同工, 校务主任 |
| 评审依据 | Issue #194 — 学生管理模块根本性重构（2026-07-02）|
| 关联模块 | MOD-USER-001（用户管理），MOD-CLASS-001（班级管理），MOD-CYCL-001（周期性校务）|

### 16.2 设计背景与问题

**根本性设计错误（Issue #194）：**
- 旧设计将学生档案混用于 `users` 表（`role='student'`），包含 `username/password` 字段
- 学生档案是**业务数据**（姓名、学号、性别、出生日期等），不应与系统用户账户混淆
- 正确的设计应使用独立的 `students` 表存储学生档案数据

**正确设计原则：**
```
students (学生档案表 - 业务数据)
├── id (UUID) - 学生档案唯一标识
├── student_id (VARCHAR) - 学号，格式 YYYYNNNN，自动生成，不可修改
├── name_zh (VARCHAR) - 中文姓名
├── name_en (VARCHAR) - 英文姓名
├── gender (ENUM) - 性别
├── birth_date (DATE) - 出生日期
├── address (TEXT) - 家庭地址
├── phone (VARCHAR) - 联系电话
├── email (VARCHAR) - 邮箱
├── admission_date (DATE) - 入学日期
├── status (ENUM) - 在校/毕业/退学
└── created_at/updated_at/deleted_at

users (系统用户表 - 技术账户)
├── id (UUID) - 用户唯一标识
├── username (VARCHAR) - 登录账号（可为学号或独立账号）
├── password (VARCHAR) - 登录密码
├── role (ENUM) - 用户角色
└── student_profile_id (FK→students.id, 可选) - 关联学生档案（学生/家长角色）
```

### 16.3 数据模型设计

#### 核心实体关系

```
┌─────────────┐       ┌──────────────────────┐       ┌─────────────────┐
│  students   │◄─────►│  class_allocations    │◄─────►│    classes      │
│ (学生档案)  │       │    (班级分配)         │       │    (班级)       │
└──────┬──────┘       └──────────────────────┘       └─────────────────┘
       │                       │
       ▼                       ▼
┌─────────────┐       ┌──────────────────────┐
│    users    │       │ student_id_sequences │
│ (系统用户)  │       │    (学号序列表)       │
└─────────────┘       └──────────────────────┘
```

### Function F-STU-001: 学生档案 CRUD

**目的：** 管理学生档案的全生命周期，包括创建、查询、更新、软删除

**学号自动生成规则：**
- 格式：`YYYYNNNN`（YYYY=入学年份，NNNN=4位序号，0001-9999）
- 按入学年份分组，每年从 0001 开始递增
- 自动分配，创建时无需输入，确认后不可修改

**输入：**
| 字段 | 类型 | 必填 | 来源 | 说明 |
|------|------|------|------|------|
| name_zh | String | 是 | 用户输入 | 中文姓名 |
| name_en | String | 可选 | 用户输入 | 英文姓名 |
| gender | Enum | 是 | 用户输入 | male/female/other |
| birth_date | Date | 是 | 用户输入 | 出生日期 |
| address | Text | 可选 | 用户输入 | 家庭地址 |
| phone | String | 可选 | 用户输入 | 联系电话 |
| email | String | 可选 | 用户输入 | 邮箱 |
| admission_date | Date | 是 | 用户输入 | 入学日期 |
| guardian_name | String | 可选 | 用户输入 | 监护人姓名 |
| guardian_phone | String | 可选 | 用户输入 | 监护人电话 |
| guardian_relationship | String | 可选 | 用户输入 | 监护人关系 |
| emergency_contact | String | 可选 | 用户输入 | 紧急联系人 |
| emergency_phone | String | 可选 | 用户输入 | 紧急联系电话 |
| notes | Text | 可选 | 用户输入 | 备注 |

**业务规则：**
- 学号按 `YYYYNNNN` 格式自动生成，不可手动输入，不可修改
- 创建学生档案时，系统自动生成对应的 `users` 记录（`role='student'`），或与已有用户关联
- 软删除时不物理删除，保留 `deleted_at` 时间戳
- 学生状态：active（在校）、graduated（毕业）、withdrawn（退学）、transferred（转学）

**输出：**
```json
{
  "id": "uuid",
  "student_id": "2026000001",
  "name_zh": "王小明",
  "name_en": "WONG SIU MING",
  "gender": "male",
  "birth_date": "2011-03-15",
  "address": "香港仔田灣大樓A座12樓",
  "phone": "91234567",
  "email": "parent@example.com",
  "admission_date": "2026-09-01",
  "status": "active",
  "guardian_name": "王大明",
  "guardian_phone": "91234568",
  "guardian_relationship": "父亲",
  "emergency_contact": "王小華",
  "emergency_phone": "91234569",
  "notes": "",
  "created_at": "2026-07-02T10:00:00+08:00",
  "updated_at": "2026-07-02T10:00:00+08:00",
  "current_class": {
    "class_id": "uuid",
    "class_name": "1A",
    "academic_year": "2026-2027"
  }
}
```

#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 2026年9月新学期，添加新生王小明 | 校务处同工填写学生基本信息，点击「保存」| 系统自动生成学号 `2026000001`（2026年第1个入学学生），学生档案创建成功，不要求输入 username/password |
| AC-02 | 添加第123个2026年入学新生 | 校务处同工填写信息，保存 | 系统生成学号 `2026000123` |
| AC-03 | 某学生需更新家庭地址 | 校务处同工编辑学生档案，修改地址 | 系统更新地址，student_id 保持不变 |
| AC-04 | 某学生退学 | 校务处同工将学生状态改为 withdrawn | 学生档案保留，status=withdrawn，学号保留但不重复使用 |
| AC-05 | 新建学生后查看列表 | 保存成功后返回列表页 | 新学生立即显示在列表中（不刷新），学号、姓名、性别、当前班级均正确显示 |
| AC-06 | 班级下拉框显示 | 添加学生页面加载 | 班级下拉框显示当前学年（2026-2027）的所有班级，无空值 |

### Function F-STU-002: 班级分配管理（按学年）

**目的：** 管理学生对班级的分配关系，支持按学年动态调整

**输入：**
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| student_id | UUID | 是 | 学生档案ID |
| class_id | UUID | 是 | 班级ID |
| academic_year_id | UUID | 是 | 学年ID |
| allocation_type | Enum | 是 | main（主班）/ elective（选修）/ temporary（临时）|
| effective_date | Date | 是 | 生效日期 |
| end_date | Date | 可选 | 结束日期（为空表示当前学年有效）|

**业务规则：**
- 每个学生在同一学年内只能有一个 `allocation_type='main'` 的主班分配
- 学年结束时，旧学年分配自动过期，新学年需重新分配
- 班级分配与学生档案分离，同一学生可跨学年查看分配历史

**输出：**
```json
{
  "allocation_id": "uuid",
  "student_id": "uuid",
  "student_id_display": "2026000001",
  "student_name": "王小明",
  "class_id": "uuid",
  "class_name": "1A",
  "academic_year": "2026-2027",
  "allocation_type": "main",
  "effective_date": "2026-09-01",
  "end_date": null,
  "status": "active"
}
```

#### 验收标准 (AC)

| # | Given（前置条件）| When（操作）| Then（预期结果）|
|---|----------------|------------|----------------|
| AC-01 | 新生王小明入读1A班 | 创建学生档案后，分配班级 | 系统生成 class_allocation 记录，allocation_type=main |
| AC-02 | 学年末，1A班升为2A | 学年切换，重新分配班级 | 王小明新学年分配到2A，旧分配自动过期 |
| AC-03 | 某学生转班 | 校务处同工更新班级分配 | 系统保留原分配记录（标记end_date），新增新分配记录 |
| AC-04 | 查询某学生历史班级 | 查看学生详情 | 显示该生历次班级分配记录（学年+班级+类型）|

### Function F-STU-003: 学生-用户关联

**目的：** 建立学生档案与系统用户的关联关系

**关联规则：**
- 创建学生档案时，可选择创建关联的 `users` 记录（role='student'）
- 也可将学生档案关联到已有的 `users` 记录
- 家长用户（role='parent'）通过 `parent_student_links` 表与学生档案关联

---

## 第十七部分：Module 16 — QR Code 校园签到考勤模块 (v2.0.0-draft.1 新增，P1)

> **Module 16 (QR Code Campus Attendance Check-in / v2.0.0-draft.1)** 是基于校园入口场景新增的智能签到考勤模块，以动态 QR Code 扫码替代传统刷卡/人工签到方式，提升通行效率与考勤数据实时性。
> 核心场景：学生入校时展示动态 QR Code 供教职员工或闸机扫码，自动记录考勤时间，支持离线容灾与防重复签到。

### 17.1 模块概述

| 属性 | 描述 |
|------|------|
| 模块名称 | QR Code Campus Attendance Check-in — QR Code 校园签到考勤模块 |
| 模块ID | MOD-ATT-QR-001 |
| 优先级 | **P1（核心补充功能，预计 v2.0.0 发布）** |
| 用户 | 学生（展示QR码）、教职员工（扫码记录）、班主任（查看日报表）、校务处（全局管理） |
| 评审依据 | CR-20260714-001 需求规格新增 |
| 关联模块 | MOD-STU-001（学生档案），MOD-USER-001（用户管理），MOD-CLASS-001（班级管理），MOD-ATT-001（考勤管理参考）|

### 17.2 设计背景与目标

**现状问题：**
- 传统校园考勤依赖实体 IC 卡刷卡或人工签字，存在卡片丢失、代刷、统计滞后等问题
- 2000+ 学生的考勤数据无法实时汇总，班主任需手动统计迟到/缺勤
- 早高峰期校门口闸机通行速度慢（刷卡→识别→开门平均 5-10 秒/人）

**核心目标：**
1. 将入校通行时间压缩至 2-3 秒/人（展示二维码 + 扫码）
2. 考勤数据实时更新，班主任可在系统即时查看当日到校情况
3. 替换物理卡片成本（补卡、发卡、消磁等维护工作）
4. 防代签、防伪造、防重复签到
5. 支持离线场景（校园网络不稳定时仍可正常运作）

### 17.3 需求规格

#### Function F-ATTQR-001: 动态 QR Code 生成

**目的：** 为学生生成短期有效的动态 QR Code，用于入口签到考勤

**输入：**
| 字段 | 类型 | 必填 | 来源 | 说明 |
|------|------|------|------|------|
| student_id | UUID | 是 | 系统自动 | 学生档案ID |
| device_id | String | 否 | 客户端上报 | 学生端设备标识（可选，用于安全风控）|
| session_token | String | 是 | 系统生成 | OAuth/token 会话标识 |

**QR 码编码规则：**
| 字段 | 格式 | 长度 | 说明 |
|------|------|------|------|
| 协议标识 | `SCHQR` | 5字节 | 固定前缀，标识校园QR码 |
| 版本号 | `01` | 2字节 | QR码协议版本 |
| 时间戳 | `YYYYMMDDHHMMSS` | 14字节 | 生成时间（UTC+8） |
| 学生标识 | `student_id` UUID | 36字节 | 学生唯一标识 |
| 随机数 | 16位十六进制 | 16字节 | 防重放攻击随机因子 |
| 密钥签名 | SHA256 完整哈希 | 32字节 | 服务端私钥签名，防篡改 |
| **总计** | | **≤ 105字节** | 编码为 Base64 后约 140 字符 |

**编码输出格式：**
```
SCHQR|01|YYYYMMDDHHMMSS|UUID|RANDOM|SIGNATURE
```
最终编码为 QR Code 时使用 `Byte mode`，纠错等级 **M (15%)**，版本自动适配以适应约 140 字符内容。

**业务规则：**
- QR Code 有效期：**30 秒**（从生成时间算起，过期后自动失效）
- 每生成新码时上次生成的码自动失效（一人一码，一次有效）
- 服务端校验：解析 QR 内容 → 验证签名 → 检查有效期 → 检查是否已被使用
- 签名密钥每日轮换，由服务端密钥管理系统管理

**输出：**
```json
{
  "qr_code_content": "SCHQR|01|20260714080000|uuid-xxx|a1b2c3d4e5f6g7h8|signature_bytes_base64",
  "qr_code_image_url": "https://attendance-api.school.edu/v1/qr/generate-image?data=...",
  "expires_at": "2026-07-14T08:00:30+08:00",
  "generated_at": "2026-07-14T08:00:00+08:00"
}
```

#### 验收标准 (AC) — F-ATTQR-001

| # | Given（前置条件） | When（操作） | Then（预期结果） |
|---|----------------|------------|----------------|
| AC-01 | 学生进入校门，打开签到页面 | 系统调用 QR 生成接口 | 返回有效 QR Code，过期时间 = 生成时间 + 30秒 |
| AC-02 | QR 码生成后超过 30 秒 | 扫码枪扫描旧码 | 服务器返回 410 Gone，提示"二维码已过期" |
| AC-03 | 签名密钥每日轮换 | 生成新 QR 码 | 使用当日最新密钥签名，旧密钥签名的码视为无效 |
| AC-04 | QR 码内容包含 UUID | 攻击者尝试篡改内容 | 签名验证失败，返回 403 禁止访问 |

#### Function F-ATTQR-002: 扫码签到（教职员工/闸机）

**目的：** 教职员工手持终端或闸机扫码枪扫描学生 QR Code，完成考勤记录

**输入：**
| 字段 | 类型 | 必填 | 来源 | 说明 |
|------|------|------|------|------|
| qr_raw_content | String | 是 | 扫码设备 | 从 QR Code 解码出的原始字符串 |
| scanner_device_id | String | 是 | 扫码设备 | 扫码终端的设备标识 |
| scanner_location | String | 是 | 扫码设备 | 扫码位置（如：正门/侧门/地下入口） |
| scanned_at | DateTime | 是 | 扫码设备 | 设备本地时间（用于离线校验） |

**业务规则：**
- **防重复签到（核心安全约束）：** 同一 QR Code 的唯一签名组合只能被扫码一次，第二次返回 409 Conflict
- **时间窗口防刷：** 同学生在 **5 分钟窗口期**内，即使生成新 QR 码也只能记一次签到（防止一分钟多次刷码）
- **离线容灾：** 当闸机检测到网络中断时，自动进入**离线模式**：扫码记录本地缓存（SQLite 本地数据库或文件存储），网络恢复后批量同步至服务端
- 教职员工的扫码操作本身也被记录为**操作审计日志**

**离线模式数据格式：**
```json
{
  "cache_id": "UUID",
  "qr_content_hashed": "SHA256",
  "qr_student_id": "uuid",
  "scanner_device_id": "string",
  "scanner_location": "string",
  "scanned_at_local": "2026-07-14T08:01:00+08:00",
  "cached_at": "2026-07-14T08:01:00+08:00",
  "synced": false
}
```

**输出（在线模式）：**
```json
{
  "attendance_id": "uuid",
  "student_id": "uuid",
  "student_name": "王小明",
  "student_class": "1A",
  "check_in_time": "2026-07-14T08:00:15+08:00",
  "location": "正门",
  "scanner_device": "gate-01",
  "status": "on_time",
  "message": "签到成功"
}
```

#### 验收标准 (AC) — F-ATTQR-002

| # | Given（前置条件） | When（操作） | Then（预期结果） |
|---|----------------|------------|----------------|
| AC-01 | 学生展示有效 QR Code | 教职员工扫码枪扫描 | 系统记录签到成功，状态 on_time |
| AC-02 | 同一 QR Code 被连续扫码两次 | 第二次扫码请求 | 返回 409 Conflict，提示"该二维码已被使用" |
| AC-03 | 学生在 5 分钟内连续生成并展示新码 | 第二次被扫码 | 返回 429 Too Many Requests，提示"请勿重复签到" |
| AC-04 | 闸机网络断线 | 扫码后在本地缓存 | SQLite 缓存条目 synced=false，本地存储可用空间 > 100MB |
| AC-05 | 闸机网络恢复 | 自动触发批量同步 | 所有 synced=false 的记录上传至服务端，synced 标记为 true |

#### Function F-ATTQR-003: 日报表推送

**目的：** 每日定时向班主任推送班级考勤日报

**业务规则：**
- 每日 08:30 自动生成当日截至当前的签到日报
- 迟到判定标准：08:00 之后签到记为迟到（可配置）
- 缺勤判定：08:30 仍未签到且无提前请假记录的学生记为缺勤
- 推送渠道：系统通知、飞书/钉钉消息、Web 后台仪表板
- 日报内容：应到人数、已签到人数、迟到人数、缺勤人数、未签到学生名单

**输出：**
```json
{
  "report_date": "2026-07-14",
  "class": "1A",
  "total_students": 32,
  "checked_in": 28,
  "late": 2,
  "absent": 2,
  "leave_approved": 1,
  "unchecked_students": [
    {"student_id": "uuid", "name": "李四", "status": "absent"},
    {"student_id": "uuid", "name": "张三", "status": "absent"}
  ]
}
```

#### Function F-ATTQR-004: 家长考勤查询

**目的：** 支持多家长绑定同一学生，家长可通过门户查看子女的签到记录

**业务规则：**
- 支持多家长绑定（通过 `parent_student_links` 关联表）
- 家长可查看：当日签到状态、历史签到记录（最近 30 天）、迟到/缺勤统计
- 家长不可修改签到记录（仅读权限）
- 数据隔离：家长只能查看自己关联学生的数据

### 17.4 学生端展示渠道选型

| 选型维度 | 原生 App (Native) | 移动 Web (Web Portal / PWA) |
|----------|-----------------|---------------------------|
| **QR 码渲染性能** | ★★★★★ 原生渲染，快速稳定 | ★★★★☆ Web Canvas 渲染，低端机有延迟 |
| **离线能力** | ★★★★★ 本地缓存+离线QR生成 | ★★★☆☆ PWA Service Worker 有限离线 |
| **摄像头调用** | ★★★★★ 原生 API 无缝 | ★★★★☆ WebRTC 调用，需 HTTPS + 用户授权 |
| **自动刷新** | ★★★★★ 后台定时器精确刷新 | ★★★★☆ JavaScript setInterval 受标签页活跃度影响 |
| **部署成本** | ★★☆☆☆ App Store / 应用商店审核 | ★★★★★ 零部署，浏览器访问即用 |
| **更新频率** | ★★☆☆☆ 需发版更新 | ★★★★★ 服务端更新即生效 |
| **设备兼容性** | ★★★★☆ 需 iOS/Android 双端开发 | ★★★★★ 跨平台，无需双端开发 |
| **安全性** | ★★★★★ 沙箱环境+Fingerprint | ★★★★☆ HTTPS + 浏览器安全模型 |
| **开发维护成本** | ★☆☆☆☆ 高（双平台+发版） | ★★★★★ 低（一套代码） |

**选型结论：推荐使用 Web Portal（PWA）方案**

**选型理由：**
1. **部署零门槛**：学生无需安装 App，打开浏览器即可使用，特别适合 2000+ 学生规模的 K-12 学校（学生手机可能无足够存储空间或家长不允许安装额外 App）
2. **成本优势**：无需 iOS/Android 双端开发团队，单一 Web 代码库降低 70%+ 开发维护成本
3. **QR 码刷新**：通过 Service Worker + Web Workers 实现后台定时刷新，配合高频（2 秒）setInterval 确保 30 秒有效期内的二维码始终可用
4. **离线补偿**：PWA 的 Service Worker 可缓存初始页面，QR 码实时刷新仍需在线（与闸机离线模式互补）
5. **渐进增强**：先以 Web Portal 上线，后续可按需升级为 App（留原生 API 接口）

### 17.5 核心约束与安全要求

| 维度 | 约束 |
|------|------|
| **QR 码有效期** | 30 秒（短期有效，过期自动失效） |
| **QR 编码格式** | 见 F-ATTQR-001 编码规则（协议前缀 + 版本 + 时间戳 + UUID + 随机数 + 签名） |
| **签名算法** | HMAC-SHA256，密钥每日轮换 |
| **防重放** | 随机数 16 字节 + 一次性使用标记 |
| **防重复** | 同一学生 5 分钟窗口内仅记一次签到 |
| **传输安全** | 全链路 HTTPS，QR 内容仅服务端完整解析 |
| **离线缓存加密** | 闸机本地缓存用 AES-256 加密存储 |
| **扫码设备认证** | 每个扫码终端需注册并分配 API Key，未经注册的设备扫码请求被拒绝 |
| **日志审计** | 所有 QR 生成、扫码、同步操作记入 `attendance_audit_log` 表 |

---

## 第十八部分：Module 17 — 学生&家长门户权限与菜单访问控制模块 (v2.0.0-draft.1 新增，P1)

> **Module 17 (Student & Parent Portal Access Control / v2.0.0-draft.1)** 是基于自主门户场景新增的权限与菜单控制模块，为 Student/Parent 角色提供差异化的 Web 门户体验。
> 核心目标：学生和家长可通过门户查看个人信息、提交电子请假、管理联系信息，并确保严格的数据隔离与权限控制。

### 18.1 模块概述

| 属性 | 描述 |
|------|------|
| 模块名称 | Student & Parent Portal Access Control — 学生&家长门户权限与菜单访问控制模块 |
| 模块ID | MOD-PORTAL-AC-001 |
| 优先级 | **P1（核心补充功能，预计 v2.0.0 发布）** |
| 用户 | 学生（Student）、家长（Parent）、校务处（后台管理） |
| 评审依据 | CR-20260714-001 需求规格新增 |
| 关联模块 | MOD-STU-001（学生档案），MOD-USER-001（用户管理），MOD-ATT-QR-001（QR签到），MOD-ATT-001（考勤管理）|

### 18.2 设计背景与目标

**现状问题：**
- 当前系统仅支持校务处和教师角色，无学生/家长自主门户
- 纸质请假条流程效率低，审批状态不可追溯
- 家长无法在线查看子女信息（成绩、考勤、作业等）
- 学生档案联系方式变更需线下提交申请

**核心目标：**
1. 建立 Student/Parent 角色分离的自主门户
2. 实现角色差异化菜单与功能权限控制
3. 提供个人档案查看 + 有限自修改能力
4. 提供电子请假全流程（提交→审批→历史追溯）
5. 严格数据隔离：家长只能查看自己关联的孩子信息

### 18.3 角色差异化菜单与权限集

#### 授权菜单模块一览

| 菜单模块 | Student | Parent | 管理员 | 说明 |
|---------|---------|--------|--------|------|
| 个人档案 | ✅ 查看 | ✅ 查看 | ✅ 管理 | Student 可修改联系方式等；Parent 仅读 |
| 我的QR码签入 | ✅ 使用 | ❌ | ❌ | Student-only：生成入校QR码 |
| 电子请假 | ✅ 提交+查看 | ✅ 查看+代提交 | ✅ 审批 | 双方均可查看，Student 主提交 |
| 考勤记录 | ✅ 查看本人 | ✅ 查看关联子女 | ✅ 管理 | Parent 只能看自己孩子的 |
| 成绩查询 | ✅ 查看本人 | ✅ 查看关联子女 | ✅ 管理 | 数据隔离同考勤规则 |
| 课表查询 | ✅ 查看 | ❌ | ✅ 管理 | Student-only 课表 |
| 校历/通告 | ✅ 查看 | ✅ 查看 | ✅ 管理 | 全校通告，双方共享 |
| 校内缴费 | ❌ | ✅ 操作 | ✅ 管理 | Parent 负责缴费操作 |
| 请假审批 | ❌ | ❌ | ✅ 操作 | 管理员/班主任权限 |
| 系统用户管理 | ❌ | ❌ | ✅ 管理 | 管理员禁止菜单 |
| 班级管理 | ❌ | ❌ | ✅ 管理 | 管理员禁止菜单 |
| 财务审计 | ❌ | ❌ | ✅ 管理 | 管理员禁止菜单 |

> **分类规则：**
> - **Student-only 菜单**：我的QR码签入、课表查询（与学生身份强相关）
> - **Parent shared 菜单**：个人档案查看、考勤记录、成绩查询、校历/通告
> - **Parent exclusive 菜单**：校内缴费（家长负责缴费）
> - **管理员禁止菜单**：系统用户管理、班级管理、财务审计等后台管理功能不对学生/家长开放

### 18.4 RBAC 权限设计

#### Role: Student (student)
| 权限标识 | 权限名称 | 范围 | 说明 |
|---------|---------|------|------|
| `profile:view:self` | 查看个人档案 | Self | 查看自己的学生档案 |
| `profile:update:self` | 有限修改个人信息 | Self | 仅可修改联系方式、紧急联系人、地址 |
| `attendance:view:self` | 查看本人考勤 | Self | 查看个人考勤记录 |
| `attendance:qr:generate` | 生成QR签到码 | Self | 使用 QR Code 功能 |
| `leave:create:self` | 提交请假 | Self | 创建请假申请 |
| `leave:view:self` | 查看请假记录 | Self | 查看自己的请假历史和审批状态 |
| `leave:cancel:self` | 撤回请假 | Self | 在审批完成前可撤回 |
| `grade:view:self` | 查看本人成绩 | Self | 查看个人成绩 |
| `timetable:view:self` | 查看课表 | Self | 查看个人课表 |
| `notice:view` | 查看校历/通告 | Global | 查看学校发布的通告 |

#### Role: Parent (parent)
| 权限标识 | 权限名称 | 范围 | 说明 |
|---------|---------|------|------|
| `profile:view:linked_children` | 查看关联子女档案 | Children | 查看关联的学生档案（只读） |
| `attendance:view:linked_children` | 查看关联子女考勤 | Children | 查看签到考勤记录 |
| `leave:view:linked_children` | 查看子女请假记录 | Children | 查看请假审批状态 |
| `leave:create:linked_children` | 代子女提交请假 | Children | 为低龄子女代操作 |
| `grade:view:linked_children` | 查看子女成绩 | Children | 查看成绩 |
| `payment:operate:linked_children` | 校内缴费 | Children | Parent 专属缴费权限 |
| `notice:view` | 查看校历/通告 | Global | 与 student 共享 |
| `emergency:update:linked_children` | 更新子女紧急联系方式 | Children | 紧急情况下可更新联系方式 |

### 18.5 功能规格

#### Function F-PORTAL-001: 个人档案查看与有限自修改

**目的：** 学生和家长可以查看个人/子女档案，学生可有限修改部分字段

**允许学生修改的字段（可编辑）：**
| 字段 | 类型 | 验证规则 |
|------|------|---------|
| phone | String | 香港手机号格式 (8位数字) |
| email | String | 标准邮箱格式 |
| address | Text | 最多 200 字符 |
| emergency_contact | String | 必填，中文姓名 |
| emergency_phone | String | 香港手机号格式 (8位数字) |
| guardian_phone | String | 香港手机号格式 |

**锁定不可编辑字段：**
| 字段 | 原因 |
|------|------|
| student_id (学号) | 入学自动生成，不可修改 |
| name_zh / name_en (姓名) | 身份标识，修改需校务处审核 |
| gender (性别) | 核心身份信息 |
| birth_date (出生日期) | 核心身份信息 |
| class / grade (班级/年级) | 由校务处统一管理 |
| admission_date (入学日期) | 系统记录不可修改 |
| admission_number (入学编号) | 系统唯一编号 |

**业务规则：**
- 学生修改可编辑字段后，系统自动记录变更历史（见审计日志要求）
- 修改即时生效，无需审批
- 家长端为只读视图，不可编辑子女档案字段（`emergency:update` 权限除外）

**验收标准 (AC)：**

| # | Given（前置条件） | When（操作） | Then（预期结果） |
|---|----------------|------------|----------------|
| AC-01 | 学生登录门户 | 访问「个人档案」页面 | 显示完整档案信息，学号、姓名、班级等字段为灰色只读状态 |
| AC-02 | 学生「个人档案」页面 | 尝试编辑学号字段 | 编辑框不可操作（disabled），鼠标悬停提示「此字段不可修改」 |
| AC-03 | 学生「个人档案」页面 | 编辑手机号并保存 | 保存成功，档案更新，记录审计日志 |
| AC-04 | 家长登录门户 | 访问「子女档案」页面 | 全部字段为只读，无编辑按钮 |

#### Function F-PORTAL-002: 电子请假

**目的：** 学生/家长在线提交请假申请，查看审批状态与历史记录

**输入（请假申请）：**
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| student_id | UUID | 是 | 请假学生 |
| leave_type | Enum | 是 | sick（病假）/ personal（事假）/ family（家庭假）/ other（其他）|
| start_date | Date | 是 | 请假起始日期 |
| end_date | Date | 是 | 请假结束日期 |
| reason | Text | 是 | 请假原因 |
| attachments | File[] | 可选 | 证明文件（医生证明等）|
| contact_phone | String | 可选 | 请假期间联系方式 |
| submitter_role | Enum | 是 | student / parent（标识提交人角色）|

**业务规则：**
- 请假天数 ≤ 3 天：班主任审批，自动通知
- 请假天数 > 3 天：班主任初审 → 校务主任终审
- 审批状态流转：`pending` → `approved` / `rejected` / `cancelled`
- 已提交的请假若状态为 `pending`，学生/家长可**撤回**（`cancelled`）
- 已审批的请假不可撤回，需联系校务处

**学生端操作流程：**
1. 学生登录门户 → 点击「电子请假」→ 填写请假信息 → 提交
2. 系统展示提交成功提示，请假状态为 `pending`
3. 班主任/校务主任审批后，学生收到系统通知
4. 学生可在「请假记录」页面查看所有历史申请及当前状态

**验收标准 (AC)：**

| # | Given（前置条件） | When（操作） | Then（预期结果） |
|---|----------------|------------|----------------|
| AC-01 | 学生填写完整请假信息（病假 2 天）| 点击提交 | 系统创建 leave 记录，状态 pending，发送通知给班主任 |
| AC-02 | 班主任批准请假 | 审批通过 | 系统更新状态为 approved，通知学生/家长 |
| AC-03 | 学生提交的病假单仍在 pending | 学生点击撤回 | 状态变为 cancelled，通知班主任 |
| AC-04 | 学生提交的病假单已被 approved | 学生尝试撤回 | 按钮不可用，提示「已审批不可撤回，请联系校务处」| 
| AC-05 | 请假超过 3 天（5 天事假）| 班主任初审通过 | 状态变为 pending_principal，触发校务主任二审 |
| AC-06 | 家长代子女提交请假 | 家长操作提交 | 系统记录 submitter_role=parent，与 student 提交的同样处理流程 |

#### Function F-PORTAL-003: 数据隔离规则

**目的：** 确保家长只能查看自己关联子女的数据，不能越权访问其他学生信息

**隔离实现机制：**
- 基于 `parent_student_links` 表建立家长-学生关联
- 所有学生相关数据的查询（考勤、成绩、请假等）必须 JOIN 该关联表进行权限过滤
- 后端 API 层统一拦截：在 Repository/Service 层自动追加 `WHERE student_id IN (parent_student_links.children_ids)`
- 前端不依赖前端路由隐藏实现安全性（后端二次验证为强制防线）

**后端查询示例：**
```sql
-- 家长查询子女考勤记录
SELECT a.* FROM attendance a
JOIN parent_student_links psl 
  ON a.student_id = psl.student_id 
  AND psl.parent_user_id = :current_user_id
WHERE a.created_at >= :start_date AND a.created_at <= :end_date;
```

**验收标准 (AC)：**

| # | Given（前置条件） | When（操作） | Then（预期结果） |
|---|----------------|------------|----------------|
| AC-01 | 家长 A 关联学生 X、Y | 登录门户后查看「考勤记录」| 仅显示学生 X 和 Y 的记录，无其他学生信息 |
| AC-02 | 家长尝试直接调用 API 查询学生 Z 的考勤 | 发送 GET /api/attendance?student_id=Z-ID | 返回 403 Forbidden，日志记录越权尝试 |
| AC-03 | 前端通过 URL 参数尝试访问非关联学生页面 | 发起请求 | 后端拦截返回 403，前端提示「无权访问」|

### 18.6 数据安全需求

#### 敏感字段掩码

| 字段 | 掩码规则 | 示例（原始→掩码） |
|------|---------|----------------|
| 手机号 | 显示后 4 位，前 4 位掩码 | `91234567` → `****4567` |
| 邮箱 | 显示域名 + 用户名首字符 | `wong.siu.ming@example.com` → `w***@example.com` |
| 地址 | 显示到街道，门牌号掩码 | `香港仔田灣大樓A座12樓` → `香港仔田灣大樓A座****` |
| 紧急联系人电话 | 显示后 4 位 | `91234568` → `****4568` |
| 学生身份证号 | 显示前 6 位+后 2 位 | `A123456(7)` → `A1234***(7)` |

**掩码规则适用范围：**
- Student 查看本人档案：基本掩码（地址、电话部分掩码）
- Parent 查看子女档案：完整掩码（所有敏感字段掩码显示）
- 校务处/教师：完整显示（内部操作权限）
- 系统日志中的敏感字段脱敏后再记录

#### 审计日志

**所有学生/家长操作必须记录：**

| 审计事件 | 记录内容 | 保存期限 |
|---------|---------|---------|
| 学生修改联系方式 | 修改人、修改时间、旧值→新值（脱敏） | ≥ 2 年 |
| 学生/家长提交请假 | 提交人、学生、请假类型、时间 | ≥ 3 年 |
| 请假审批操作 | 审批人、审批结果、审批意见、时间 | ≥ 3 年 |
| 家长查看子女敏感数据 | 查看人、查看的学生ID、查看时间 | ≥ 1 年 |
| 越权访问尝试 | 尝试者、目标资源、时间、来源 IP | ≥ 1 年 |
| QR 码签到失败（安全相关） | 学生ID、失败原因、时间 | ≥ 6 个月 |

**审计日志格式规范：**
```json
{
  "audit_id": "uuid",
  "event_type": "PROFILE_UPDATE",
  "actor_id": "uuid",
  "actor_role": "student",
  "target_id": "uuid",
  "target_type": "students",
  "action": "UPDATE",
  "changes": [
    {"field": "phone", "old": "****4567", "new": "****7890"},
    {"field": "address", "old": "香港仔田灣****", "new": "灣仔軒尼詩道****"}
  ],
  "ip_address": "192.168.1.100",
  "user_agent": "Mozilla/5.0...",
  "created_at": "2026-07-14T10:00:00+08:00",
  "result": "SUCCESS"
}
```

### 18.7 门户访问与认证

| 需求 | 说明 |
|------|------|
| 登录方式 | 学号/家长账号 + 密码（支持飞书 SSO 可选集成） |
| 首次登录 | 强制修改初始密码 |
| 密码策略 | 最少 8 位，含大小写字母 + 数字，90 天过期 |
| Session 管理 | JWT Token，有效期 24 小时，支持多设备登录 |
| 退出机制 | 主动退出后 Token 立即失效，同一账号可在「已登录设备」管理页面下线其他设备 |
| 家长多重关联 | 一个家长账号可关联多个子女（同一家庭的多名孩子）| 

---

**文档结束**

