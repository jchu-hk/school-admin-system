...
### 核心原则
**所有输出都必须经过独立验收，不能自产自验**

| 角色 | 输出 | 验收人 | 验收标准 |
|------|------|--------|----------|
| **DEV** | 代码 | QA | 功能测试、回归测试 |
| **ARCH** | 设计文档 | CHECKER | 架构合理性、完整性 |
| **REQ** | 需求文档 | CHECKER | 需求清晰性、可行性 |
| **CHECKER** | 审查报告 | PM | 审查质量、标准符合度 |
| **QA** | 测试报告 | PM | 测试覆盖、缺陷记录 |
| **DEVOPS** | 部署环境 | OPS | 环境稳定性、可用性 |
| **OPS** | 监控报告 | PM | 报告完整性、准确性 |

### 验收流程

```
角色输出 → 独立验收人审查 → 通过/失败 → 如需整改则返回原角色修改
                ↓
        CHECKER独立验收（工业商业标准）
        - 代码质量审查
        - 架构合理性审查
...
- ✅ **测试账号变更时** → 更新账号信息
- ✅ **新功能模块上线时** → 更新模块完成度
- ✅ **Dashboard URL变更时** → 更新看板链接

**不需要放的内容**:
- ❌ 进行中的工作状态 (GitHub Issues可查)
- ❌ 实时统计数据 (GitHub可查)
- ❌ 临时信息 (HEARTBEAT.md记录)
- ❌ Agent详细状态 (Dashboard实时显示)

**更新流程**:
```
1. PM检查更新触发条件
2. DEVOPS/OPS提供最新环境信息
3. PM更新PROJECT-WIKI.md
4. PM创建Wiki更新Issue (Documentation label)
5. CHECKER审查Wiki更新完整性
6. PM提交commit并推送
7. 验证GitHub显示正确
```
...
**示例Issue**: 
```
Title: Update PROJECT-WIKI.md - v1.5.7 release
Labels: documentation
Body:
- Update version to v1.5.7
- Update release date
- Add changelog link
- Update test environment URL
```
